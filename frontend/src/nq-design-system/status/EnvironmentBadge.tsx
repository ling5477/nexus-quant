...
export function EnvironmentBadge({ env, size = 'md' }: EnvironmentBadgeProps) {
  ...
  const liveStyle: CSSProperties = { background: color, color: '#0b0b0b', border: `1px solid ${color}`, fontWeight: 600 };
  const normalStyle: CSSProperties = { background: 'transparent', color, border: `1px solid ${color}` };
