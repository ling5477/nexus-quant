...
- 当前唯一动作是 `NQ-GATEY-6D-COMMIT-AND-PUSH`；只允许提交已通过独立安全评审且尚未实际物化的 exact pilot scope/prerequisite capability，不授权补造输入、创建 `ExecutionIntent`、PLACE、CANCEL、mutation、micro-live、第一笔真实订单或 LIVE，exact transition 以 [STATUS.md](docs/current/STATUS.md) 为准。
