...
- NQ-GATEO-O5-MANUAL-PUBLIC-OUTBOUND-SMOKE-PLAN = COMPLETED（已完成）/ PLAN ONLY（仅规划）/ NOT IMPLEMENTED（未实现）；O-5A plan review 已 PASS / ACCEPTED；O-5B execution 经只读核对为 BLOCKED / MANUAL RUNNER NOT BOUND / NOT EXECUTED；O-5 smoke execution 仍 NOT STARTED；O-5D DataOrigin.PUBLIC_OUTBOUND decision、O-FREEZE 均 NOT STARTED；GateO stage 仍 NOT COMPLETED
- NQ-GATEO-O5B-RUNNER-BINDING-PLAN = PASS（通过）/ ACCEPTED（已接受）；新增 O-5B runner binding planning-only 文档，推荐后续只实现 test-only manual JUnit entry，绑定 manual profile、feature flag、allowlist/denylist、redacted evidence 和默认不运行策略；本轮不实现 runner，不执行真实 HTTP，不读取 credential，不改代码/CI/API/migration；runner implementation ALLOWED / TEST-ONLY MANUAL ENTRY PREFERRED
