...
- NQ-GATEO-O4-MARKETDATA-QUALITY-UI-PLAN = PASS（通过）/ PLAN ONLY（仅规划）/ NOT IMPLEMENTED（未实现）；新增 `docs/current/NQ_GATEO_O4_MARKETDATA_QUALITY_UI_PLAN.md`，规划现有 `/marketdata` 页面优先复用、`GET /api/marketdata/readiness` 数据消费、source health / freshness / gap / error / dataOrigin 展示、null 字段“暂无稳定事实”规则、no-trading 文案和 O-4A..O-4E 批次；未实现页面，未改 frontend / backend / API / migration / tests，未执行 O-5 manual public outbound smoke
...
- `docs/current/NQ_GATEO_O4_MARKETDATA_QUALITY_UI_PLAN.md`：GateO O-4 MarketData Quality UI planning-only 与 O-4A review 状态入口；记录 `/marketdata` 页面复用建议、readiness API 字段映射、状态/文案/测试策略、no-trading 边界和 O-4A..O-4E 批次
