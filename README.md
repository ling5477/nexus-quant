...
- NQ-GATEO-O3E-MARKETDATA-READINESS-API-FREEZE-REVIEW = PASS（通过）/ ACCEPTED（已接受）/ FROZEN（已冻结）；冻结 commit `7a42ca03 feat(marketdata): extend readiness API read model` 中既有 `GET /api/marketdata/readiness` read-only response baseline；P0/P1=0；scoped Maven 与后端全量 Maven 均 PASS / BUILD SUCCESS；GateO stage 仍 NOT COMPLETED，O-4 plan 已 PASS / PLAN ONLY / NOT IMPLEMENTED，O-4A review 已 PASS / ACCEPTED，O-4B read-only UI implementation 已 ALLOWED / NOT STARTED，O-5 / O-FREEZE 仍 NOT STARTED，public marketdata readiness 不等于 trading authorization
- NQ-GATEO-O4-MARKETDATA-QUALITY-UI-PLAN = PASS（通过）/ PLAN ONLY（仅规划）/ NOT IMPLEMENTED（未实现）；O-4B read-only UI implementation = IMPLEMENTED（已实现）/ SELF-REVIEWED（已自查）/ READY TO COMMIT（可进入提交前复核）；已复用现有 `/marketdata` 页面，只读消费 `GET /api/marketdata/readiness`，展示 source health / freshness / gap / error / dataOrigin 和 null 字段“暂无稳定事实”规则；未改 backend / API / migration，未执行 O-5 manual public outbound smoke
...
- `docs/current/NQ_GATEO_O4_MARKETDATA_QUALITY_UI_PLAN.md`：GateO O-4 MarketData Quality UI planning-only、O-4A review 与 O-4B read-only UI implementation 状态入口；记录 `/marketdata` 页面复用、readiness API 字段映射、状态/文案/测试策略、no-trading 边界和 O-4A..O-4E 批次
