...
- 当前唯一动作是 `NQ-GATEY-6D-EXACT-PILOT-SCOPE-PREREQUISITE-MATERIALIZATION-IMPLEMENTATION`；仅允许在独立任务中物化 exact pilot scope/prerequisite，当前尚未开始，也不授权创建 `ExecutionIntent`、PLACE、CANCEL、mutation、micro-live、第一笔真实订单或 LIVE，exact transition 以 [STATUS.md](docs/current/STATUS.md) 为准。
