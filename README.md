...
- `docs/current/GATEO_PLAN.md`：GateO O-0 planning-only baseline、O-1 controlled public outbound guard frozen baseline、O-2 Data Quality Center implementation 状态与后续 O-3/O-4/O-5/O-FREEZE 计划；O-1 已 PASS / ACCEPTED / FROZEN，O-2 已 IMPLEMENTED / PENDING REVIEW，O-5 manual real public smoke 仍 NOT STARTED
- `docs/current/NQ_GATEO_O2_DATA_QUALITY_CENTER_PLAN.md`：GateO O-2 Data Quality Center implementation 状态入口；记录后端纯模型、mapper、freshness/gap/source health 规则、测试结果和 no-real / no-credential / no-trading-authorization 边界
