...
- 当前唯一动作是 `NQ-GATEY-6C-SCOPED-CREDENTIAL-IP-PRIVATE-PERMISSION-READONLY-VERIFICATION-IMPLEMENTATION`；仅允许进入 scoped credential/IP/private permission 的只读验证实现，不授权 PLACE、CANCEL、mutation、micro-live、第一笔真实订单或 LIVE，exact transition 以 [STATUS.md](docs/current/STATUS.md) 为准。
