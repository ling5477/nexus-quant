...
- NQ-GATEO-O3E-MARKETDATA-READINESS-API-FREEZE-REVIEW = PASS（通过）/ ACCEPTED（已接受）/ FROZEN（已冻结）；冻结 commit `7a42ca03 feat(marketdata): extend readiness API read model` 中既有 `GET /api/marketdata/readiness` read-only response baseline；P0/P1=0；scoped Maven 与后端全量 Maven 均 PASS / BUILD SUCCESS；GateO stage 仍 NOT COMPLETED，O-4 final status 已 FROZEN / ACCEPTED，manual smoke execution 后续为 ALLOWED / MANUAL PUBLIC READONLY ONLY / NOT EXECUTED，O-5 smoke execution / O-FREEZE 仍 NOT STARTED，public marketdata readiness 不等于 trading authorization
- NQ-GATEO-O4E-MARKETDATA-QUALITY-UI-FREEZE-REVIEW = PASS（通过）/ ACCEPTED（已接受）；冻结 commit `e62f1e43 feat(frontend): add marketdata quality readiness view` 中已提交的 `/marketdata` Quality / Readiness 只读 UI baseline；O-4 final status 为 FROZEN / ACCEPTED；O-4B read-only UI implementation 为 COMPLETED / ACCEPTED；未改 backend / API / migration，未执行 O-5 manual public outbound smoke，public marketdata readiness 不等于 trading authorization
- NQ-GATEO-O5-MANUAL-PUBLIC-OUTBOUND-SMOKE-PLAN = COMPLETED（已完成）/ PLAN ONLY（仅规划）/ NOT IMPLEMENTED（未实现）；O-5A plan review 已 PASS / ACCEPTED；O-5B-R1 runner binding implementation 已 IMPLEMENTED / SELF-REVIEWED / COMMITTED；O-5B-R2 runner binding review 已 PASS / ACCEPTED；O-5B manual smoke execution 已 COMPLETED / PENDING RESULT REVIEW；O-5C first smoke result review、O-5D DataOrigin.PUBLIC_OUTBOUND decision、O-5E freeze review、O-FREEZE 均 NOT STARTED；GateO stage 仍 NOT COMPLETED
...
- NQ-GATEO-O4-MARKETDATA-QUALITY-UI-PLAN = FROZEN（已冻结）/ ACCEPTED（已接受）；O-4B read-only UI implementation = COMPLETED（已完成）/ ACCEPTED（已接受）；已复用现有 `/marketdata` 页面，只读消费 `GET /api/marketdata/readiness`，展示 source health / freshness / gap / error / dataOrigin 和 null 字段“暂无稳定事实”规则；未改 backend / API / migration，未执行 O-5 manual public outbound smoke
...
- `docs/current/GATEO_PLAN.md`：GateO O-0 planning-only baseline、O-1 controlled public outbound guard frozen baseline、O-2 Data Quality Center frozen baseline、O-3 API plan / O-3B backend read-only API implementation / O-3E freeze review、O-4 UI freeze baseline 与 O-5 / O-FREEZE 计划；O-1 已 PASS / ACCEPTED / FROZEN，O-2 已 PASS / ACCEPTED / FROZEN，O-3 plan 已 PASS / PLAN ONLY / NOT IMPLEMENTED，O-3B 已 COMPLETED / ACCEPTED，O-3E 已 PASS / ACCEPTED / FROZEN，O-4 final status 已 FROZEN / ACCEPTED，O-4A review 已 PASS / ACCEPTED，O-4B read-only UI implementation 已 COMPLETED / ACCEPTED，O-4E freeze review 已 PASS / ACCEPTED，O-5 plan 已 COMPLETED / PLAN ONLY / NOT IMPLEMENTED，O-5B-R1 runner binding implementation 已 IMPLEMENTED / SELF-REVIEWED / COMMITTED，O-5B-R2 runner binding review 已 PASS / ACCEPTED，O-5B manual smoke execution 已 COMPLETED / PENDING RESULT REVIEW
- `docs/current/NQ_GATEO_O2_DATA_QUALITY_CENTER_PLAN.md`：GateO O-2 Data Quality Center frozen baseline 状态入口；记录后端纯模型、mapper、freshness/gap/source health 规则、测试结果、freeze review 和 no-real / no-credential / no-trading-authorization 边界
- `docs/current/NQ_GATEO_O3_MARKETDATA_RUNTIME_READINESS_API_PLAN.md`：GateO O-3 MarketData Runtime Readiness API planning、O-3B implementation 与 O-3E freeze 状态入口；记录现有 `/api/marketdata/readiness` inventory、O-2 Data Quality mapping、response 字段、安全边界、测试规划和 O-3A/O-3B/O-3C/O-3D/O-3E 批次状态
- `docs/current/NQ_GATEO_O4_MARKETDATA_QUALITY_UI_PLAN.md`：GateO O-4 MarketData Quality UI planning、O-4A review、O-4B read-only UI implementation 与 O-4E freeze review 状态入口；记录 `/marketdata` 页面复用、readiness API 字段映射、状态/文案/测试策略、no-trading 边界和 O-4 final `FROZEN / ACCEPTED` baseline
- `docs/current/NQ_GATEO_O5_MANUAL_PUBLIC_OUTBOUND_SMOKE_PLAN.md`：GateO O-5 manual public outbound smoke planning 与 O-5B execution result 状态入口；记录前置条件、manual profile、feature flag、public allowlist、private denylist、证据脱敏、失败降级、DataOrigin 后置决策和 O-5A..O-5E / O-FREEZE 路线；O-5A 已 PASS / ACCEPTED；O-5B-R1 runner binding implementation 已 IMPLEMENTED / SELF-REVIEWED / COMMITTED；O-5B-R2 runner binding review 已 PASS / ACCEPTED；O-5B manual smoke execution 已 COMPLETED / PENDING RESULT REVIEW
- `docs/current/NQ_GATEO_O5B_RUNNER_BINDING_PLAN.md`：GateO O-5B manual smoke runner binding plan、R1 implementation addendum 与 R2 review addendum；runner implementation 已绑定 test-only manual entry 并通过 R2 review，本轮不执行真实 smoke，后续只允许单独 manual public readonly smoke execution
