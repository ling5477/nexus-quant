...
- GateX：`IN PROGRESS / NOT FROZEN`（进入治理容器 / 未冻结）；GateX-0A=`IMPLEMENTED / SELF-REVIEWED`（已实现 / 已自审），尚未 commit/push，production capability 尚未完成。
- Attempt-13=`COMPLETED / ACCEPTED`; production deployment=`STOPPED`；production soak=`COMPLETED`，worker=`STOPPED`。
- 当前唯一动作是 `NQ-GATEX-0A-COMMIT-AND-PUSH`；只提交并推送已验证的 GateX-0A staged diff，不启动 GateX-0B，exact transition 以 [STATUS.md](docs/current/STATUS.md) 为准。
