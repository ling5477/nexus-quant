...
- NQ-GATEO-O3B-MARKETDATA-READINESS-READ-ONLY-API-IMPLEMENTATION = IMPLEMENTED（已实现）/ SELF-REVIEWED（已自查）/ READY TO COMMIT（可进入提交前复核）；扩展既有 `GET /api/marketdata/readiness` read model，保留旧字段并追加 O-2 Data Quality 语义字段；仍只读本地 DB facts，不新增 endpoint、migration、frontend、research、scripts、deploy、CI workflow，不真实外联，不读取 credential；O-3E freeze review 仍 NOT STARTED，public marketdata readiness 不等于 trading authorization
...
- `docs/current/NQ_GATEO_O3_MARKETDATA_RUNTIME_READINESS_API_PLAN.md`：GateO O-3 MarketData Runtime Readiness API planning 与 O-3B implementation 状态入口；记录现有 `/api/marketdata/readiness` inventory、O-2 Data Quality mapping、response 字段、安全边界、测试规划和 O-3A/O-3B/O-3C/O-3D/O-3E 批次状态
