...
- `docs/current/NQ_GATEO_O5_MANUAL_PUBLIC_OUTBOUND_SMOKE_PLAN.md`：GateO O-5 manual public outbound smoke planning-only 状态入口；记录前置条件、manual profile、feature flag、public allowlist、private denylist、证据脱敏、失败降级、DataOrigin 后置决策和 O-5A..O-5E / O-FREEZE 路线；O-5A 已 PASS / ACCEPTED；O-5B-R1 runner binding implementation 已 IMPLEMENTED / SELF-REVIEWED / READY TO COMMIT；O-5B execution 当前 BLOCKED / PENDING O-5B-R2 RUNNER BINDING REVIEW / NOT EXECUTED
- `docs/current/NQ_GATEO_O5B_RUNNER_BINDING_PLAN.md`：GateO O-5B manual smoke runner binding plan 与 R1 implementation addendum；runner implementation 已绑定 test-only manual entry，不执行真实 smoke，下一步只能 R2 runner binding review
