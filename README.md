...
- NQ-GATEO-O5-MANUAL-PUBLIC-OUTBOUND-SMOKE-PLAN = COMPLETED（已完成）/ PLAN ONLY（仅规划）/ NOT IMPLEMENTED（未实现）；新增 O-5 手动公开行情只读外联 smoke 安全计划，固化前置条件、manual profile、feature flag、public allowlist、private denylist、证据脱敏、失败降级和 O-5A..O-5E / O-FREEZE 路线；本轮不执行真实 HTTP，不改代码/CI/API/migration，不读取 credential；O-5 execution 仍 NOT STARTED，GateO stage 仍 NOT COMPLETED
...
- `docs/current/NQ_GATEO_O5_MANUAL_PUBLIC_OUTBOUND_SMOKE_PLAN.md`：GateO O-5 manual public outbound smoke planning-only 状态入口；记录前置条件、manual profile、feature flag、public allowlist、private denylist、证据脱敏、失败降级、DataOrigin 后置决策和 O-5A..O-5E / O-FREEZE 路线；O-5 execution 仍 NOT STARTED
