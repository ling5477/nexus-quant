...
class L6PgProjectionGuardTest {
    static ObjectNode sample(long at, long free, long orders, long backlog) {
        ...
        n.putObject("sources").putObject("postgres").putObject("values")
                .put("pgTmpfsCapacityBytes", L6PgCapacityContractTest.contract().capacity())
                .put("pgTmpfsUsedBytes", L6PgCapacityContractTest.contract().capacity()-free).put("pgTmpfsFreeBytes", free)
    ...
    @Test void freeEqualityContinuesOneByteBelowStopsBothAdmissionAndDispatch() throws Exception {
        var c = L6PgCapacityContractTest.contract(); var safe = new L6PgProjectionGuard(c); long need = safe.need(0, 0, 0);
        safe.observe(sample(0, need, 0, 0), 0); assertTrue(safe.producerAllowed());
        var risky = new L6PgProjectionGuard(c); risky.observe(sample(0, need-1, 0, 0), 0);
    ...
    @Test void drainUsesExistingInventoryAndBacklogOnly() {
        var c = L6PgCapacityContractTest.contract(); var guard = new L6PgProjectionGuard(c);
    ...
    @Test void singleDeltaDoesNotChangeRateButCompleteComparableWindowCanRaiseIt() {
        ...
        for (int i=0;i<=120;i++) {
            ...
            guard.observe(sample(i*10_000L, c.capacity()-used, 0, 0), i*10_000_000_000L);
    ...
    @Test void missingCadenceCapacityAndStaleSamplesFailClosed() {
        ...
        var missing = sample(0, c.capacity(), 0, 0); ((ObjectNode)missing.path("sources").path("postgres").path("values")).remove("pgTmpfsFreeBytes");
        assertThrows(IllegalStateException.class, () -> new L6PgProjectionGuard(c).observe(missing,0));
        assertThrows(IllegalStateException.class, () -> new L6PgProjectionGuard(c).observe(sample(10_000,c.capacity(),0,0),10_000_000_000L));
        assertThrows(IllegalStateException.class, () -> new L6PgProjectionGuard(c).observe(sample(0,c.capacity(),0,0),11_000_000_000L));
        var drift = sample(0,c.capacity(),0,0); ((ObjectNode)drift.path("sources").path("postgres").path("values")).put("pgTmpfsCapacityBytes",c.capacity()+1);
        assertThrows(IllegalStateException.class, () -> new L6PgProjectionGuard(c).observe(drift,0));
    }
}
