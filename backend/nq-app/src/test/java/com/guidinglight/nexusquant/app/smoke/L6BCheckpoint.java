...
final class L6BCheckpoint {
    ...
    static ObjectNode verify(Connection reader, String endpoint, List<B0Processes.Child> children,
        ...
        return verify(reader, endpoint, children, dir, phase, checkIndex, calibration, formalBudget, slots, L6BContract.MODE);
    }
    static ObjectNode verify(Connection reader, String endpoint, List<B0Processes.Child> children,
                             Path dir, String phase, int checkIndex, boolean calibration, int formalBudget,
    ...
    static ObjectNode verify(Connection reader, String endpoint, List<B0Processes.Child> children,
        ...
        Path file=dir.resolve(String.format("checkpoint-%03d.json.gz",checkIndex));
        try (var compressed = new java.util.zip.GZIPOutputStream(Files.newOutputStream(file,StandardOpenOption.CREATE_NEW))) {
            JSON.writeValue(compressed,check);
        }
        String snapshotSha=L6PgCapacityContract.hash(Files.readAllBytes(file));
        point.put("checkpointPath",file.getFileName().toString()).put("checkpointSha256",snapshotSha)
                .put("checkpointBytes",Files.size(file));
        String oracle=B0Processes.commandUntil(capacityStop,"python","-X","utf8",B0Processes.root().resolve(
                "backend/nq-app/src/test/java/com/guidinglight/nexusquant/app/smoke/l6_b_oracle.py").toString(),file.toString());
