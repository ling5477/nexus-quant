...
class L6PgCapacityFixtureTest {
    @Test void realPgScopePreflightObservationProjectionAndCleanup() throws Exception {
        var c = L6PgCapacityContractTest.contract(); var run = L6PgCapacityContractTest.run(c); var json = new ObjectMapper();
        ...
        proof.set("hostMemoryPreflight",L6HostMemoryPreflight.verify(c, run.capacity(),L6HostMemoryPreflight.observe()));
        var containers = proof.putArray("containers");
        try {
            for (boolean formal : new boolean[]{false,true}) {
                String id;
                try (var pg = formal ? B0Processes.Pg.startL6(c, run) : B0Processes.Pg.startBounded();
                    ...
                    assertEquals(formal ? run.capacity() : B0Processes.Pg.defaultTmpfsBytes(),observation.path("pgTmpfsCapacityBytes").asLong());
                    long memory = Long.parseLong(B0Processes.command("docker","inspect","--format","{{.HostConfig.Memory}}",id));
                    assertEquals(formal ? c.pgMemory(run.capacity()) : 768L*1_048_576,memory);
                    ...
                    if(formal) {
                        var guard = new L6PgProjectionGuard(c, run);
                        ...
                        assertTrue(memory >= run.capacity());
                    }
                }
