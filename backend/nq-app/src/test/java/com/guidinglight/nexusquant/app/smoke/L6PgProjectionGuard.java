...
final class L6PgProjectionGuard {
    ...
    synchronized long need(long now, long orders, long backlog) {
        L6PgCapacityContract.require(now >= 0 && now <= 3_600_000_000_000L && orders >= 0
                && orders <= contract.maximumOrders() && backlog >= 0 && backlog <= orders);
        long result = contract.reserve();
        if (stopped()) {
            ...
            return Math.addExact(result, Math.multiplyExact(backlog, contract.backlogUnit()));
        }
        if (now < 600_000_000_000L) result = Math.addExact(result, contract.growth(now, 600_000_000_000L, rates[0], -1));
        if (now < 3_000_000_000_000L) result = Math.addExact(result, contract.growth(Math.max(now, 600_000_000_000L), 3_000_000_000_000L, rates[1], -1));
        // ACTIVE尚有潜在订单；DRAIN严格只使用已存在订单库存及未完成账务的显式余量。
        result = Math.addExact(result, contract.growth(Math.max(now, 3_000_000_000_000L), 3_600_000_000_000L,
                rates[2], now < 3_000_000_000_000L ? contract.maximumOrders() : orders));
        return Math.addExact(result, Math.multiplyExact(backlog, contract.backlogUnit()));
    }
    ...
    synchronized void observe(JsonNode sample, long elapsed) {
        ...
        if (stopped()) {
            long drainNeed = need(Math.min(elapsed, 3_600_000_000_000L), orders, backlog) - contract.reserve();
