...
public interface StrategyRunRepository {
     ...
     * <p>
     * Why:
     * 执行与补偿契约 / 2.x的所有运行血缘都以 strategy_runs 为唯一事实表，因此任何 accepted trigger
     * 都必须先写入 run，再继续进入执行链。
     */
     ...
     * <p>
     * Why:
     * 执行与补偿契约的最小 dedup 语义依赖 requestId 判定“这次 schedule 命中是否已经触发过 run”。
     */
     ...
     * <p>
     * Why:
     * 执行与补偿契约只要求单实例内最小串行化，但为了避免 schedule 触发把同策略 run 挤成并发双跑，
