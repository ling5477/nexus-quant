 ...
 * <p>
 * Why:
 * 仅返回 status 不足以让调用方解释“为什么 fail-closed”。这些结构化原因区分无真实连接、
 * 禁用端点、凭证缺失、LIVE 禁用及能力禁止等边界，供交易入口、marketdata 入口与上层服务诊断。
 * 原因只解释边界，不构成真实交易授权。
 */
public enum AdapterReadinessReason {

    /** Noop / no-real stub 明确禁用，未创建真实订阅或真实执行（对应无真实连接安全边界 NO_REAL_DISABLED）。 */
    NO_REAL_DISABLED,
    ...
    /** 默认 endpoint 为 disabled:// sentinel，请求期 loud fail-closed（对应无真实连接安全边界 NETWORK_DISABLED）。 */
    ENDPOINT_DISABLED_SENTINEL,

    /** runtime credential 未配置；authenticated 请求网络前 fail-closed（对应无真实连接安全边界 CREDENTIALS_MISSING）。 */
    CREDENTIALS_MISSING,

    /** runtime credential 显式处于 unconfigured，占位不是可用 credential。 */
    CREDENTIAL_UNCONFIGURED,

    /** real provider / real permission probe 尚未实现，不能据此启用真实交易。 */
    ...
    /** LIVE 处于 DISABLED；真实交易能力 fail-closed（对应无真实连接安全边界 baseline LIVE DISABLED）。 */
    LIVE_DISABLED,
