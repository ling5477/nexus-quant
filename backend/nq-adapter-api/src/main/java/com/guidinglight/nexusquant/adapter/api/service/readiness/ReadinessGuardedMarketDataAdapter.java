package com.guidinglight.nexusquant.adapter.api.service.readiness;
...
/**
 * ReadinessGuardedMarketDataAdapter 把适配器就绪策略的 {@link AdapterReadinessService} 接到行情订阅入口。
 * <p>
 * Why:
 * 适配器就绪策略要把 service-level guard 接进真实 runtime 路径，但不改主链路语义。本类是一个 decorator：
 ...
 * 因此 marketdata 路径在适配器就绪策略 baseline 下永远不会被误判为 success（满足 Noop NO_REAL_DISABLED 不得当成功）。
 *
...
public final class ReadinessGuardedMarketDataAdapter implements MarketDataAdapter {
     ...
     * @param realCall 就绪时才执行的真实订阅；适配器就绪策略 baseline 下不会被触达
