...
- [ ] `./ARCHITECTURE.md`：架构基线（域模型、状态机、幂等、账本、恢复）
- [ ] `./MODULES.md`：模块边界与依赖方向（含 `nq-app` 启动载体）
- [ ] `./CONTRACTS.md`：事件契约（Envelope / Topic / 去重字段）
- [ ] `./EVOLUTION_RULES.md`：事件版本演进规则（兼容/破坏性/迁移流程）
- [ ] `./NUMERIC_POLICY.md`：数值精度/舍入/交易所 tickSize 对齐策略
- [ ] `./DB_SCHEMA.md`：表清单、关键字段、索引与一致性口径
- [ ] `./RECOVERY_RUNBOOK.md`：恢复/回放流程与校验口径
- [ ] `./DECISIONS.md`：ADR 记录（至少包含本 Gate 的关键决策）
- [ ] `./ROADMAP.md`：v1→v3 里程碑边界
