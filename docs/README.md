...
本仓库文档按“当前入口 + 最近当前 Gate 卷宗 + 历史冻结卷宗”分层管理：

- `docs/current/`：当前阶段入口，当前表示 **GateF-DOC-1 已完成**
- `docs/gates/gate-f/`：GateF 当前权威卷宗
...
2. 当前阶段：
   - `docs/current/*` 表示 GateF 当前入口
   - `docs/current/GATEF_INPUTS.md` 记录 GateF 输入清单
   - `docs/gates/gate-f/*` 表示 GateF 主卷宗

3. 最近已冻结 Gate：
   - `docs/gates/gate-e/*` 为最近完成且冻结的 GateE 卷宗
   - 只读参考，不再继续扩功能
