...
- `NQ-GATEW-ATTEMPT-10-PRECREATE-PREREQUISITE-REMEDIATION-DEPLOYMENT-VERIFICATION` 已完成 release supply-chain、Linux root/POSIX、systemd 与离线回归验证，但生产 canonical readback 仍返回 `INTERNAL_SANITIZED_READBACK_FAILURE`，因此分类为 `DEPLOYMENT VERIFICATION FAILED / CODE REMEDIATION REQUIRED`（部署验证失败 / 需要代码整改）。Attempt-10 RunId/state/runtime/clock 均未创建，OKX 未调用；current/unit links 已 canonical 回滚到 `c16f27c3...`。下一步只允许 internal readback 脱敏代码 RCA/fix，不得直接重试或触碰生产 credential/permission/IP allowlist、手工 SQL、LIVE/交易写侧、freeze/archive/tag。
