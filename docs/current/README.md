...
`docs/current/` 是 NexusQuant 当前事实入口。当前状态是 **GateJ completed；Next: GateK-PLAN；NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A PostgreSQL/Flyway smoke FROZEN / ACCEPTED；NQ CI Batch 2B FROZEN / ACCEPTED；NQ CI Batch 2C FROZEN / ACCEPTED；GateK product/runtime implementation not started；AI not started**。
...
- GateJ completed。
- Next: GateK-PLAN。
- NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A PostgreSQL/Flyway smoke FROZEN / ACCEPTED；NQ CI Batch 2B FROZEN / ACCEPTED；Batch 2C FROZEN / ACCEPTED；Batch 2D-2E and Batch 3-5 pending；GateK product/runtime implementation not started。
- AI not started。
...
- `NQ_CI_POSTGRES_FLYWAY_PLAN.md`：Batch 2 PostgreSQL / Flyway 当前文件；2A / 2B / 2C 均为 FROZEN / ACCEPTED；下一步只允许 `NQ-CI-POSTGRES-FLYWAY-2D-PLAN`、`NQ-CI-POSTGRES-FLYWAY-2E-PLAN`、`NQ-CI-POSTGRES-FLYWAY-2C-HYGIENE-FIX` 或 Batch 3 pre-planning。
