...
`docs/current/` 是 NexusQuant 当前事实入口。当前状态是 **GateJ completed；Next: GateK-PLAN；NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A PostgreSQL/Flyway smoke FROZEN / ACCEPTED；NQ CI Batch 2B FROZEN / ACCEPTED；NQ CI Batch 2C FROZEN / ACCEPTED；NQ CI Batch 2C hygiene fix FROZEN / ACCEPTED；NQ CI Batch 2D PLAN ONLY / NOT IMPLEMENTED；GateK product/runtime implementation not started；AI not started**。
...
- GateJ completed。
- Next: GateK-PLAN。
- NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A PostgreSQL/Flyway smoke FROZEN / ACCEPTED；NQ CI Batch 2B FROZEN / ACCEPTED；Batch 2C FROZEN / ACCEPTED；Batch 2C hygiene fix FROZEN / ACCEPTED；Batch 2D PLAN ONLY / NOT IMPLEMENTED；Batch 2E NOT STARTED；Batch 3-5 PENDING；GateK product/runtime implementation not started。
- AI not started。
...
- `NQ_CI_POSTGRES_FLYWAY_2D_PLAN.md`：NQ CI Batch 2D `nq-app` context smoke planning 文件；盘点 `@SpringBootTest` / profile / `AuthSeedConfiguration` / runner / scheduler / adapter / no-real probe 边界，规划 CI-only fake profile / explicit properties、no seed、no outbound、rollback 和 required-check 评估；当前为 PLAN ONLY / NOT IMPLEMENTED。
- `WORKLOG.md`：执行日志。
...
- `NQ_CI_POSTGRES_FLYWAY_PLAN.md`：Batch 2 PostgreSQL / Flyway 当前文件；2A / 2B / 2C 均为 FROZEN / ACCEPTED；2C-HYGIENE-FIX 已 FROZEN / ACCEPTED；2D 为 PLAN ONLY / NOT IMPLEMENTED；下一步只允许 `NQ-CI-POSTGRES-FLYWAY-2D-PLAN-REVIEW` 或 2D plan fix。
...
- `NQ_CI_POSTGRES_FLYWAY_2D_PLAN.md`：Batch 2D `nq-app` context smoke planning 文件；PLAN ONLY / NOT IMPLEMENTED，不修改 workflow、不新增测试、不新增 migration、不启动真实 provider、不触发真实交易所。
