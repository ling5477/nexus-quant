...
- [NQ_GATEM_FREEZE_READINESS_REVIEW.md](NQ_GATEM_FREEZE_READINESS_REVIEW.md)：GateM stage-level freeze readiness review（**PASS / READY FOR GATEM FREEZE REVIEW / READY TO COMMIT**）。审查 GateM-1..6、current docs、adapter / MarketData / NoReal / Paper-to-Real / Runtime UI / Operational Readiness / test baseline / security boundary；P0/P1/P2 blocking = 0；该 readiness decision 已由 `NQ_GATEM_FREEZE_REVIEW.md` 消费。不是 freeze，不授权 LIVE / real provider。
- [NQ_GATEM_FREEZE_REVIEW.md](NQ_GATEM_FREEZE_REVIEW.md)：GateM stage freeze review（**PASS / FROZEN / ACCEPTED / READY TO COMMIT**）。冻结 GateM no-real runtime readiness baseline；GateM-1..6 completed / closed；P0/P1/P2 freeze blockers = 0；已由 `NQ-GATEM-RELEASE-TAG-AND-ARCHIVE` 消费并 tagged。不是 production readiness，不授权 LIVE、real provider、RealClient、真实 private trading、real permission probe、AI 或 DH runtime。
