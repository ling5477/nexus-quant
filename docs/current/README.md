...
- `NQ_CI_SECURITY_GUARD_PLAN.md`：Batch 4 security guard / secret scan 当前 plan + implementation 文件；Batch 4A plan review ACCEPTED；Batch 4B 最小 `secret-scan` job 已实现，first CI run（`27662197509`）失败于 gitleaks 1 finding，状态 IMPLEMENTED / FIRST CI RUN FAILED / FIRST-RUN-FIX REQUIRED；下一步只允许 `NQ-CI-SECURITY-GUARD-BATCH-4B-FIRST-RUN-FIX`（先让 finding 可见再精确处置），随后重跑 CI / second-pass review、Batch 4C、可选 Batch 4F，或暂停 CI 线；Batch 4C / 4F / Batch 5 仍 PENDING。
