...
当前阶段：**GateK finalized / frozen / tagged（tag：`nq-gatek-freeze`）；GateM = Exchange / MarketData Runtime Readiness；GateM runtime readiness STARTED / PARTIALLY IMPLEMENTED**。
...
- [frontend/NQ_GATEM_5_RUNTIME_GUARDED_UI_PLAN.md](frontend/NQ_GATEM_5_RUNTIME_GUARDED_UI_PLAN.md)：GateM-5 Runtime Guarded UI planning（**PLAN ONLY / NOT IMPLEMENTED / READY FOR REVIEW**）。规划后续以只读 `/runtime/readiness` 为主入口，汇总 Adapter readiness、MarketData readiness、Paper runtime readiness、LIVE disabled、permission probe skipped、Paper-to-Real boundary 与 runtime blockers；不改 frontend/backend/research/scripts/deploy/workflow/migration，不新增 API，不触发真实交易所、credential、采集、adapter call、LIVE、AI 或 DH runtime。本计划内 5A-5D 是 Runtime Guarded UI 局部批次命名，避免与既有 GateM-5A/5B/5C adapter readiness 历史记录混淆。
...
GateK：规划 / 架构 / 产品化 / 部署化 / 可观测性 / 安全边界收口（NEXT）
  ↓
GateL：No-Real Exchange / MarketData Readiness → GateM：Exchange / MarketData Runtime Readiness（STARTED / PARTIALLY IMPLEMENTED） → Future AI Paper Trading candidate（NOT CURRENT GATEM；AI/DH runtime boundaries must be separately planned） → GateN：AI 小资金 LIVE → GateO：美股适配 → GateP：A 股适配
```
