...
- GateO planning / O-1 / O-2 / O-3 baseline：[GATEO_PLAN.md](GATEO_PLAN.md)（公开行情受控外联与数据质量运行化阶段；O-0 planning-only；O-1 controlled public outbound guard = `PASS / ACCEPTED / FROZEN`；O-2 Data Quality Center = `PASS / ACCEPTED / FROZEN`；O-3 MarketData Runtime Readiness API Plan = `PASS / PLAN ONLY / NOT IMPLEMENTED`；O-3B backend read-only API implementation = `IMPLEMENTED / SELF-REVIEWED / READY TO COMMIT`；O-5 manual real public smoke `NOT STARTED`）
