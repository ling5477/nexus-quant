...
- `NQ-GATEW-ATTEMPT-10-PRECREATE-PREREQUISITE-REMEDIATION-DEPLOYMENT-VERIFICATION` 的生产 canonical readback 曾返回 `INTERNAL_SANITIZED_READBACK_FAILURE`，并按 `DEPLOYMENT VERIFICATION FAILED / CODE REMEDIATION REQUIRED`（部署验证失败 / 需要代码整改）回滚到 `c16f27c3...`。后续 readback/launcher 与 canonical ZIP/CRC 修复已在 source head `5e7a9c4e...` 完成，exact-head CI `30576297678` 10/10 成功，且同一 `EXACT_COMMIT` release 已通过 disposable Linux/PostgreSQL 正反向验证；该 release 未连接、上传或部署到生产。
