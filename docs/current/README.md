...
- 当前唯一动作是 `NQ-GATEY-6-OKX-SPOT-REAL-PROVIDER-MUTATION-CONTRACT-IMPLEMENTATION`；它只允许在独立任务中启动 GateY-6B 的 fake/stub/no-egress contract implementation，不授权 credential lookup、OKX network、真实 PLACE/CANCEL、micro-live、第一笔真实订单或 LIVE，exact transition以 [STATUS.md](STATUS.md) 为准。
