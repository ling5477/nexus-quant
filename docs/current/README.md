...
GateS 过程型 residual 已迁入 [../gates/gate-s/source/](../gates/gate-s/source/) 作为 historical evidence（历史证据），不再保留在 `docs/current`，也不作为 current authority。GateR 两份过程型 residual 已通过 `git mv` 迁入 [../gates/gate-r/source/](../gates/gate-r/source/)；这些 durable historical copies（持久历史副本）不作为 current authority。GateR residual move 不启动 GateU。
...
GateS 已完成 freeze closeout，GateS-0 plan、GateS-1 backend/frontend work order 和 GateS freeze readiness review 已通过 `git mv` 移入 `docs/gates/gate-s/source/`。这些 source copy 只作为 historical evidence（历史证据）保留，不是 current authority；不得把 GateS archive source 写成 GateU implementation、LIVE、AI/DH runtime、real provider、private trading 或真实交易路径。

GateR 已完成 freeze closeout，`GATER_PLAN.md` 与 `GATER_1_SHADOW_RUN_DATA_MODEL_MIGRATION_PLAN_REVIEW.md` 已通过 `git mv` 移入 `docs/gates/gate-r/source/`。这些 source copy 只作为 historical evidence（历史证据）保留，不是 current authority；不得把 GateR archive source 写成 GateU implementation、LIVE、AI/DH runtime、real provider、private trading 或真实交易路径。
