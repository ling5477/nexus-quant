...
`docs/current/` 是 NexusQuant 当前事实入口。当前状态是 **GateJ completed；Next: GateK-PLAN；NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A PostgreSQL/Flyway smoke FROZEN / ACCEPTED；NQ CI Batch 2B PLAN ONLY / NOT IMPLEMENTED；GateK product/runtime implementation not started；AI not started**。
...
- GateJ completed。
- Next: GateK-PLAN。
- NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A PostgreSQL/Flyway smoke FROZEN / ACCEPTED；NQ CI Batch 2B PLAN ONLY / NOT IMPLEMENTED；Batch 2C-2E and Batch 3-5 pending；GateK product/runtime implementation not started。
- AI not started。
...
- `NQ_CI_POSTGRES_FLYWAY_PLAN.md`：Batch 2 PostgreSQL / Flyway 当前文件；2B 为 PLAN ONLY / NOT IMPLEMENTED，下一步只允许 `NQ-CI-POSTGRES-FLYWAY-2B-PLAN-REVIEW` 或评审接受后的 `NQ-CI-POSTGRES-FLYWAY-2B-IMPL`。
- `NQ_CI_POSTGRES_FLYWAY_2B_PLAN.md`：Batch 2B schema artifact / docs review planning-only 文件；不代表 workflow 或 artifact 已实现。
