...
- `NQ-GATEW-ATTEMPT-10-PRECREATE-PREREQUISITE-REMEDIATION-DEPLOYMENT-VERIFICATION` 的生产 canonical readback 曾返回 `INTERNAL_SANITIZED_READBACK_FAILURE`，并按 `DEPLOYMENT VERIFICATION FAILED / CODE REMEDIATION REQUIRED`（部署验证失败 / 需要代码整改）回滚到 `c16f27c3...`。旧 fixed RC `5e7a9c4e...` 与整改 RC `ef803568...` 均已被独立 review 拒绝。新 RC `5a7e824e...` 已关闭 JAR duplicate-directory/full-stream/CRC P1，exact-head CI run `30632959743` 为 10/10 GREEN；Windows PowerShell 5.1/7 与 no-egress Linux exact-build bytes 一致，122 JAR 全量 entry 读取通过，但仅达到 `RC_READY_FOR_REVIEW / PRODUCTION_DEPLOYMENT_NOT_STARTED`。
- 当前唯一下一动作是 `NQ-GATEW-ATTEMPT-10-RELEASE-CANDIDATE-STABILIZATION-REVIEW`。Attempt-10 仍为 `NOT_CREATED / NOT_AUTHORIZED`；不得直接重试或触碰生产 credential/permission/IP allowlist、手工 SQL、LIVE/交易写侧、freeze/archive/tag。
