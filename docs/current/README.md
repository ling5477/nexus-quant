...
NQ-DH-I1-P1-CONTRACT-DRYRUN-PLAN = **COMPLETED / PLAN ONLY / NOT IMPLEMENTED**。含义：`COMPLETED`（已完成）、`PLAN ONLY`（仅规划）、`NOT IMPLEMENTED`（未实现）。本轮新增 [NQ_DH_INTEGRATION1_DRYRUN_PLAN.md](NQ_DH_INTEGRATION1_DRYRUN_PLAN.md)，只规划 contract dry-run 的消息 schema、canonical `X-NQ-DH-*` header、tenant / requestId / traceId / timestamp / nonce / HMAC、安全拒绝、错误码、audit、后续 batch 和 no-side-effect 测试前置；不新增 API / migration / production code / test code，不启动 NQ/DH runtime，不真实 HTTP，不接 RealClient / real provider / LIVE / AI / LangGraph。下一步只允许 `NQ-DH-I1-P1-A-CONTRACT-SCHEMA-FIXTURE-PLAN-REVIEW / NOT STARTED`。
...
- NQ-DH Integration-1 contract dry-run plan：[NQ_DH_INTEGRATION1_DRYRUN_PLAN.md](NQ_DH_INTEGRATION1_DRYRUN_PLAN.md)
