...
- `NQ_CI_SECURITY_GUARD_PLAN.md`：Batch 4 security guard / secret scan 当前 plan + implementation 文件；Batch 4A plan review ACCEPTED；Batch 4B 最小 `secret-scan` job 已实现，first CI run（`27662197509`）失败于 frozen Gate-C generic-api-key FP，first-run-fix `31540de8` 已应用（gitleaks finding 可见性 sanitized 输出 + 单文件精确 allowlist），second run `27674393780` 7/7 jobs green，Batch 4E freeze review 固化为 **FROZEN / ACCEPTED**（frozen baseline commit `31540de8`）；下一步只允许 `NQ-CI-SECURITY-GUARD-BATCH-4C`（artifact/log redaction planning）、`NQ-CI-SECURITY-GUARD-BATCH-4F`（dependency audit later plan）、Batch 5 planning，或暂停 CI 线；Batch 4C / 4F / Batch 5 仍 PENDING。
