...
- 当前唯一动作是 `NQ-GATEY-6F-CREDENTIAL-AUTHENTICATION-REMEDIATION-BLOCKED`；release reproducibility、Spring composition与最小权限DB写窗口均已关闭并通过exact-head CI，production已运行exact V43 release，但current OKX permission refresh确定性返回`HTTP_UNAUTHORIZED`。PLACE/CANCEL/transfer/withdraw均为0；operator只能通过安全credential管理路径修复或轮换material，不得在聊天中提供明文。精确边界以 [STATUS.md](STATUS.md) 为准。
