...
`docs/current/` 是 NexusQuant 当前事实入口。当前状态是 **GateJ completed；Next: GateK-PLAN；NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A/2B/2C/2C hygiene/2D/2E FROZEN / ACCEPTED；Batch 3 no-outbound guard FROZEN / ACCEPTED；Batch 4B secret scan FROZEN / ACCEPTED；Batch 4C overall security artifact/log redaction baseline FROZEN / ACCEPTED（4C-B pre-upload artifact redaction gate FROZEN / ACCEPTED；4C-C log redaction proof FROZEN / ACCEPTED）；Batch 4F execution sequence SYNCED / ACCEPTED，4F-A preflight FROZEN / ACCEPTED，Python local audit NOT READY，4F-B/4F-C/4F-D/4F-E/4F-F OPTIONAL BACKLOG / NOT STARTED；Static workflow assertion OPTIONAL FUTURE HARDENING / NOT IMPLEMENTED；Batch 5A no-backend E2E FROZEN / ACCEPTED（两次 immutable green run 一致零 drift：run `27750279096`/commit `861c3e78` + run `27750976632`/commit `3d26c84d`，各 4 passed；ci.yml blob `6941d60a…` 与 playwright.ci.config.ts blob `d039fe82…` 两 commit 完全一致；无 backend/DB/认证/api/artifact，见 `NQ_CI_FRONTEND_E2E_5A_FREEZE_REVIEW.md`）；Batch 5B-ENV P1 SECURITY ENHANCEMENT / NOT STARTED；Batch 5B-SMOKE BLOCKED BY 5B-ENV；NQ GateK CI mainline COMPLETED / ACCEPTED；documentation governance plan ready（只读盘点 277 份 md/txt + 分类/authority/目标结构/迁移映射/不可删除清单/G1~G6 批次，未收口、未移动/删除/重命名任何文档，见 `NQ_DOCS_GOVERNANCE_PLAN.md`）；GateK product/runtime implementation not started；AI not started**。
...
- GateJ completed。
- Next: GateK-PLAN。
- NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A PostgreSQL/Flyway smoke FROZEN / ACCEPTED；NQ CI Batch 2B FROZEN / ACCEPTED；Batch 2C FROZEN / ACCEPTED；Batch 2C hygiene fix FROZEN / ACCEPTED；Batch 2D FROZEN / ACCEPTED；Batch 2E FROZEN / ACCEPTED；Batch 3 no-outbound guard FROZEN / ACCEPTED（run `27634370657`）；Batch 4A plan review accepted；Batch 4B secret scan FROZEN / ACCEPTED（run `27674393780`，frozen baseline commit `31540de8`）；Batch 4C overall security artifact/log redaction baseline **FROZEN / ACCEPTED**（4C-A plan review PASS / ACCEPTED；4C-B pre-upload artifact redaction gate **FROZEN / ACCEPTED**，immutable green run `27701669084`，frozen baseline = `ci.yml` pre-upload redaction gate step blob `4a40ef78` / commit `c734102d`；4C-C log redaction proof **FROZEN / ACCEPTED**，freeze review，immutable green run `27732660516`，14 类 pattern 真实值命中 = 0，`NQ_CI_LOG_REDACTION_PROOF_FREEZE_REVIEW.md`；overall freeze review 见 `NQ_CI_SECURITY_GUARD_BATCH_4C_FREEZE_REVIEW.md`）；Batch 4F plan review **PASS / ACCEPTED**，execution sequence **SYNCED / ACCEPTED**，4F-A preflight **FROZEN / ACCEPTED**（freeze review `NQ_CI_DEPENDENCY_AUDIT_PREFLIGHT_FREEZE_REVIEW.md`），Python local audit **NOT READY**，4F-B/4F-C/4F-D/4F-E/4F-F **NOT STARTED**；Static workflow assertion **OPTIONAL FUTURE HARDENING / NOT IMPLEMENTED**；Batch 5 plan review **PASS / ACCEPTED**（见 `NQ_CI_FRONTEND_E2E_PLAN_REVIEW.md`）；**Batch 5A no-backend E2E FROZEN / ACCEPTED**（CI job `frontend-no-backend-e2e` + `frontend/playwright.ci.config.ts`，allowlist = `login-page-smoke` / `design-system-table-smoke` / `design-system-live-query-smoke` / `design-system-backtest-chart-smoke`，真实路径 `frontend/tests/e2e/`；两次 immutable green run 一致零 drift —— 首跑 run `27750279096`/commit `861c3e78`/job `82098741200`（4 passed）+ freeze run `27750976632`/commit `3d26c84d`/job `82101090359`（4 passed），冻结基线 ci.yml blob `6941d60ade2bfce456e203f708b633e595285178` + playwright.ci.config.ts blob `d039fe82fbf7db6f55c3e6fc089bac59a2fe9014`；其余 23 spec 0 次、无 skip-as-pass，无 backend/DB/Flyway/Docker/认证/seed/`/api`/storageState/artifact，permissions Contents: read，trace/video/screenshot off，见 `NQ_CI_FRONTEND_E2E_5A_IMPLEMENTATION.md`、`NQ_CI_FRONTEND_E2E_5A_FIRST_RUN_REVIEW.md` 与 `NQ_CI_FRONTEND_E2E_5A_FREEZE_REVIEW.md`）；Batch 5B-ENV = **P1 PREREQUISITE / NOT STARTED**；Batch 5B-SMOKE = **BLOCKED BY 5B-ENV**；GateK product/runtime implementation not started。
- AI not started。
...
- `NQ_CI_SECURITY_GUARD_PLAN.md`：GateK CI Batch 4 security guard / secret scan plan + implementation 文件；Batch 4A plan review ACCEPTED；Batch 4B secret scan FROZEN / ACCEPTED（run `27674393780`，frozen baseline commit `31540de8`）；Batch 4C overall security artifact/log redaction baseline FROZEN / ACCEPTED（4C-B pre-upload artifact redaction gate FROZEN / ACCEPTED，4C-C log redaction proof FROZEN / ACCEPTED；overall freeze review 见 `NQ_CI_SECURITY_GUARD_BATCH_4C_FREEZE_REVIEW.md`）；Batch 4F execution sequence SYNCED / ACCEPTED，4F-A preflight FROZEN / ACCEPTED，Python local audit NOT READY，4F-B 至 4F-F NOT STARTED；Batch 5 PENDING。
- `NQ_CI_DEPENDENCY_AUDIT_PLAN.md`：GateK CI Batch 4F dependency audit / supply-chain audit planning 文件；plan review 已由 `NQ_CI_SECURITY_GUARD_BATCH_4F_PLAN_REVIEW.md` 接受为 implementation baseline；execution sequence 已同步为 4F-A dependency audit input / toolchain preflight、4F-B sanitized advisory audit summary、4F-C SBOM report-only、4F-D PR dependency delta review、4F-E GitHub Actions / CLI supply-chain pinning、4F-F Dependabot / Renovate governance；4F-A preflight 已由 `NQ_CI_DEPENDENCY_AUDIT_PREFLIGHT_FREEZE_REVIEW.md` 冻结为 FROZEN / ACCEPTED，4F-B 至 4F-F NOT STARTED；未修改 workflow / code / tests / migration / frontend / research / scripts / deploy。
- `NQ_CI_DEPENDENCY_AUDIT_PREFLIGHT.md`：GateK CI Batch 4F-A dependency audit input / toolchain preflight implementation 文件；原结论 PASS / READY FOR REVIEW；记录 Maven / npm / Python / GitHub Actions 输入清单、本机工具链可用性、4F-B sanitized summary 交接标准、raw output / SBOM / lockfile hygiene、action major tag 与 gitleaks checksum pinning 作为 4F-E 输入；未运行 vulnerability scanner、未生成 SBOM、未上传 artifact、未修改 workflow 或依赖文件。
- `NQ_CI_DEPENDENCY_AUDIT_PREFLIGHT_REVIEW.md`：GateK CI Batch 4F-A preflight review 主文档；结论 PASS / ACCEPTED，Batch 4F-A preflight ACCEPTED / READY FOR FREEZE REVIEW，P0/P1=0；review-time clarification 将 `scope` 固定为 4F-B mandatory bounded field，并确认 Python WindowsApps stub 仅为后续 4F-B execution environment 条件，不代表 local audit ready。
- `NQ_CI_DEPENDENCY_AUDIT_PREFLIGHT_FREEZE_REVIEW.md`：GateK CI Batch 4F-A freeze review 主文档；结论 PASS / ACCEPTED / FROZEN，Batch 4F-A preflight FROZEN / ACCEPTED，Python local audit NOT READY，4F-B 至 4F-F NOT STARTED；冻结 4F-B 十字段 sanitized summary、bounded `scope`、report-only policy、Batch 4C artifact gate 与 4F-E supply-chain gap。
- `NQ_CI_FRONTEND_E2E_PLAN.md`：GateK CI Batch 5 frontend Playwright E2E hardening planning 主文档；结论 PASS / READY FOR REVIEW，Batch 5 PLAN ONLY / NOT IMPLEMENTED；冻结 4-spec no-backend allowlist、isolated PostgreSQL/Flyway/backend/auth fixture 拓扑、Batch 3 runtime no-outbound 入场条件与 Batch 4C artifact/log gate，未修改 workflow/code/test/dependency/migration。
- `NQ_CI_SECURITY_GUARD_BATCH_4F_PLAN_REVIEW.md`：GateK CI Batch 4F dependency audit / supply-chain audit plan review 历史文档；结论 PASS / ACCEPTED，Batch 4F plan ACCEPTED AS IMPLEMENTATION BASELINE，execution sequence SYNCED / ACCEPTED；其记录的 4F-A IMPLEMENTED / READY FOR REVIEW 先由 preflight review 推进为 ACCEPTED / READY FOR FREEZE REVIEW，后由 `NQ_CI_DEPENDENCY_AUDIT_PREFLIGHT_FREEZE_REVIEW.md` 推进为 FROZEN / ACCEPTED；4F-B 至 4F-F NOT STARTED。
...
- `NQ_DOCS_G2_FREEZE_REVIEW.md`：G2 冻结复核（semantic baseline freeze，非 blob lock）；结论 **PASS / ACCEPTED / FROZEN**，**G2 current-control drift repair = FROZEN / ACCEPTED**，**G3 = READY FOR IMPLEMENTATION**，G4~G6 = NOT STARTED；明确失效条件（恢复缺陷/越冻结边界）与允许维护（带日期追加、新增导航、未来规划）；current-control 文档不被锁成 immutable blob。属 freeze evidence（HISTORICAL_EVIDENCE / RETAIN_IN_PLACE）。
...
- `NQ_CI_SECURITY_GUARD_PLAN.md`：Batch 4 security guard / secret scan 当前 plan + implementation 文件；Batch 4A plan review ACCEPTED；Batch 4B FROZEN / ACCEPTED（run `27674393780`，frozen baseline commit `31540de8`）；Batch 4C overall security artifact/log redaction baseline FROZEN / ACCEPTED（4C-B pre-upload artifact redaction gate FROZEN / ACCEPTED，immutable run `27701669084`；4C-C log redaction proof FROZEN / ACCEPTED，immutable run `27732660516`，14 类 pattern 真实值命中 = 0；overall freeze review 见 `NQ_CI_SECURITY_GUARD_BATCH_4C_FREEZE_REVIEW.md`）；Batch 4F execution sequence SYNCED / ACCEPTED，4F-A preflight FROZEN / ACCEPTED，Python local audit NOT READY，4F-B 至 4F-F NOT STARTED；后续只允许独立 scoped 4F-B、optional static assertion、Batch 5 planning，或暂停 CI 线；Batch 4F-B 至 4F-F 不得写成 implemented，Batch 5 仍 PENDING。
- `NQ_CI_DEPENDENCY_AUDIT_PLAN.md`：Batch 4F dependency audit / supply-chain audit planning 文件；覆盖 Maven / npm / Python audit、GitHub Actions supply-chain、action SHA pinning、SBOM、Dependency Review、Dependabot / Renovate、CI blocking / advisory 分层、raw artifact / report hygiene、与 Batch 4C redaction baseline 和 Batch 5 frontend E2E hardening 的关系；plan review PASS / ACCEPTED，execution sequence SYNCED / ACCEPTED，4F-A preflight freeze review PASS / ACCEPTED / FROZEN，4F-B 至 4F-F NOT STARTED。
- `NQ_CI_DEPENDENCY_AUDIT_PREFLIGHT.md`：Batch 4F-A dependency audit input / toolchain preflight implementation 文件；原状态 PASS / READY FOR REVIEW；只建立 4F-B 输入和输出卫生基线，不运行 audit / SBOM / scanner，不改 workflow / dependency files。
- `NQ_CI_DEPENDENCY_AUDIT_PREFLIGHT_REVIEW.md`：Batch 4F-A preflight review 文件；PASS / ACCEPTED / READY FOR FREEZE REVIEW；`scope` 为 4F-B mandatory bounded field，Python local interpreter 仍不可用，4F-B 至 4F-F NOT STARTED。
- `NQ_CI_DEPENDENCY_AUDIT_PREFLIGHT_FREEZE_REVIEW.md`：Batch 4F-A freeze review 当前权威文件；PASS / ACCEPTED / FROZEN；Python local audit NOT READY，4F-B 至 4F-F NOT STARTED。
- `NQ_CI_SECURITY_GUARD_BATCH_4F_PLAN_REVIEW.md`：Batch 4F plan review 历史文件；结论 PASS / ACCEPTED，Batch 4F plan ACCEPTED AS IMPLEMENTATION BASELINE，execution sequence SYNCED / ACCEPTED；4F-A 当前状态以 `NQ_CI_DEPENDENCY_AUDIT_PREFLIGHT_FREEZE_REVIEW.md` 的 FROZEN / ACCEPTED 为准，4F-B 至 4F-F NOT STARTED。
