...
- `NQ_CI_POSTGRES_FLYWAY_PLAN.md`：Batch 2 PostgreSQL / Flyway 当前文件；2A / 2B / 2C 均为 FROZEN / ACCEPTED；2C-HYGIENE-FIX 已 FROZEN / ACCEPTED；2D 为 FROZEN / ACCEPTED；2E 为 FIRST GREEN RUN CONFIRMED；下一步只允许 `NQ-CI-POSTGRES-FLYWAY-2E-FREEZE-REVIEW`、Batch 3 pre-planning 或暂停 CI 线。
...
- `NQ_CI_POSTGRES_FLYWAY_2E_PLAN.md`：Batch 2E CI-only seed watcher cleanup 文件；已修改 workflow 删除 background seed watcher；first-run fix 已新增同步 post-Flyway CI-only legacy `accounts` fixture，并校验不创建 `exchange_accounts` 或 credential rows；run `27614046762` first green confirmed。
