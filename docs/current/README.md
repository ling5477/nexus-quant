...
GateL-1A..1E 已完成 No-Real exchange/marketdata 文档边界（contract / error model / capability matrix / readiness checklist）。当前进入 **GateM adapter readiness runtime enforcement = IMPLEMENTATION STARTED / PENDING REVIEW**：GateM-0 在 `nq-adapter-api` 新增运行时 `AdapterReadinessService` / `DefaultAdapterReadinessService` 与 readiness 模型；GateM-1 进一步把 guard 接入行情订阅与交易动作入口（`ReadinessGuardedMarketDataAdapter` / `ReadinessGuardedTradingAdapter`），运行时强制 OKX / Binance / Noop fail-closed（NOT_READY / NO_REAL / UNKNOWN_REQUIRES_REVIEW），marketdata 永不 subscribed=true、trading 未就绪即抛出且绝不触达真实 adapter。GateM-2 再把 guard 经 `ReadinessGuardedAdapterFactory` 接入装配层（`nq-app` `LocalTestFallbackConfiguration` 的 marketdata Bean），使调用方默认拿到经 readiness 守卫的 adapter（PAPER→NO_REAL_DISABLED、OKX/Binance→ENDPOINT_DISABLED_SENTINEL）。不允许真实交易所、LIVE、真实 credential、AI、DH runtime 或 future-real-ready。详见 STATUS.md / WORKLOG.md / TESTING.md。
