...
- `NQ_CI_SECURITY_GUARD_PLAN.md`：Batch 4 security guard / secret scan 当前 plan + implementation 文件；Batch 4A plan review ACCEPTED；Batch 4B 最小 `secret-scan` job 已实现，first CI run（`27662197509`）失败于 frozen Gate-C generic-api-key FP，first-run-fix `31540de8` 已应用（gitleaks finding 可见性 sanitized 输出 + 单文件精确 allowlist），second run `27674393780` 7/7 jobs green，Batch 4E freeze review 固化为 **FROZEN / ACCEPTED**（frozen baseline commit `31540de8`）；Batch 4C artifact/log redaction 已落档为 PLAN ONLY / NOT IMPLEMENTED（`NQ_CI_ARTIFACT_LOG_REDACTION_PLAN.md`）；下一步只允许 `NQ-CI-SECURITY-GUARD-BATCH-4C-A`（plan review）/ `4C-B`（pre-upload gate impl）、`NQ-CI-SECURITY-GUARD-BATCH-4F`（dependency audit later plan）、Batch 5 planning，或暂停 CI 线；Batch 4C / 4F / Batch 5 仍 PENDING。
- `NQ_CI_ARTIFACT_LOG_REDACTION_PLAN.md`：Batch 4C artifact / log redaction proof 当前文件；4C-A plan review PASS / ACCEPTED，4C-B pre-upload redaction gate IMPLEMENTED（commit `c734102d`），4C-D first-run review FAIL（gate 本身 first-run GREEN，整体 run `27698183911` 失败于无关 secret-scan 文档 gitleaks 误报），4C-B FIRST-RUN-FIX APPLIED（doc-only 中和示例串，commit `66cb3d40`）→ second-pass run `27701669084` 7/7 jobs green → FIRST GREEN RUN CONFIRMED AFTER DOC FIX，Batch 4C 整体仍 NOT FROZEN；下一步只允许 Batch 4C-E（pre-upload gate freeze review）、Batch 4C-C planning，或暂停 CI 线；复用 Batch 4B pattern，不做 Batch 4F / Batch 5。
