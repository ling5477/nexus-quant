...
- `NQ_CI_ARTIFACT_LOG_REDACTION_PLAN.md`：GateK CI Batch 4C artifact / log redaction proof plan 文件；盘点已上传 schema artifacts（唯一 upload，已有专用 redaction check）、未上传的 gitleaks report / surefire / frontend / research outputs、log 风险（env dump / connection string / raw req-resp / signature / credential / encrypted_payload-decrypted_payload 真实值），规划可复用 pre-upload redaction gate、credential pattern 收敛、禁止上传 raw gitleaks report、log redaction proof，并拆分 4C-A/4C-B/4C-C/4C-D/4C-E。当前 4C-A plan review ACCEPTED、4C-B pre-upload artifact redaction gate FROZEN / ACCEPTED（immutable run `27701669084`）、4C-C log redaction proof PLAN ONLY / NOT IMPLEMENTED（独立成文 `NQ_CI_LOG_REDACTION_PROOF_PLAN.md`），**Batch 4C 整体仍 NOT FROZEN**；复用 Batch 4B pattern，不重复 4B，不做 Batch 4F dependency audit / Batch 5 frontend E2E hardening。
- `NQ_CI_LOG_REDACTION_PROOF_PLAN.md`：GateK CI Batch 4C-C log redaction proof plan 文件；review-time `gh run view --log` log proof（7 jobs）+ 静态 `printenv`/`env`/`set -x` 断言 + 保留 `::add-mask::`，log 风险盘点、Pattern checklist、finding 只输出 job / category / rule / safe excerpt、FP 逐条精确裁定，与 4C-B artifact gate 扫描目标不同、不重复 artifact gate；当前 PLAN ONLY / NOT IMPLEMENTED，本轮只 planning，不实现 workflow / job / step，不做 Batch 4F / Batch 5。
- `WORKLOG.md`：执行日志。
...
- `NQ_CI_SECURITY_GUARD_PLAN.md`：Batch 4 security guard / secret scan 当前 plan + implementation 文件；Batch 4A plan review ACCEPTED；Batch 4B 最小 `secret-scan` job 已实现，first CI run（`27662197509`）失败于 frozen Gate-C generic-api-key FP，first-run-fix `31540de8` 已应用（gitleaks finding 可见性 sanitized 输出 + 单文件精确 allowlist），second run `27674393780` 7/7 jobs green，Batch 4E freeze review 固化为 **FROZEN / ACCEPTED**（frozen baseline commit `31540de8`）；Batch 4C artifact/log redaction 进行中——**4C-B pre-upload artifact redaction gate FROZEN / ACCEPTED**（Batch 4C-E freeze review，immutable run `27701669084`，frozen baseline = `ci.yml` pre-upload redaction gate step blob `4a40ef78` / commit `c734102d`），**Batch 4C 整体仍 NOT FROZEN**（4C-C log redaction proof 未开始）（详见 `NQ_CI_ARTIFACT_LOG_REDACTION_PLAN.md`）；下一步只允许 `NQ-CI-SECURITY-GUARD-BATCH-4C-C`（log redaction proof planning）、`NQ-CI-SECURITY-GUARD-BATCH-4F`（dependency audit later plan）、Batch 5 planning，或暂停 CI 线；Batch 4C-C / 4F / Batch 5 仍 PENDING / NOT STARTED。
...
- `NQ_CI_LOG_REDACTION_PROOF_PLAN.md`：Batch 4C-C log redaction proof 当前 planning 文件；**PLAN ONLY / NOT IMPLEMENTED**。规划 review-time `gh run view --log` log proof（覆盖 7 jobs：Diff check / No-outbound guard / Backend Maven test / PostgreSQL / Flyway smoke / Frontend build / Research quality gate / Secret scan）+ 静态断言无 `printenv` / `env` dump / `set -x` + 保留 `::add-mask::`；log 风险盘点（env dump / connection string / raw req-resp / signature / credential / encrypted_payload-decrypted_payload / disposable CI 值 / Spring Boot dev password）、Pattern checklist、finding 只输出 job / category / rule / safe excerpt、FP 逐条精确裁定（禁止 broad allowlist）、与 4C-B artifact gate 扫描目标不同、不读取本地 logs / 不上传 logs artifact / 不自扫 streaming 日志、自动化只作为 future hardening。本轮只 planning，不实现 workflow / job / step；**Batch 4C 整体仍 NOT FROZEN**；不做 Batch 4F dependency audit / Batch 5 frontend E2E hardening。
