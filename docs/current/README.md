...
- `NQ-GATEW-ATTEMPT-10-PRECREATE-PREREQUISITE-REMEDIATION-DEPLOYMENT-VERIFICATION` 的生产 canonical readback 曾返回 `INTERNAL_SANITIZED_READBACK_FAILURE`，并按 `DEPLOYMENT VERIFICATION FAILED / CODE REMEDIATION REQUIRED`（部署验证失败 / 需要代码整改）回滚到 `c16f27c3...`。旧 fixed RC `5e7a9c4e...` 已因 4 个 P1 与 1 个 P2 被独立 review 拒绝；整改 RC `ef803568...` 的 exact-head CI run `30616271884` 虽为 10/10 GREEN、Windows clean-build bytes 可复现，但 attempt-02 review 又发现 `UNSAFE_DUPLICATE_JAR_ENTRY` P1：verifier 未对 JAR entry data 执行 CRC readback，且可放行带载荷的重复目录 entry。因此该 RC 不可部署。
- 当前唯一下一动作是 `NQ-GATEW-ATTEMPT-10-RELEASE-CANDIDATE-STABILIZATION-FIX`。Attempt-10 仍为 `NOT_CREATED / NOT_AUTHORIZED`；不得直接重试或触碰生产 credential/permission/IP allowlist、手工 SQL、LIVE/交易写侧、freeze/archive/tag。
