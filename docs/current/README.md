...
- [GATEL_1C_CAPABILITY_MATRIX_CONTRACT.md](GATEL_1C_CAPABILITY_MATRIX_CONTRACT.md)：GateL-1C capability matrix contract（**PASS / CONTRACT FROZEN**）。冻结 `CLOSED_NO_REAL` / `DISABLED_SENTINEL` / `NO_REAL_DISABLED` / `STUB_ONLY` / `NOT_IMPLEMENTED` / `FUTURE_REAL_REQUIRES_GATE` / `FORBIDDEN_IN_GATEL` / `UNKNOWN_REQUIRES_REVIEW` 状态枚举与 Noop / OKX / Binance / future-real / permission probe / marketdata placeholder 能力矩阵；OKX/Binance existing adapters 仍不是 future-real-ready，也不等于真实交易所授权；adapter readiness 仍 NOT READY / NOT FROZEN / NOT AUTHORIZED；LIVE、真实 credential、AI、DH runtime 仍不允许。下一步 `NQ-GATEL-1C-CAPABILITY-MATRIX-CONTRACT-REVIEW`。
