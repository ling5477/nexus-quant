...
`docs/current/` 是 NexusQuant 当前事实入口。当前状态是 **GateJ completed；Next: GateK-PLAN；NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A PostgreSQL/Flyway smoke FROZEN / ACCEPTED；NQ CI Batch 2B FROZEN / ACCEPTED；GateK product/runtime implementation not started；AI not started**。
...
- GateJ completed。
- Next: GateK-PLAN。
- NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A PostgreSQL/Flyway smoke FROZEN / ACCEPTED；NQ CI Batch 2B FROZEN / ACCEPTED；Batch 2C PLAN READY FOR REVIEW / NOT IMPLEMENTED；Batch 2D-2E and Batch 3-5 pending；GateK product/runtime implementation not started。
- AI not started。
...
- `NQ_CI_POSTGRES_FLYWAY_2C_PLAN.md`：NQ CI Batch 2C repository real PostgreSQL smoke planning-only 文件；盘点 repository / JDBC / Spring context 测试边界，冻结 2C-1 / 2C-2 / 2C-3 切片、seed / fixture、transaction / cleanup、安全和 rollback 策略；当前为 PLAN READY FOR REVIEW / NOT IMPLEMENTED。
- `WORKLOG.md`：执行日志。
...
- `NQ_CI_POSTGRES_FLYWAY_PLAN.md`：Batch 2 PostgreSQL / Flyway 当前文件；2B 为 FROZEN / ACCEPTED；2C plan ready for review / not implemented；下一步只允许 `NQ-CI-POSTGRES-FLYWAY-2C-PLAN-REVIEW` 或 `NQ-CI-POSTGRES-FLYWAY-2C-PLAN-FIX`。
