...
- `NQ_CI_ARTIFACT_LOG_REDACTION_PLAN.md`：Batch 4C artifact / log redaction proof 当前文件；4C-A plan review PASS / ACCEPTED，4C-B pre-upload redaction gate IMPLEMENTED / PENDING FIRST CI RUN（2026-06-17，仅改 `ci.yml` `postgres-flyway` job），Batch 4C 整体仍 NOT FROZEN；下一步只允许 Batch 4C-D first-run review（或 4C-B first-run-fix）、Batch 4C-C log redaction proof，或暂停 CI 线；复用 Batch 4B pattern，不做 Batch 4F / Batch 5。
