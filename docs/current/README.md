...
`docs/current/` 是 NexusQuant 当前事实入口。当前状态是 **GateJ completed；Next: GateK-PLAN；NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A PostgreSQL/Flyway smoke FROZEN / ACCEPTED；NQ CI Batch 2B FROZEN / ACCEPTED；NQ CI Batch 2C FROZEN / ACCEPTED；NQ CI Batch 2C hygiene fix FROZEN / ACCEPTED；NQ CI Batch 2D IMPLEMENTED / FIRST-RUN-FIX APPLIED / PENDING FIRST CI RUN；GateK product/runtime implementation not started；AI not started**。
...
- GateJ completed。
- Next: GateK-PLAN。
- NQ CI baseline Batch 1 implemented / first green confirmed；NQ CI Batch 2A PostgreSQL/Flyway smoke FROZEN / ACCEPTED；NQ CI Batch 2B FROZEN / ACCEPTED；Batch 2C FROZEN / ACCEPTED；Batch 2C hygiene fix FROZEN / ACCEPTED；Batch 2D IMPLEMENTED / FIRST-RUN-FIX APPLIED / PENDING FIRST CI RUN；Batch 2E NOT STARTED；Batch 3-5 PENDING；GateK product/runtime implementation not started。
- AI not started。
...
- `NQ_CI_POSTGRES_FLYWAY_2D_PLAN.md`：NQ CI Batch 2D `nq-app` context smoke planning + implementation + first-run review 文件；盘点 `@SpringBootTest` / profile / `AuthSeedConfiguration` / runner / scheduler / adapter / no-real probe 边界，并实现 CI-only fake profile / explicit properties、no seed、no outbound-by-construction、rollback 和 required-check 评估；first run failed at Spring context startup，当前为 IMPLEMENTED / FIRST-RUN-FIX APPLIED / PENDING FIRST CI RUN。
- `WORKLOG.md`：执行日志。
...
- `NQ_CI_POSTGRES_FLYWAY_PLAN.md`：Batch 2 PostgreSQL / Flyway 当前文件；2A / 2B / 2C 均为 FROZEN / ACCEPTED；2C-HYGIENE-FIX 已 FROZEN / ACCEPTED；2D 为 IMPLEMENTED / FIRST-RUN-FIX APPLIED / PENDING FIRST CI RUN；下一步只允许 re-run CI 后 `NQ-CI-POSTGRES-FLYWAY-2D-FIRST-RUN-REVIEW`，或继续 `NQ-CI-POSTGRES-FLYWAY-2D-FIRST-RUN-FIX`。
...
- `NQ_CI_POSTGRES_FLYWAY_2D_PLAN.md`：Batch 2D `nq-app` context smoke planning + implementation + first-run review 文件；IMPLEMENTED / FIRST-RUN-FIX APPLIED / PENDING FIRST CI RUN，不新增 migration、不启动真实 provider、不触发真实交易所，不允许写成 FIRST GREEN RUN CONFIRMED / FROZEN / ACCEPTED。
