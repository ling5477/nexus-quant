...
- `NQ-GATEW-ATTEMPT-10-PRECREATE-PREREQUISITE-REMEDIATION-DEPLOYMENT-VERIFICATION` 的生产 canonical readback 曾返回 `INTERNAL_SANITIZED_READBACK_FAILURE`，并按 `DEPLOYMENT VERIFICATION FAILED / CODE REMEDIATION REQUIRED`（部署验证失败 / 需要代码整改）回滚到 `c16f27c3...`。旧 fixed RC `5e7a9c4e...` 与整改 RC `ef803568...` 均已被独立 review 拒绝。新 RC `5a7e824e...` 的 JAR full-stream/CRC、exact-head CI、Windows/Linux exact build 与 122 JAR 全量读取通过；attempt-03 首次发现的 nondeterministic regression fixture 已获授权做最小 timestamp 修复，PS5.1/PS7/Linux pwsh7 各连续 3 次 34/34，最终 `REVIEW_ACCEPTED / READY_TO_COMMIT`。
- 当前唯一下一动作是 `NQ-GATEW-ATTEMPT-10-RELEASE-CANDIDATE-STABILIZATION-REVIEW-COMMIT-AND-PUSH`。Attempt-10 仍为 `NOT_CREATED / NOT_AUTHORIZED`；review exact-head CI 与 authority-sync 前不得直接重试、部署或触碰生产 credential/permission/IP allowlist、手工 SQL、LIVE/交易写侧、freeze/archive/tag。
