...
- `NQ-GATEW-ATTEMPT-10-PRECREATE-PREREQUISITE-REMEDIATION-DEPLOYMENT-VERIFICATION` 的生产 canonical readback 曾返回 `INTERNAL_SANITIZED_READBACK_FAILURE`，并按 `DEPLOYMENT VERIFICATION FAILED / CODE REMEDIATION REQUIRED`（部署验证失败 / 需要代码整改）回滚到 `c16f27c3...`。后续 fixed RC `5e7a9c4e...` 虽有 exact-head CI 10/10 与 disposable evidence，但独立 review 因 Java runtime contract、进程 timeout、声明 hashes 不可重建及 mapping taxonomy 共 4 个 P1而拒绝；该 RC 未连接、上传或部署到生产。
