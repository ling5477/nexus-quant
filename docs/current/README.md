...
GateS 过程型 residual 已迁入 [../gates/gate-s/source/](../gates/gate-s/source/) 作为 historical evidence（历史证据），不再保留在 `docs/current`，也不作为 current authority。GateR 过程型 residual 的后续迁移计划见 [NQ_DOCS_ARCHIVE_RULE_HARDENING_AND_RESIDUAL_MOVE_PLAN.md](NQ_DOCS_ARCHIVE_RULE_HARDENING_AND_RESIDUAL_MOVE_PLAN.md)。该计划的 GateT / GateS move batch 已执行；GateR 后续 move 仍未启动，不启动 GateU。
