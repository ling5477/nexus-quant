...
- `NQ_CI_POSTGRES_FLYWAY_2D_PLAN.md`：NQ CI Batch 2D `nq-app` context smoke planning + implementation + first-run review 文件；盘点 `@SpringBootTest` / profile / `AuthSeedConfiguration` / runner / scheduler / adapter / no-real probe 边界，并实现 CI-only fake profile / explicit properties、no seed、rollback 和 required-check 评估；third run `27596768301` failed after servlet web context startup with `NotAMockException`，当前为 IMPLEMENTED / FIRST-RUN-FIX #2 CI FAILED / FIRST-RUN-FIX REQUIRED。
- `WORKLOG.md`：执行日志。
...
- `NQ_CI_POSTGRES_FLYWAY_PLAN.md`：Batch 2 PostgreSQL / Flyway 当前文件；2A / 2B / 2C 均为 FROZEN / ACCEPTED；2C-HYGIENE-FIX 已 FROZEN / ACCEPTED；2D 为 IMPLEMENTED / FIRST-RUN-FIX #2 CI FAILED / FIRST-RUN-FIX REQUIRED；下一步只允许 `NQ-CI-POSTGRES-FLYWAY-2D-FIRST-RUN-FIX`，修复后再 re-run CI 并做 first-run review。
...
- `NQ_CI_POSTGRES_FLYWAY_2D_PLAN.md`：Batch 2D `nq-app` context smoke planning + implementation + first-run review 文件；IMPLEMENTED / FIRST-RUN-FIX #2 CI FAILED / FIRST-RUN-FIX REQUIRED，不新增 migration、不启动真实 provider、不触发真实交易所，不允许写成 FIRST GREEN RUN CONFIRMED / FROZEN / ACCEPTED。
