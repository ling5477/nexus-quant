...
- `NQ-GATEW-ATTEMPT-10-PREPARATION-AND-START` attempt-01 在 canonical `precreate-prerequisite` 返回 `readyForAttemptCreation=false` 后 fail closed；Attempt-10 RunId/state/runtime/clock 均未创建，OKX 未调用。再次执行前必须先完成脱敏 pre-create RCA/fix 并重新获得生产授权，不得直接重试、恢复 Attempt-09、复用旧 clock、扩大 OKX endpoint、触碰 LIVE/交易写侧或进入 freeze/archive/tag。
