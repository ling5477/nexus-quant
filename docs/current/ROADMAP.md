...
- `NQ_CI_BASELINE_PLAN.md` 已作为 CI baseline 文档落档；Batch 1 已 implemented / first green confirmed，Batch 2 PostgreSQL / Flyway hardening 已完成并冻结，Batch 3 no-outbound guard 当前为 FIRST GREEN RUN CONFIRMED（run `27634370657`，Batch 3E freeze review pending），Batch 4 security guard / secret scan 与 Batch 5 frontend E2E hardening 仍 PENDING。
- `NQ_CI_NO_OUTBOUND_GUARD_PLAN.md` 已作为 Batch 3 plan / implementation baseline 落档；Batch 3B 已实现最小 workflow / test-scope no-outbound guard，并由 GitHub Actions run `27634370657` first green confirmed（6 jobs green），当前为 FIRST GREEN RUN CONFIRMED，freeze 仍是 Batch 3E，不得直接写 FROZEN / ACCEPTED。
