...
GateW Attempt-11 operational scope remediation ACCEPTED / CI GREEN
  ↓
NQ-GATEW-ATTEMPT-12-PREPARATION-AND-START BLOCKED / STARTUP FAILED / TERMINALIZED / ROLLED BACK
  ↓
...
- `b103069d...` 是服务器 current 与 Attempt-13 已 sealed 的 immutable release；worker 已停止，release/DB/journal/heartbeat/evidence/credential reference 保留。`c16f27c3...` 保留为上一 last-known-good rollback release；Attempt-10/11/12 的失败 release 与 evidence 均保留；Attempt-13 的唯一 RunId 不得复用或自动重试。
