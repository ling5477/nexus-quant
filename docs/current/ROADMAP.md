...
- GateT：已完成 GateT-0 planning、GateT-1 backend / frontend 最小只读切片、GateT-2 backend / frontend 最小只读切片和 GateT-3 work order；仍不是 `FROZEN`（已冻结）、`ACCEPTED`（已接受）或 `TAGGED`（已打 tag）。
...
- GateT-3 work order entry：`docs/current/GATET_3_INCIDENT_REPLAY_REVIEW_WORKFLOW_WO.md`，`PLAN READY / NOT IMPLEMENTED / READY TO COMMIT`（规划已就绪 / 未实现 / 可进入提交前复核）。
...
| GateT-3 | `PLAN READY / NOT IMPLEMENTED`（规划已就绪 / 未实现） | Incident / Replay Review Workflow work order；候选 `GET /api/incidents/replay/review/overview`，尚未实现 |
