...
日期：2026-06-17
状态：Batch 4A plan review PASS / ACCEPTED；Batch 4B secret scan minimal implementation **FROZEN / ACCEPTED**（run `27674393780`，frozen baseline commit `31540de8`）；Batch 4C overall security artifact/log redaction baseline **FROZEN / ACCEPTED**（overall freeze review `NQ_CI_SECURITY_GUARD_BATCH_4C_FREEZE_REVIEW.md`；4C-A plan review PASS / ACCEPTED；4C-B pre-upload artifact redaction gate **FROZEN / ACCEPTED**，immutable run `27701669084`，frozen baseline = `ci.yml` pre-upload redaction gate step blob `4a40ef78` / commit `c734102d`；4C-C log redaction proof **FROZEN / ACCEPTED**，`NQ-CI-SECURITY-GUARD-BATCH-4C-C-FREEZE-REVIEW`，2026-06-18，P0/P1/P2 blockers = 0，immutable green run `27732660516`，14 类 pattern 真实值命中 = 0，未打印 secret value / 完整匹配行）。Batch 4F dependency audit plan review **PASS / ACCEPTED**，execution sequence **SYNCED / ACCEPTED**，4F-A **IMPLEMENTED / READY FOR REVIEW**，4F-B/4F-C/4F-D/4F-E/4F-F **NOT STARTED**；Static workflow assertion first-run review **FAIL / FIRST-RUN-FIX REQUIRED**（run `28312306143` overall success，但 workflow log hygiene 未达标）；production log shape fix **IMPLEMENTED / PENDING RE-RUN**（2026-06-28，生产 fingerprint label 改为 neutral credential fingerprint，workflow pattern 运行时拼接，OKX raw-body suppression test class renamed；需 GitHub Actions re-run review-2）；Batch 5 frontend E2E hardening PENDING。Batch 3 no-outbound guard 仍 FROZEN / ACCEPTED（run `27634370657`）。Batch 4B FROZEN 只冻结 minimal secret scan baseline；Batch 4C overall 已冻结 artifact/log redaction baseline；不得把 static workflow assertion 写成 frozen；不得把 4F-B 至 4F-F dependency audit 写成 implemented；不得把 Batch 5 写成 started。
...
- Status: **FROZEN / ACCEPTED**（first run `27662197509` FAILED（gate-c generic-api-key FP）→ first-run-fix `31540de8` APPLIED → second run `27674393780` 7/7 jobs green → Batch 4E freeze review PASS，2026-06-17）。frozen baseline = commit `31540de8` 的 secret-scan job + first-run / second-run / freeze docs。详见「Batch 4E: freeze review」。
...
- Status: 4C-A plan review **PASS / ACCEPTED**；4C-B pre-upload artifact redaction gate **FROZEN / ACCEPTED**（`NQ-CI-SECURITY-GUARD-BATCH-4C-E-PRE-UPLOAD-REDACTION-GATE-FREEZE-REVIEW`，2026-06-17，P0/P1/P2 blockers = 0；frozen baseline = `ci.yml` pre-upload redaction gate step blob `4a40ef78` / commit `c734102d`，immutable green run `27701669084`）；4C-C log redaction proof **FROZEN / ACCEPTED**（`NQ-CI-SECURITY-GUARD-BATCH-4C-C-FREEZE-REVIEW`，2026-06-18，P0/P1/P2 blockers = 0；immutable green run `27732660516`，7 jobs 全复核，14 类 pattern 真实值命中 = 0，未打印 secret value / 完整匹配行）；Batch 4C overall security artifact/log redaction baseline **FROZEN / ACCEPTED**（`NQ-CI-SECURITY-GUARD-BATCH-4C-FREEZE-REVIEW`，2026-06-18，P0/P1 blockers = 0，详见 `NQ_CI_SECURITY_GUARD_BATCH_4C_FREEZE_REVIEW.md`）。Static workflow assertion 已最小实现为 `Verify CI log redaction proof` step，状态 **IMPLEMENTED / PENDING FIRST CI RUN**；下一步只能是 static assertion first-run review、Batch 4F later plan、Batch 5 planning，或暂停 CI 线；**不得把 static workflow assertion 写成 frozen；不得把 Batch 4F / Batch 5 写成 started 或 implemented**。
...
- 4C-C log redaction proof：**FROZEN / ACCEPTED**（`NQ-CI-SECURITY-GUARD-BATCH-4C-C-FREEZE-REVIEW`，2026-06-18，P0/P1/P2 blockers = 0；前序 plan + plan review 28 项满足，独立成文 `docs/current/NQ_CI_LOG_REDACTION_PROOF_PLAN.md`；freeze review 文档见 `docs/current/NQ_CI_LOG_REDACTION_PROOF_FREEZE_REVIEW.md`）。基于 immutable green run `27732660516`（commit `a6d4bf74`，7/7 jobs green，ci.yml blob `4a40ef78` 与 HEAD 等价）的 review-time per-job log proof：7 jobs 全复核，14 类 pattern 真实值命中 = 0，仅 disposable CI 值（`123456` / `nq_ci_password`）/ Spring ephemeral dev password / platform `***` mask / step-script 回显非阻断 FP；proof 不输出 secret value / 完整匹配行。本轮**未改 ci.yml**（静态断言列为 optional future hardening / not implemented），未读本地 logs、未上传 logs artifact。**Batch 4C-C = FROZEN / ACCEPTED；Batch 4C overall = FROZEN / ACCEPTED**（overall freeze review 见 `NQ_CI_SECURITY_GUARD_BATCH_4C_FREEZE_REVIEW.md`）。
- 4C-C static assertion addendum（2026-06-28）：**FIRST-RUN-FIX APPLIED / PENDING RE-RUN REVIEW**。`.github/workflows/ci.yml` 的 `secret-scan` job 已包含 `Verify CI log redaction proof` step；clean path 只输出 `PROOF_OK`；failure path 只输出 `REDACTION_HIT rule=<rule> file=<file>`；任一 forbidden pattern 命中即 fail closed。First-run review run `28312306143` 因 workflow log 中仍出现 sensitive key-shape 而 FAIL；本修复将 OKX/Binance production fingerprint 输出 label 收口为 neutral credential fingerprint，将 workflow redaction patterns 改为 runtime 拼接且不打印完整 pattern，并将 OKX suppression test 类名改为 raw-body wording。GitHub Actions re-run review-2 未执行，故不得写成 green / frozen。
...
#### Batch 4B first-run fix（FIRST-RUN-FIX APPLIED / PENDING SECOND CI RUN）

任务 `NQ-CI-SECURITY-GUARD-BATCH-4B-FIRST-RUN-FIX`，2026-06-17。

定位（本地复现）：本轮在本地 MINGW64 用 pinned gitleaks `8.18.4` Windows CLI 复刻 CI 扫描（同一排除清单 + staging + `--redact`），从 redacted JSON 报告只提取 RuleID / File / Line（不读 Secret / Match）：

- CI 命中的唯一 finding = `docs/gates/gate-c/WORK.md`，RuleID `generic-api-key`，约 line 325，内容是一个**非敏感的 WebSocket client request UUID**（`client.request.id`）；该卷宗中真实凭证已脱敏为 `apiKey=<masked>`（line 327）。确认为 **false positive，不是真实 credential**（P0=0）。
- 本地额外多出的 4 个 `private-key` finding（Binance fake 测试私钥 / PEM 协议常量）是 Windows 反斜杠路径导致 forward-slash `paths` allowlist 在本地不匹配的假象；CI（Linux，正斜杠）下这 4 个已被现有 allowlist 抑制——这正是 CI 只报 `leaks found: 1` 的原因。

修复（最小、精确）：

1. 可见性：`secret-scan` gitleaks step 失败分支改为从 redacted JSON 报告输出 sanitized finding metadata——**仅 RuleID / File（去 staging 前缀）/ StartLine-EndLine / Fingerprint**，绝不输出 Secret / Match / 匹配行 / commit / author。保持 `--redact`、fail closed（仍 `exit 1`）、tracked safe paths only、no full-history scan、不上传报告。
2. 精确 allowlist：gitleaks inline 配置 `paths` 增加单文件条目 `.*docs/gates/gate-c/WORK\.md$`（带注释说明为 frozen Gate-C 诊断卷宗的 generic-api-key UUID FP）。未放宽核心规则、未关闭 default ruleset、未 broad allowlist、未删任何测试样例、未改 frozen 卷宗本身（`docs/gates` 不在允许修改清单，且为冻结归档）。
...
- Status: **PASS / FROZEN / ACCEPTED**（`NQ-CI-SECURITY-GUARD-BATCH-4B-FREEZE-REVIEW`，2026-06-17，P0/P1/P2 blockers = 0）。
- Scope: 冻结 Batch 4B minimal secret scan baseline；同步 `docs/current` current facts 与 next action。本 freeze 只冻结 secret scan baseline，不进入 Batch 4C / 4F / Batch 5，不改 workflow / 代码 / 测试 / migration。

freeze review 复核（基于 immutable run `27674393780`，commit `31540de8`，重新拉取 job logs + HEAD config 校验）：

- ci.yml 自 green-run commit `31540de8` 起未变（`git diff 31540de8 HEAD -- .github/workflows/ci.yml` 为空；其后仅 docs 提交 `7369ed4f`）。frozen baseline = 该 ci.yml secret-scan job。
- Run `27674393780` conclusion `completed / success`；7 jobs 全 `success`：Diff check、No-outbound guard、Backend Maven test、PostgreSQL / Flyway smoke、Frontend build、Research quality gate、Secret scan。
- secret-scan job（`81846054679`）re-fetch 证据：`Installed gitleaks version: 8.18.4`；`gitleaks detect --no-git --redact` -> `scan completed in 868ms` -> `no leaks found`；`tracked=1303 safe_scanned=1300 excluded=3`（排除恰为三个 `.env.example` 模板）；custom backstop `no non-allowlisted matches`；`GITHUB_TOKEN Permissions: Contents: read, Metadata: read`（无 write / id-token）；无 `continue-on-error` / repository secret / `gitleaks-action` / `GITLEAKS_LICENSE`。
- HEAD config 完整性：`useDefault = true`（default ruleset 未关）；gate-c allowlist 为单文件 `.*docs/gates/gate-c/WORK\.md$`（1 条）；4 Binance fake-key / PEM 协议常量 path allowlist（gitleaks）+ backstop `allow_pem` 同 4 文件，均精确；`--no-git --redact --exit-code 1`（tracked-tree-only、no full-history、fail closed）。
- 安全：日志未输出 secret value / matched line / Secret / Match / commit / author（0 finding，sanitized 失败分支未触发；日志中相关字样仅为 runner 回显的 step 脚本本体，非执行输出、非真实凭证）。无真实 API key / secret / passphrase / token / private key / credential material；`token: ***` mask。

frozen 边界：本 freeze 接受当前 `dev` minimal secret scan baseline，固定以下事实——secret-scan job 用 pinned gitleaks 8.18.4 CLI（非 action / 无 license）+ custom regex backstop，只扫 tracked safe paths（排除 `.env*` / secrets / credentials / `*.pem` / `*.key` / `*.p12` / `*.jks` / `*.keystore` / target / node_modules / dist / build / coverage / logs / dumps / backups / `.git`），no full-history scan，`--redact` fail closed，sanitized finding 可见性（仅 RuleID / File / Lines / Fingerprint），精确 allowlist（4 Binance + 1 gate-c + 3 placeholder marker），`contents: read`，无 repository secret / write / id-token / continue-on-error；不开启 LIVE / AI / DH runtime，不实现 RealClient / real provider / real permission probe adapter。Batch 4C artifact/log redaction proof、Batch 4F dependency audit、Batch 5 frontend E2E hardening 仍 NOT STARTED / PENDING，不得写成 started。

freeze 必录 P3（非阻断，留作后续 hardening follow-up）：

1. forward-slash `paths` allowlist 在 Windows 反斜杠路径本地不匹配——只影响本地复现，不影响 Linux CI（CI 正斜杠正常抑制）；如需本地可复现可改 separator-tolerant 写法，但当前不改 frozen config。
2. gitleaks release binary 当前仅 pinned 版本 + `gitleaks version` 校验，无 SHA256 checksum pinning；留作供应链 hardening follow-up。
3. gitleaks 配置 inline 写入 `RUNNER_TEMP`，无 tracked single-source；如未来需要审计可评估固化为受控 config 文件（非本 freeze 范围）。
...
### Batch 4F（可选）: dependency audit / supply-chain audit

- Status: plan review **PASS / ACCEPTED**；execution sequence **SYNCED / ACCEPTED**；4F-A **IMPLEMENTED / READY FOR REVIEW**（见 `NQ_CI_DEPENDENCY_AUDIT_PREFLIGHT.md`）；4F-B / 4F-C / 4F-D / 4F-E / 4F-F **NOT STARTED**（详见 `NQ_CI_DEPENDENCY_AUDIT_PLAN.md` 与 `NQ_CI_SECURITY_GUARD_BATCH_4F_PLAN_REVIEW.md`）。
...
- Execution sequence:
  - 4F-A：dependency audit input / toolchain preflight；**IMPLEMENTED / READY FOR REVIEW**；docs-only；未改 workflow、未运行 scanner、未上传 artifact；已确认 dependency sources、lockfile/无 lockfile 状态、tool candidates、pin/checksum policy、output hygiene 和 4F-B input conditions。
  - 4F-B：sanitized advisory audit summary；NOT STARTED。
  - 4F-C：SBOM report-only；NOT STARTED。
  - 4F-D：PR dependency delta review；NOT STARTED。
  - 4F-E：GitHub Actions / CLI supply-chain pinning；NOT STARTED。
  - 4F-F：Dependabot / Renovate governance；NOT STARTED。
- Success: dependency audit 仍不阻塞已冻结 Batch 4C baseline；与 secret scan / artifact redaction / Batch 5 frontend E2E hardening 分离；任何 4F artifact / SBOM / report 上传前仍必须经过 Batch 4C redaction gate。
- Not implemented: 本轮未修改 `.github/workflows/ci.yml`，未新增 CI job，未运行外部 dependency audit 上传服务，未上传 artifact，未改 package lock / POM / pyproject。4F-A review 接受前，禁止启动 4F-B 至 4F-F。
...
- 本轮（Batch 4B freeze review）只评审 immutable run `27674393780` + HEAD config 并改允许的 5 个 docs；**未修改 `.github/workflows/ci.yml`**（frozen baseline = commit `31540de8`）。
- ci.yml 自 green-run commit `31540de8` 起未变（仅 docs 提交 `7369ed4f` 在其后）；secret-scan job 边界（`contents: read`、无 repository secret / `gitleaks-action` / `GITLEAKS_LICENSE` / `id-token` / write / `continue-on-error`、`--no-git` / `--redact`、`useDefault = true`、tracked-tree-only、4 Binance + 1 gate-c 精确 allowlist）经 run 日志 + HEAD config 复核合规。
...
- 未新增 tracked 文件（gitleaks 配置与 backstop pattern 仍 inline 写入 `RUNNER_TEMP`，不落仓库；未新增 `.gitleaksignore` / `.github/gitleaks.toml`）。
- secret-scan job 仍 `permissions: contents: read`；未注入 repository secret；未使用 write / id-token；未使用 `continue-on-error`；不依赖 `GITLEAKS_LICENSE`；未使用 `gitleaks-action`；保持 fail closed。
...
- Batch 4B secret scan **FROZEN / ACCEPTED**（run `27674393780`，frozen baseline commit `31540de8`）；Batch 4C / 4F NOT STARTED；Batch 5 仍 PENDING。
...
Batch 4B second-run review：PASS / ACCEPTED FOR FIRST GREEN RUN AFTER FIX（second run `27674393780` 7/7 jobs green）。
Batch 4B freeze review（Batch 4E）：**PASS / FROZEN / ACCEPTED**（2026-06-17，P0/P1/P2 blockers = 0）。基于 immutable run `27674393780`（commit `31540de8`）+ HEAD config 复核：secret-scan job gitleaks `8.18.4` + `--no-git --redact` `no leaks found` + backstop `no non-allowlisted matches`，`tracked=1303 / safe=1300 / excluded=3`，`useDefault = true`，4 Binance + 1 gate-c 精确 allowlist，`contents: read`，无 repository secret / write / id-token / continue-on-error / `gitleaks-action` / `GITLEAKS_LICENSE`，未输出 secret value。Batch 4B minimal secret scan 成为当前 `dev` security guard secret-scan baseline。frozen baseline = commit `31540de8` + first-run / second-run / freeze docs。Batch 4C / 4F 仍 NOT STARTED；Batch 5 仍 PENDING；Batch 3 仍 FROZEN / ACCEPTED。已记录 3 项非阻断 P3（Windows backslash 本地复现限制、gitleaks binary 无 SHA256 pin、inline config 无 tracked single-source）。Batch 4B FROZEN 不代表 Batch 4 整体完成。

## Next concrete action

Next concrete action：`NQ-CI-SECURITY-GUARD-BATCH-4C-C-FIRST-RUN-REVIEW-2`（GitHub Actions re-run evidence review）；或暂停 CI 线。不得进入 Batch 5，不得把 static workflow assertion 写成 frozen。

Batch 4B 当前 **FROZEN / ACCEPTED**（run `27674393780`，frozen baseline commit `31540de8`）；Batch 4C-A plan review **ACCEPTED**、Batch 4C-B pre-upload artifact redaction gate **FROZEN / ACCEPTED**（immutable run `27701669084`，frozen baseline = `ci.yml` pre-upload redaction gate step blob `4a40ef78` / commit `c734102d`）；Batch 4C-C log redaction proof **FROZEN / ACCEPTED**（freeze review，green run `27732660516`，14 类 pattern 真实值命中 = 0；`NQ_CI_LOG_REDACTION_PROOF_FREEZE_REVIEW.md` 已落档）；Batch 4C overall security artifact/log redaction baseline **FROZEN / ACCEPTED**（`NQ_CI_SECURITY_GUARD_BATCH_4C_FREEZE_REVIEW.md`）；Batch 4F execution sequence **SYNCED / ACCEPTED**，4F-A **IMPLEMENTED / READY FOR REVIEW**，4F-B/4F-C/4F-D/4F-E/4F-F **NOT STARTED**；Batch 5 仍 PENDING；不得把 static workflow assertion 写成 frozen；不得把 4F-B 至 4F-F dependency audit / Batch 5 写成 started 或 implemented。
