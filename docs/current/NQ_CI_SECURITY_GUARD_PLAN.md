...
- Status: 4C-A plan review **PASS / ACCEPTED**（2026-06-17）；4C-B pre-upload redaction gate **IMPLEMENTED**（commit `c734102d`）；4C-D first-run review **FAIL / FIRST-RUN-FIX REQUIRED**（first run `27698183911`，2026-06-17，completed / failure）。**pre-upload gate 本身 first-run GREEN**（gate step 在 upload 前 success、postgres-flyway job 全绿、artifact 正常上传、无 finding、无值输出），整体 run 失败仅因 secret-scan job 的 gitleaks default-ruleset 误报：4C-B 文档更新把 AWS 官方示例 access key id 写进 `docs/current/TESTING.md`（`aws-access-token` 命中，非真实凭证、非 gate 缺陷；sanitized 日志仅 RuleID / File / Lines / Fingerprint，未输出值）。Batch 4C 整体仍 **NOT FROZEN**；4C-C log redaction proof 未开始。详细见 `docs/current/NQ_CI_ARTIFACT_LOG_REDACTION_PLAN.md`「Batch 4C-A/B/D」。下一步只能是 `NQ-CI-SECURITY-GUARD-BATCH-4C-B-FIRST-RUN-FIX`（doc-only：中和示例串，不改 ci.yml / gate），或暂停 CI 线；不得写成 FIRST GREEN RUN CONFIRMED / FROZEN。
