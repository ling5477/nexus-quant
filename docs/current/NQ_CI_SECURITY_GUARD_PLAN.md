...
- 4C-C static assertion addendum（2026-06-28）：**IMPLEMENTED / PENDING FIRST CI RUN**。`.github/workflows/ci.yml` 的 `secret-scan` job 新增 `Verify CI log redaction proof` step；只在 `${RUNNER_TEMP}/nq-ci-log-redaction-proof` 生成 synthetic sanitized log，不上传 artifact；clean path 只输出 `PROOF_OK`；failure path 只输出 `REDACTION_HIT rule=<rule> file=<file>`；任一 forbidden pattern 命中即 fail closed。本地 PowerShell simulation 已证明 clean pass、synthetic assignment/raw payload fail、failure output 不含 synthetic value；GitHub Actions first-run review 未执行，故不得写成 frozen。
...
Next concrete action：`NQ-CI-SECURITY-GUARD-BATCH-4C-C-STATIC-ASSERTION-FIRST-RUN-REVIEW`（GitHub Actions 首跑评审）；或可选 Batch 4F 后续、Batch 5 planning，或暂停 CI 线。
