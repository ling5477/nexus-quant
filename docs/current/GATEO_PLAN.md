...
12. O-1 freeze review 当时 O-2 / O-3 / O-4 / O-5 / O-FREEZE 仍为 `PLANNED / NOT STARTED`；当前 O-2 Data Quality Center baseline 已进入 `PASS / ACCEPTED / FROZEN`，O-3 final status 已为 `FROZEN / ACCEPTED`，O-4 final status 已为 `FROZEN / ACCEPTED`，O-5 plan 已完成，O-5A review 与 O-5B runner binding plan 已接受，O-5B-R1 runner binding implementation 已完成；O-5B execution 当前 `BLOCKED / PENDING O-5B-R2 RUNNER BINDING REVIEW / NOT EXECUTED`，O-5 smoke execution、O-FREEZE 仍未开始。
...
| O-5B-R1 runner implementation | `IMPLEMENTED / SELF-REVIEWED / READY TO COMMIT` | 已新增 test-only manual runner，不执行 smoke |
| O-5B-R2 runner binding review | `NOT STARTED` | 后续复核 runner gate、allowlist/denylist、redaction 与默认 skip |
| O-5B smoke execution | `BLOCKED / PENDING O-5B-R2 RUNNER BINDING REVIEW / NOT EXECUTED` | manual public outbound smoke execution |
...
O-5A review 和 O-5B runner binding plan 均 P0/P1=0；O-5B-R1 已实现 test-only manual runner；下一步只允许执行 O-5B-R2 runner binding review，不得从 implementation 直接跳到 O-5B smoke execution 或 O-FREEZE。
...
1. `NQ-GATEO-O5B-R2-MANUAL-RUNNER-BINDING-REVIEW`：只复核 test-only manual runner，默认不运行，不执行真实 HTTP，不进默认 CI。
2. `NQ-GATEO-O5B-MANUAL-PUBLIC-OUTBOUND-SMOKE-EXECUTION`：仅在 runner implementation review P0/P1=0 后执行手动 public readonly smoke。
3. `NQ-GATEO-O5C-FIRST-SMOKE-RESULT-REVIEW`：只复核脱敏 smoke 结果。
4. `NQ-GATEO-O5D-DATAORIGIN-PUBLIC-OUTBOUND-DECISION-REVIEW`：单独决定是否引入 `PUBLIC_OUTBOUND`，不得在 O-5 plan 中预写成当前事实。
5. `NQ-GATEO-O5E-FREEZE-REVIEW`：冻结 O-5 baseline。
6. `NQ-GATEO-O-FREEZE-CRITERIA-PLAN-REVIEW`：只在 O-5 前置证据完成后规划 GateO freeze criteria。
