...
| O-4 | MarketData Quality UI Plan | `PASS / PLAN ONLY / NOT IMPLEMENTED`（通过 / 仅规划 / 未实现）；O-4A `PASS / ACCEPTED`（通过 / 已接受） | 已规划并评审数据质量 UI 信息架构、路由建议、字段映射、状态文案、测试策略和实现批次 | O-4B 仅允许 read-only UI implementation；不改 backend/API/migration，不做 O-5 public smoke，不做 mock AI/DH/LIVE |
...
Final decision：O-3B backend read-only API implementation 已完成并接受；O-3 final status 为 `FROZEN / ACCEPTED`。O-4 plan 已完成，O-4A review 已 `PASS / ACCEPTED`；下一步只能进入 `NQ-GATEO-O4B-MARKETDATA-QUALITY-READ-ONLY-UI-IMPLEMENTATION`，或另起 O-5/O-FREEZE plan/review；O-4B 仅允许只读 UI，O-5 manual real public smoke 仍不得提前执行。

## 9. O-4 MarketData Quality UI Plan

O-4 plan status：`PASS`（通过）/ `PLAN ONLY`（仅规划）/ `NOT IMPLEMENTED`（未实现）。

O-4 implementation status：`NOT STARTED`（未开始）。
...
| O-4B | MarketData Quality read-only page/table implementation | `ALLOWED / READ-ONLY UI ONLY / NOT STARTED` | 推荐复用 `/marketdata`，补齐前端类型和只读表格。 |
...
Final decision：O-4A UI contract plan review 已 `PASS / ACCEPTED`，允许进入 O-4B read-only UI implementation；O-4B 不得改 backend/API/migration，不得执行 O-5 manual public outbound smoke。
...
- O-4 UI plan：已 `PASS / PLAN ONLY / NOT IMPLEMENTED`；O-4A plan review 已 `PASS / ACCEPTED`，后续 O-4B 需审查前端类型、只读表格、状态文案、null 展示和 UI copy 是否避免 real-ready / live-ready / trading-authorized。
...
- O-4 UI plan 完成。
- O-4 implementation 如已启动，必须有 O-4A plan review 与 O-4B/O-4C 实现验证记录；当前仅 O-4 plan 完成，不代表 UI 已实现。
