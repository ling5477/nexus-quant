...
- date: 2026-07-09
- scope: GateT-4 frontend implementation；NQ-only；只在现有 `/strategies/validation` 页面最小只读消费 `GET /api/strategy-validation/evaluation-artifacts/preview/overview`。
- result: `IMPLEMENTED / SELF-REVIEWED / READY TO COMMIT`（已实现 / 已自审 / 可进入提交前复核）。
- changed files:
  - `frontend/src/types/evaluation-artifact-preview.ts`
  - `frontend/src/api/evaluation-artifact-preview.ts`
  - `frontend/src/hooks/useEvaluationArtifactPreviewOverview.ts`
...
- implementation:
  - 新增 `PythonEvaluationArtifactPreviewOverviewResponse`、`PythonEvaluationArtifactPreviewItem`、checksum / freshness / metric status、evidence anchor、blocker、warning、next step 前端类型，字段覆盖后端 No-file baseline 契约。
  - 新增 `getEvaluationArtifactPreviewOverview()`，仅 GET `/api/strategy-validation/evaluation-artifacts/preview/overview`；未新增 upload / import / bind / validate-file / execute / start / stop / trade 写侧 client。
  - 新增 `evaluationArtifactPreviewQueryKeys.overview()` 和 `useEvaluationArtifactPreviewOverview()`，复用现有 Axios + TanStack Query 风格，不引入新依赖、不做 aggressive polling。
  - 在 `StrategyValidationPage` 的 Shadow Validation / Consistency / Incident Review panels 附近新增 Python Evaluation Artifact Preview panel，展示 No-file baseline、counts、latestArtifactPreview 空态、schemaVersionSummary、checksumSummary、metricSummaryCoverage、blockers / warnings / nextSteps、evidenceAnchors、traceId 和固定安全边界 badges。
  - UI 文案明确页面颜色只表示诊断状态，success 不表示盈利，danger 不表示下跌；`VALID` checksum 只表示 payload integrity，`metricSummary` 不表示真实收益，`FAKE_FIXTURE_ONLY` 只表示测试 fixture，`pythonMlReady=false` 和 `pythonLiveExecutionReady=false` 必须可见。
- validation:
  - `npm run build` -> PASS / BUILD SUCCESS（通过 / 构建成功）。
  - `npm run test:e2e -- tests/e2e/strategy-validation-paper-shadow-smoke.spec.ts` -> PASS / 2 passed（通过 / 2 条通过）。
- boundary:
  - 未新增 route、Dashboard v2、artifact upload、file path input、import、manifest reader、artifact JSON reader、Python subprocess、review / acknowledge / approve / reject 写侧操作、交易按钮、start / stop / execute / trade 操作。
  ...
  - 未读取 artifact 文件或 credential，未执行 Python，未调用真实交易所，未修改 Paper / Shadow / account / order / ledger 状态。
- next action: 提交前复核当前工作区 diff 后提交；推荐 commit message：`feat(gatet): add evaluation artifact preview frontend`。
...
- NQ-only GateQ release tag and archive。
- 基于 GateQ freeze closeout commit、最新 NQ CI Baseline success、GateQ plan、freeze readiness review、freeze closeout、API / DB docs 和 GateP archive pattern 建立 `docs/gates/gate-q/`。
- 同步 root `README.md`、`docs/current/README.md`、`STATUS.md`、`ROADMAP.md`、`TESTING.md`、`WORKLOG.md`、`FACT_SOURCE_INDEX.md` 与三份 GateQ current evidence pointer。
...
```text
docs/gates/gate-q/README.md
docs/gates/gate-q/GATEQ_FREEZE_CLOSEOUT.md
docs/gates/gate-q/GATEQ_FREEZE_READINESS_REVIEW.md
docs/gates/gate-q/GATEQ_PLAN.md
docs/gates/gate-q/GATEQ_BATCH_0_6_EVIDENCE_MATRIX.md
docs/gates/gate-q/GATEQ_TESTING_EVIDENCE_SUMMARY.md
docs/gates/gate-q/GATEQ_API_AND_FRONTEND_EVIDENCE_INDEX.md
docs/gates/gate-q/GATEQ_BOUNDARY_STATEMENT.md
```
...
README.md
docs/current/FACT_SOURCE_INDEX.md
docs/current/GATEQ_FREEZE_CLOSEOUT.md
docs/current/GATEQ_FREEZE_READINESS_REVIEW.md
docs/current/GATEQ_PLAN.md
docs/current/README.md
docs/current/ROADMAP.md
docs/current/STATUS.md
docs/current/TESTING.md
...
Freeze closeout commit: 9c8cbfe740751a1896cd6afdd04d1b9141531b10
Archive commit: docs(gateq): archive GateQ freeze evidence
CI evidence: NQ CI Baseline run 28763029176 success on 9c8cbfe740751a1896cd6afdd04d1b9141531b10
GateR: PLAN / NOT STARTED ONLY
```

Findings：

- GateQ-0 planning 已由 GateQ-1..6 消费。
- GateQ-1 Strategy Evaluation Gate read-only baseline 已完成；不是交易授权。
- GateQ-2 Paper vs Shadow Comparison read-only baseline 已完成；不是交易授权。
- GateQ-3 Shadow Live no-side-effect preview skeleton 已完成；不启动 runner，不创建 shadow run。
- GateQ-4 Python Evaluation Artifact Binding Preview contract 已完成；不导入、不上传、不持久化，不代表 ML ready 或 live execution ready。
- GateQ-5 / GateQ-6 `/strategies/validation` 前端只读视图与 Evidence Matrix 已完成；不代表交易台、AI 决策中心或 Shadow Live 执行入口。

Validation：

- 写入前 `git status --short`：clean。
- `git branch --show-current`：`dev`。
- `git rev-parse HEAD` / `git rev-parse origin/dev`：均为 `9c8cbfe740751a1896cd6afdd04d1b9141531b10`。
- `git tag --list "nq-gateq-freeze"`：写入前无输出。
- `git ls-remote --tags origin "refs/tags/nq-gateq-freeze*"`：写入前无输出。
- GitHub Actions `NQ CI Baseline` run `28763029176`：status=`completed`，conclusion=`success`，headSha=`9c8cbfe740751a1896cd6afdd04d1b9141531b10`。
- 本轮最终执行 `git diff --check`、`git diff --stat`、forbidden-area diff、migration diff、risk-word scan、cached checks、commit / push / tag / remote tag verification；最终命令结果以任务最终报告为准。
- 本轮未重跑 Maven、frontend build、Playwright 或 Python；原因是 docs-only release tag / archive，未修改 Java / TypeScript / Python / tests / migration / workflow。
...
```text
NQ-GATER-PLAN / PLAN ONLY / NOT STARTED
```
...
- 基于 GateQ plan、freeze readiness review、GateQ-1..4 后端只读 / preview API、GateQ-5 / GateQ-6 前端只读视图、API / DB docs、GateP archive 与最新 CI success 做最终冻结收口。
- 本轮只允许新增 closeout 文档并同步 current docs / root README pointer。
- 不实现新功能，不改业务代码，不新增 API / migration / CI workflow / 前端页面 / 测试。
- 不调用真实交易所，不读取 credential material，不启动 Shadow Live runner，不创建 shadow run，不开启 LIVE / AI / DH runtime。
...
GateQ archive / tag: NOT DONE IN THIS ROUND / REQUIRES SEPARATE AUTHORIZATION
GateR: PLAN / NOT STARTED ONLY
```
...
- GateQ-0 planning 已由 GateQ-1..6 消费。
- GateQ-1 `GET /api/strategies/evaluation-gate` 为只读 evaluation gate baseline，不是交易授权。
- GateQ-2 `GET /api/strategies/paper-shadow/comparison` 为只读 Paper / Shadow 对照，Shadow 未实现时 fail-closed。
- GateQ-3 `GET /api/strategies/shadow-live/preview` 为 no-side-effect preview，不写库、不外联、不读 credential、不启动 runner。
- GateQ-4 `POST /api/research/evaluation-artifacts/binding-preview` 为 Python artifact binding preview contract，不导入、不上传、不持久化，不代表 ML ready 或 live execution ready。
- GateQ-5 / GateQ-6 `/strategies/validation` 前端只读视图与 Evidence Matrix 已完成；GateQ-4 在前端仍显示 `PENDING_FRONTEND_SUPPORT` / `NOT_CONNECTED`。

Validation：

- 写入前 `git status --short`：clean。
- `git branch --show-current`：`dev`。
- `git rev-parse HEAD` / `git rev-parse origin/dev`：均为 `972c0d806f33a1f511a6a2b8f944fae006ac0c28`。
- GitHub Actions `NQ CI Baseline` run `28748448316`：status=`completed`，conclusion=`success`，headSha=`972c0d806f33a1f511a6a2b8f944fae006ac0c28`。
- 本轮 closeout 后执行 `git diff --check`、`git diff --stat`、forbidden-area diff、migration diff、risk-word scan 与 cached diff checks；结果见本轮最终报告。
- 本轮未重跑 Maven、frontend build 或 Playwright；原因是 docs-only closeout，代码级验证沿用 readiness review 已接受证据与最新 CI success。
...
```text
GateQ release tag / archive 已由本文件顶部 `NQ-GATEQ-RELEASE-TAG-AND-ARCHIVE` 消费；下一步只能 `NQ-GATER-PLAN / PLAN ONLY / NOT STARTED`。
```
...
GateQ current facts after 2026-07-06: GateQ-0..6 completed; final freeze closeout PASS / FROZEN / ACCEPTED / READY FOR ARCHIVAL
```
...
该 release/archive closeout 当时只允许另起 GateQ planning；当前 GateQ 已推进至 GateQ-0..6 completed 与 final freeze closeout，下一步只允许另起 GateQ archive/tag 任务或 GateR `PLAN / NOT STARTED` planning 任务，不得借此新增 runtime、API、migration、页面、测试、CI workflow 或业务能力。
...
- 新增 canonical fixtures plan：`docs/current/NQ_DH_INTEGRATION1_CONTRACT_FIXTURES_PLAN.md`。
- 最小同步 `docs/current/NQ_DH_INTEGRATION1_DRYRUN_CONTRACT_PLAN.md`、`docs/current/NQ_DH_INTEGRATION1_DRYRUN_PLAN_REBASEN.md`、`docs/current/README.md`、`docs/current/STATUS.md`、`docs/current/ROADMAP.md`、`docs/current/WORK_ORDER.md`、`docs/current/TESTING.md`、`docs/current/WORKLOG.md`。
- 本轮仍是 docs-only / plan-only；不修改 backend、frontend、research、scripts、deploy、`.github`、migration、contracts 或 golden_cases。
...
- 固定 future NQ -> DH request fixture inventory：valid、signature missing/invalid、timestamp skew、nonce replay、source denied、payload too large、rate limited、tenant mismatch、forbidden credential/order/account/quantity/price/side/mutation、duplicate requestId、no evidence、high risk。
- 固定 future DH -> NQ response fixture inventory：ABSTAIN、OBSERVE、NO_TRADE、LONG_BIAS/SHORT_BIAS readonly、policy denied、provider disabled/timeout/budget、risk blocked、internal fail-closed、replay/audit ref gap、schema gap disallowed。
- 固定 schema gap、golden case alignment、error taxonomy alignment、no-runtime/no-live fixture boundary 和 P3-P6 后续批次。
...
修复 GateO O-1 implementation review 发现的 P1：endpointPath 未锁定为相对路径，`baseUri.resolve("//example.invalid/ticker")` 可切换 host。本轮只关闭 controlled public outbound endpoint authority escape，不执行真实 public outbound smoke，不提交。

### 完成内容

- `PublicMarketDataOutboundRequest` 不再把 blank endpointPath 归一成 `/`，空路径交给 policy fail-closed。
- `PublicMarketDataOutboundPolicy` 新增 endpoint reference validation：拒绝 `http://...`、`https://...`、`//host/path`、authority / userInfo、fragment、only-query、blank 和非法 URI；拒绝原因不回显 raw query。
- `JdkPublicMarketDataOutboundClient` 在 `baseUri.resolve(endpointPath)` 后二次校验 resolved URI 的 scheme / host / port 与 base URI 一致，防止 future policy drift 改写出站 host。
- 新增回归测试：恶意 endpoint 被 `DENIED`、attempts=0、fake server 0 请求；`/ticker?symbol=BTC-USDT` 与 `ticker?symbol=BTC-USDT` 保持 path-only + query 允许并解析到 base host。
- 补充 P2 测试覆盖：high latency -> `DEGRADED`、stale -> `STALE`、gap -> `gapCount > 0`。`DataOrigin.FAKE_SERVER` 语义保留为 O-1 fake-server baseline，后续 O-5 前再单独评估。
...
未执行 O-5 manual real public outbound smoke；未调用真实 OKX / Binance / Bybit / Gate / Coinbase / Kraken；未改 frontend / research / scripts / deploy / `.github` / migration；未新增对外 API；未读取或输出 credential material；未实现 signed request、private endpoint、private WebSocket、RealClient、real provider 或 real permission probe；未开启 LIVE；未接 AI runtime；未接 DH runtime；未下单、撤单、转账或提现。public marketdata readiness 不等于 trading authorization。

### 推荐下一步

重新执行 `NQ-GATEO-O1-PUBLIC-MARKETDATA-CONTROLLED-OUTBOUND-IMPLEMENTATION-REVIEW`。在 review PASS 前不得提交，也不得进入 O-2/O-3/O-4/O-5 implementation。

## NQ-GATEO-O1-PUBLIC-MARKETDATA-CONTROLLED-OUTBOUND-IMPLEMENTATION

日期：2026-07-01

### 本轮目标

落地 GateO O-1 public marketdata controlled outbound 的最小实现闭环：manual profile / feature flag、默认 disabled fallback、public REST allowlist、private/signed denylist、redaction/log summary、bounded timeout/retry/backoff、fake-server/no-egress 测试和 Data Quality linkage。本轮不执行真实 public outbound smoke，不接真实 provider / RealClient / real permission probe / LIVE / AI / DH runtime。

### 完成内容

- 新增 `backend/nq-adapter-api/src/main/java/com/guidinglight/nexusquant/adapter/api/publicmarketdata/**`：public outbound client 抽象、JDK manual client、disabled fallback、endpoint category、policy decision、request/result/error/settings、Data Quality summary/mapper、redactor 和 log summary。
- 新增 `backend/nq-app/src/main/java/com/guidinglight/nexusquant/app/config/PublicMarketDataOutboundConfiguration.java` 与 `backend/nq-app/src/main/resources/application-public-marketdata-manual.yml`，固定 `public-marketdata-manual` profile 与 `NQ_PUBLIC_MARKETDATA_OUTBOUND_ENABLED` feature flag；默认 `application.yml` 保持 outbound disabled。
- 扩展 `EnvSafetyGuardConfiguration` / `EnvSafetyValidator` 与对应测试，确保 manual profile 禁止 LIVE、AI、DH runtime、real provider、RealClient 和 real exchange。
- 新增 `PublicMarketDataOutboundPolicyTest`、`JdkPublicMarketDataOutboundClientTest`、`PublicMarketDataOutboundConfigurationTest` 并扩展 `EnvSafetyValidatorTest`；fake server 只使用 localhost。
- 同步 `GATEO_PLAN.md`、`STATUS.md`、`ROADMAP.md`、`TESTING.md`、`WORKLOG.md`、`docs/current/README.md` 和 root `README.md` 的 O-1 当前状态。

### 验证

- `mvn -f backend/pom.xml -pl nq-adapter-api,nq-app -am "-Dtest=PublicMarketDataOutboundPolicyTest,JdkPublicMarketDataOutboundClientTest,PublicMarketDataOutboundConfigurationTest,EnvSafetyValidatorTest" "-Dsurefire.failIfNoSpecifiedTests=false" test`：PASS / `BUILD SUCCESS`；`nq-adapter-api` 14 tests，`nq-app` 14 tests。
- `mvn -f backend/pom.xml test`：PASS / `BUILD SUCCESS`；23 个 backend reactor module 全部 SUCCESS；仅有既有非阻塞 warning。
...
关闭 `NQ_PUBLIC_MARKETDATA_OUTBOUND_ENABLED=false` 或移除 `public-marketdata-manual` profile 即可让运行时回到 disabled fallback；如需代码回滚，还原本轮 `backend/nq-adapter-api/publicmarketdata/**`、`PublicMarketDataOutboundConfiguration`、manual profile YAML、`application.yml` public-marketdata 配置段、EnvSafety diff、对应 tests 与 current docs / README diff。无 DB、CI workflow、frontend、research 或 credential 副作用。

### 推荐下一步

执行 O-1 implementation review；只有 review 接受后，才允许单独规划 O-2 Data Quality Center 或后续 O-5 manual public outbound smoke。O-5 真实 public smoke 不得提前执行，也不得进入默认 CI。
...
- 当时明确 GateO implementation **NOT STARTED**；该 O-0 历史记录已由后续 O-1 implementation 消费，当前状态以上方最新记录为准。LIVE **DISABLED**；AI **NOT STARTED**；DH runtime **NOT_INTEGRATED**；RealClient / real provider / real permission probe **NOT_IMPLEMENTED**；public marketdata readiness 不等于 trading authorization。
...
本轮执行 GateM archive Batch 1。范围只包括 plan review 批准的 GateM freeze / release / closeout evidence：`NQ_GATEM_FREEZE_READINESS_REVIEW.md`、`NQ_GATEM_FREEZE_REVIEW.md`、`NQ_GATEM_RELEASE_TAG_AND_ARCHIVE.md`。三份文件已用 `git mv` 从 `docs/current/` 移动到 `docs/gates/gate-m/freeze/`，并新增 `docs/gates/gate-m/README.md` 作为 historical archive 入口。

索引同步：

- `docs/current/README.md`：移除 current 长证据入口，改为 GateM archive pointer。
- `README.md`：修正 GateM release/archive 路径。
- `docs/gates/README.md`：登记 GateM historical archive。
- `docs/current/STATUS.md`、`ROADMAP.md`、`TESTING.md`、`WORKLOG.md`：追加本轮 Batch 1 摘要和验证记录。

验证：已运行用户指定的 `git status --short`、`git diff --check`、`git diff --stat`、`git diff --name-status`、GateM / freeze / release `rg` 搜索和禁止范围 diff。由于 `git mv` 自动 staged rename，另补 `git diff --cached --name-status` / `git diff --cached --stat` 核对 3 个 Batch 1 文件均为 `R100` rename。未运行 Maven、frontend build/E2E、Python pytest/mypy/ruff 或真实 local backend smoke；原因是 docs-only archive move。
...
下一步：进入 `NQ-DOCS-POST-GATEM-GATEM-ARCHIVE-BATCH-2`，仅处理 GateM-5 Runtime Guarded UI docs，前提是单独授权并再次验证索引引用。
...
- 冻结对象：`GATEL_1C_CAPABILITY_MATRIX_CONTRACT.md` + `GATEL_1C_CAPABILITY_MATRIX_CONTRACT_REVIEW.md`。
- 冻结 capability status enum：`CLOSED_NO_REAL`、`DISABLED_SENTINEL`、`NO_REAL_DISABLED`、`STUB_ONLY`、`NOT_IMPLEMENTED`、`FUTURE_REAL_REQUIRES_GATE`、`FORBIDDEN_IN_GATEL`、`UNKNOWN_REQUIRES_REVIEW`。
- 冻结 adapter / venue matrix：Noop、OKX、Binance、future-real placeholder、permission probe placeholder、marketdata no-real / future-real placeholder 全覆盖；OKX/Binance existing adapters 仍不是 future-real-ready，也不等于真实交易所授权。
- 冻结 trading / marketdata / credential / endpoint / permission 解释：所有真实交易能力仍为 `FUTURE_REAL_REQUIRES_GATE` 或 `FORBIDDEN_IN_GATEL`；Noop marketdata 仍为 `NO_REAL_DISABLED`；OKX/Binance endpoint default 仍为 `DISABLED_SENTINEL`；OKX/Binance credential source 仍为 unconfigured / no-real default；real credential governance bridge 仍 NOT IMPLEMENTED。
- 保留 carve-out：rawPayload producer suppression CLOSED / ACCEPTED，但 rawPayload field deletion NOT DONE / SEPARATE COMPATIBILITY TASK。
- Adapter readiness 仍 **NOT READY / NOT FROZEN / NOT AUTHORIZED**；真实交易所、LIVE、真实 credential、AI、DH runtime 仍禁止；RiskGate / OrderStateMachine / Ledger / Audit 仍不可绕过。
- 验证：`git diff --check` 通过；`git diff --stat` 与 `git status --short` 确认仅 `docs/current/**` 变更；bounded `rg` 检查未把 adapter 写成 future-real-ready，未把 real exchange / LIVE 写成 allowed，no-real / disabled / stub 未被写成真实 success。
- 边界：docs-only freeze；未修改 Java/TypeScript/Python；未新增 API/DTO/migration/workflow；未改 frontend/research/scripts/deploy；未读取 `.env` 或真实 credential；未访问外网或真实交易所；未启用 LIVE；未接 AI/DH runtime；未实现 RealClient / real provider / real permission probe / real credential governance bridge；未删除 rawPayload 字段。
- 回滚：删除 `GATEL_1C_CAPABILITY_MATRIX_CONTRACT_FREEZE_REVIEW.md`，并还原本轮对 `GATEL_1C_CAPABILITY_MATRIX_CONTRACT.md`、`GATEL_PLAN.md`、`README.md`、`ROADMAP.md`、`STATUS.md`、`TESTING.md`、`WORKLOG.md` 的同步。
- 下一步：`NQ-GATEL-1D-ERROR-MODEL-CONTRACT`，不得直接进入 implementation / real adapter。
...
只读评审 `docs/current/NQ_CI_DEPENDENCY_AUDIT_PLAN.md` 是否可以作为 Batch 4F dependency audit / supply-chain audit implementation baseline。不实现 dependency audit，不修改 workflow，不新增 CI job，不改代码 / 测试 / migration / frontend / research / scripts / deploy，不修改 POM / package lock / pyproject / requirements，不运行外部 dependency audit 上传服务，不上传 artifact，不进入 Batch 5。
...
P0/P1 = 0。P2/P3 均为后续实现治理项：dependency audit 尚未实现；GitHub Actions major tag / gitleaks checksum pinning 仍是 supply-chain hardening gap；SBOM / Dependency Review / Dependabot-Renovate 仍需单独批次。

### 复核证据

- `NQ_CI_DEPENDENCY_AUDIT_PLAN.md` 已覆盖 Java/Maven、frontend/npm、Python/research、GitHub Actions supply-chain、action SHA pinning、CLI checksum pinning、SBOM、Dependency Review、Dependabot/Renovate、CI blocking/advisory、raw artifact hygiene、Batch 4C / Batch 5 边界。
- 计划明确不运行 `npm audit fix`，不直接升级依赖，不直接修改 POM / lockfile / pyproject / requirements，不把既有 npm advisories 直接设为 blocking。
- Python research 无 lockfile 边界已处理为 advisory/report-only 起步。
- dependency tree / lockfile / SBOM / vuln report 默认不被误判为 credential；raw report / raw SBOM / raw lockfile 禁止直接上传，必须复用 Batch 4C redaction gate。
- 高置信 credential 正则在 `docs/current` / `.github` 中无命中；宽松前缀扫描命中仅作为规则/证明文本候选，未输出 secret value。
- `.github/workflows/ci.yml`、backend、frontend、research、scripts、deploy、migration diff 均为空。

### 文档更新

- 新增 `docs/current/NQ_CI_SECURITY_GUARD_BATCH_4F_PLAN_REVIEW.md`。
- 更新 `docs/current/README.md`、`STATUS.md`、`TESTING.md`、`WORKLOG.md`。
- 未修改 `docs/current/NQ_CI_DEPENDENCY_AUDIT_PLAN.md` 本体；该文件作为被评审 planning source 保留。
- 未修改 `.github/workflows/ci.yml`、backend、frontend、research、scripts、deploy、migration、POM、lockfile、pyproject、requirements、测试。
...
冻结已完成的 Batch 4C-C log redaction proof 子基线，只判断 proof 是否可接受为 FROZEN / ACCEPTED。不修改 workflow，不新增 static assertion，不改代码 / 测试 / migration / frontend / research / scripts / deploy，不上传 logs artifact。
...
- GitHub run `27732660516` metadata：commit `a6d4bf74`，event `push / dev`，status `completed / success`，7/7 jobs green。
- `ci.yml` blob `4a40ef78` 在当前 HEAD（`d39cb3b1`）、`d3e828c0`、`a6d4bf74`、`66cb3d40`、`c734102d` 均一致。
- 4C-C proof 覆盖 7 jobs：Diff check / No-outbound guard / Backend Maven test / PostgreSQL / Flyway smoke / Frontend build / Research quality gate / Secret scan。
- 14 类 high-risk pattern 真实值命中 = 0；proof 未打印 secret value，未打印可能含值的完整 matching line。
- false positive 分类完整：regex / jq 模板 step-script 回显、disposable CI 值、Spring ephemeral dev password、platform `***` mask。
...
结果：预检 clean（编辑前），branch `dev`；workflow blob 全部 `4a40ef78...`；workflow/backend/frontend/research/scripts/deploy 0 diff（编辑前）；run `27732660516` completed / success 且 7 jobs success；credential grep 候选为 workflow/docs 规则定义、前缀描述、allowlist / false-positive 说明和历史 proof 文本，未发现真实 value-bearing credential material。

### 文档更新

- 新增 `docs/current/NQ_CI_LOG_REDACTION_PROOF_FREEZE_REVIEW.md`。
- 更新 `docs/current/NQ_CI_LOG_REDACTION_PROOF_PLAN.md`：追加 freeze review 段并把当前状态收口为 FROZEN / ACCEPTED。
- 同步 `docs/current/NQ_CI_ARTIFACT_LOG_REDACTION_PLAN.md`、`NQ_CI_SECURITY_GUARD_PLAN.md`、`NQ_CI_BASELINE_PLAN.md`、`README.md`、`STATUS.md`、`TESTING.md`、`WORKLOG.md`。
...
评审 doc-only first-run fix（commit `66cb3d40`）后的 second-pass GitHub Actions run（`27701669084`），确认 secret-scan 不再命中 `docs/current/TESTING.md` aws-access-token、postgres-flyway pre-upload gate 仍 green、artifact 仍在 gate 后上传、其余 job 未回归。只评审 + 改允许的 docs，不改 `ci.yml` / 代码 / 测试 / migration。

### run 结论

second-pass run `27701669084`（commit `66cb3d40`，event push / branch dev）= **completed / success**，7/7 jobs green：
...
- `Contents: read`；`Installed gitleaks version: 8.18.4`（pinned CLI，非 action，无 `GITLEAKS_LICENSE`）。
- `tracked=1304 safe_scanned=1301 excluded=3`（排除恰为三个 `.env.example` 模板）；`gitleaks detect ... --redact` → `scan completed in 934ms` → `no leaks found` → `gitleaks: no leaks found in tracked working tree.`。
- **不再命中 `docs/current/TESTING.md` aws-access-token**：无 `RuleID=` finding 行，sanitized 失败分支未触发，未输出 secret value / matched line。custom backstop `no non-allowlisted matches over tracked safe tree.`。

### pre-upload redaction gate 证据（仍 green）

- step-level `✓ Generate PostgreSQL schema artifacts` → `✓ Pre-upload redaction gate (PostgreSQL schema artifacts)` → `✓ Upload PostgreSQL schema artifacts`（顺序正确，gate 在 upload 前）。
- gate 执行输出 `Pre-upload redaction gate: no high-risk credential pattern in artifacts/postgres-flyway (text-only, fail closed).`（只扫 `artifacts/postgres-flyway/`、required artifacts 存在/非空、无 data-row / credential finding、binary 未误杀、fail-closed 生效、未输出 secret value / matched line）。
- artifact `nq-postgres-flyway-schema-artifacts`（74664 bytes）成功上传；仍唯一 upload-artifact；未上传 raw gitleaks report；未新增 surefire / frontend / research artifact。

### 安全确认

- `contents: read`；无 repository secret / write / id-token / continue-on-error / `gitleaks-action` / `GITLEAKS_LICENSE`；`--no-git --redact`；未扫描禁止目录；未读取/输出真实 credential material；未调用真实交易所；未开启 LIVE / AI / DH；未实现 RealClient / real provider / real probe adapter。
- `git grep -nE 'AKIA[0-9A-Z]{16}' docs/current` = 0（committed tree）。

### 文档更新

- `NQ_CI_ARTIFACT_LOG_REDACTION_PLAN.md`：header + 「Batch 4C-B」status + 新增「Batch 4C-B second-pass first-run review」段 + Review decision + Next action + Boundary confirmation 推进为 FIRST GREEN RUN CONFIRMED AFTER DOC FIX。
- `NQ_CI_SECURITY_GUARD_PLAN.md`、`NQ_CI_BASELINE_PLAN.md`、`README.md`：Batch 4C 状态同步。
- `TESTING.md`、`WORKLOG.md`：新增本轮 second-pass 评审记录。
- 未改 `ci.yml` / 代码 / 测试 / migration / frontend / research / scripts / deploy。

### Review decision

PASS / ACCEPTED FOR FIRST GREEN RUN AFTER FIX。Batch 4C-B 推进为 **FIRST GREEN RUN CONFIRMED AFTER DOC FIX**；不得直接写 FROZEN / ACCEPTED（freeze 是 4C-E）。

### 下一步

`NQ-CI-SECURITY-GUARD-BATCH-4C-E`（pre-upload gate freeze review，基于 immutable run `27701669084`），`NQ-CI-SECURITY-GUARD-BATCH-4C-C`（log redaction proof planning），或暂停 CI 线。不得把 Batch 4C-B 直接写成 FROZEN；不得把 Batch 4C-C / 4F / Batch 5 写成 started。
...
评审第一次包含 pre-upload artifact redaction gate 的 GitHub Actions run（`27698183911`，commit `c734102d`，event push / branch dev），确认 gate 在真实 runner 上的行为、upload 前执行、7 个 job 是否回归、有无真实 credential 泄露。只评审 + 改允许的 docs，不改 `ci.yml` / 代码 / 测试 / migration。

### run 结论

first run `27698183911` = **completed / failure**。per-job：
...
### pre-upload redaction gate first-run 证据（GREEN）

- postgres-flyway job step-level：`✓ Generate PostgreSQL schema artifacts` → `✓ Pre-upload redaction gate (PostgreSQL schema artifacts)` → `✓ Upload PostgreSQL schema artifacts`（顺序正确，gate 在 upload 前）。
- gate 日志：`Pre-upload redaction gate: no high-risk credential pattern in artifacts/postgres-flyway (text-only, fail closed).` → 只扫 `artifacts/postgres-flyway/`、text-only 断言通过（binary 未误杀）、无 data-row / credential finding、fail-closed 生效。
- artifact `nq-postgres-flyway-schema-artifacts`（74663 bytes）成功上传；未上传 raw gitleaks report；`if-no-files-found: error` / retention 有界不变；仍为唯一 upload-artifact；未新增 surefire / frontend / research artifact。

### 失败定位与分类

- 唯一失败 = secret-scan job 的 `Run pinned gitleaks secret scan` step，`leaks found: 1`，fail closed。
- 类别 = **gitleaks default-ruleset false positive（非 gate 缺陷、非真实泄露）**：4C-B 文档更新把 gate dry-run 用的 AWS 官方文档示例 access key id（`AKIA` 前缀 + 16 字符）写进了 `docs/current/TESTING.md`，被 `aws-access-token` 规则命中。
- sanitized 日志仅 `RuleID=aws-access-token File=docs/current/TESTING.md Lines=16-16 Fingerprint=docs/current/TESTING.md:aws-access-token:16`；`--redact` 生效，**未输出 secret value / matched line / Match / Secret / commit / author**。custom backstop step 因 gitleaks step 先失败被 skip。
- 排除：非 gate pattern FP（gate green）、非 binary 误杀、非 artifact 缺失 / 非空检查失败、非 YAML / script、非 upload ordering、非真实 leak（AWS 公共文档示例，P0=0）。

### 安全确认

- secret-scan job 仍 `permissions: contents: read`；未注入 repository secret；无 `gitleaks-action` / `GITLEAKS_LICENSE` / `id-token` / write / `continue-on-error`；`--no-git --redact`；未扫描禁止目录；未做 full-history scan。
- no-outbound guard / backend / postgres-flyway / frontend / research 全 green（除 secret-scan 外 6 个 job 未回归）；未调用真实交易所；未开启 LIVE / AI / DH；未实现 RealClient / real provider / real probe adapter。

### 文档更新

- `NQ_CI_ARTIFACT_LOG_REDACTION_PLAN.md`：header + 「Batch 4C-B」+「Batch 4C-D」段 + Review decision + Next action 推进为 4C-D FAIL / FIRST-RUN-FIX REQUIRED（gate first-run green）。
- `NQ_CI_SECURITY_GUARD_PLAN.md`、`NQ_CI_BASELINE_PLAN.md`、`README.md`：Batch 4C 状态同步。
- `TESTING.md`、`WORKLOG.md`：新增本轮 4C-D 评审记录（均未再写入可被 gitleaks 命中的示例串）。
- 未改 `ci.yml` / 代码 / 测试 / migration / frontend / research / scripts / deploy。

### Review decision

FAIL / FIRST-RUN-FIX REQUIRED。pre-upload redaction gate 本身 first-run GREEN，但整体 run 红，acceptance「run green」未满足，不得写成 FIRST GREEN RUN CONFIRMED / FROZEN / ACCEPTED。

### 下一步

`NQ-CI-SECURITY-GUARD-BATCH-4C-B-FIRST-RUN-FIX`：doc-only，把 `docs/current/TESTING.md` 内 AWS 示例 access key id 改写为不可被 gitleaks 命中的形态（如 `AKIA…EXAMPLE` 省略中段，或纯文字描述），**不改 `ci.yml` / 不改 gate / 不放宽核心规则 / 不 broad allowlist**；修复后重跑 CI 并 second-pass first-run review。不得混入 Batch 4C-C / 4F / Batch 5。
...
评审 first-run fix 后的新 GitHub Actions run，只确认 backend job 和 `postgres-flyway` job 在真实 GitHub runner 上 green，并同步允许的 `docs/current` 状态记录。不进入 Batch 3-5，不修改 workflow、Java / TypeScript / Python 代码、测试代码、migration、frontend、research、scripts 或 deploy。
...
- `Diff check` / job `81645397268`: success。
- `Frontend build` / job `81645397229`: success；`npm ci` + `npm run build` passed；only known Vite chunk-size warning and existing `npm audit` advisory summary appeared。
- `Research quality gate` / job `81645397244`: success；pytest `2 passed`，mypy `Success: no issues found in 8 source files`，ruff `All checks passed!`。
- `Backend Maven test` / job `81645397239`: success。
  - `Prepare backend CI legacy account fixture`: success。
  - `Run backend tests`: success。
  - `ResearchBacktestHappyPathLocalTest`: tests=1 / failures=0 / errors=0 / skipped=0。
...
- `git status --short`：review 前为空；review 后仅允许的 `docs/current` 文档变更。
- `git diff --check`：通过。
- `git diff --stat`：已检查。
- `git show --stat --oneline --name-only HEAD`：HEAD 为 `50c4c65e ci(gatek): add explicit backend CI account fixture`。
- `git diff -- backend` / `frontend` / `research` / `scripts` / `deploy` / `backend/**/db/migration`：均无输出。
- `rg "ci-local-account|background seed|INSERT INTO accounts|seed_pid|to_regclass|exchange_account_credentials|skipTests|continue-on-error|baselineOnMigrate|flyway clean|apiKey|secret|passphrase|token|private key|credential material|LIVE|OKX|Binance|Bybit|Gate|Coinbase|Kraken" .github docs/current`：已执行；workflow 命中为允许的 explicit fixture、防护查询、Flyway safety config 和 schema artifact redaction check；docs 命中为历史记录、禁止项或边界说明。
- GitHub MCP run jobs / backend job logs / `postgres-flyway` job logs：已复核。
- `gh run view --log` retry：触发 GitHub unauthenticated rate limit；详细日志复核以 GitHub MCP decoded logs 和 workflow static inspection 为准。
...
基于 GateN-3 public marketdata adapter skeleton plan review，执行 GateN-4 marketdata sandbox fixture smoke plan review。范围只覆盖 fixture smoke 最小范围、fixture hygiene、readiness simulation matrix、timeout / rate-limit / malformed payload simulation、no-egress validation plan、forbidden carry-over list 和 GateN-5 entry criteria；不实现 fixture smoke、fake server、adapter skeleton、测试代码、API、migration、CI workflow、页面或运行时行为。

### 完成内容

- 新增 `docs/current/NQ_GATEN_MARKETDATA_SANDBOX_FIXTURE_SMOKE_PLAN_REVIEW.md`。
- 定义 fixture smoke 最小范围：fixture OHLCV bars、fixture instrument metadata、fixture ticker、fixture exchange status、source taxonomy mapping、freshness / stale / gap / error / disabled simulation、timeout simulation、rate-limit simulated error 和 malformed payload simulation。
- 定义 fixture hygiene：fixture 必须 deterministic、offline-runnable、标注 `FIXTURE` / `FAKE_SERVER` / `NO_EGRESS_SANDBOX` source，不得从真实 API raw dump 直接复制，不得包含 credential、secret、token、signature、private key、passphrase、account/order/balance/transfer/withdraw/private payload material。
- 定义 readiness simulation matrix：`FRESH`、`STALE`、`GAP`、`ERROR`、`DISABLED`、`PENDING_BACKEND_SUPPORT`；所有 readiness 结果均为 diagnostic-only，不代表 trading authorization。
- 定义 no-egress validation plan：默认不得访问真实 `okx.com`、`binance.com`、`bybit.com`、`gate.io`、`gate.com`、`coinbase.com`、`kraken.com`；unknown host / unknown path / private endpoint / signed endpoint / credential lookup / real permission probe 必须 fail closed。
- 明确 forbidden carry-over：不得调用 private `TradingAdapter`、signed/private/account/order/balance/transfer/withdraw/user-data-stream endpoint、permission probe real execution、credential、real provider、LIVE、AI、DH runtime。
- 明确 GateN-5 entry criteria：只有在 GateN-4 fixture smoke implementation 单独授权、实现、验证、接受后，才允许进入 Runtime UI Sandbox Source Display；GateN-5 只能显示 source / readiness / diagnostic，不得显示 real-ready / live-ready / trading-authorized。
- 同步 `README.md`、`docs/current/README.md`、`STATUS.md`、`ROADMAP.md`、`TESTING.md`、`NQ_NEXT_PHASE_PLAN.md`、`NQ_GATEN_PUBLIC_MARKETDATA_SANDBOX_PLAN.md`、`NQ_GATEN_FAKE_SERVER_NO_EGRESS_TEST_PLAN.md`、`NQ_GATEN_PUBLIC_MARKETDATA_ADAPTER_SKELETON_PLAN_REVIEW.md`。
...
未改 `backend/**`、`frontend/**`、`research/**`、`scripts/**`、`deploy/**`、`.github/**` 或 `backend/**/db/migration/**`；未新增 API / migration / 页面 / E2E / CI workflow；未实现 adapter skeleton；未实现 fake server；未实现 fixture smoke；未新增测试代码；未调用真实 OKX / Binance / Bybit / Gate / Coinbase / Kraken API；未读取或输出 credential material；未开启 LIVE；未接 AI runtime；未接 DH runtime；未实现 RealClient / real provider；未执行真实 permission probe；未下单、撤单、转账或提现。public marketdata readiness 不等于 trading authorization；fixture smoke 不等于 real exchange connectivity；fake server 不等于 real provider。
...
冻结 NQ 侧 limited dry-run runtime client implementation 的边界、文件范围、安全门槛、请求/响应约束、测试要求、禁止项、回滚要求和后续验收条件。本轮为 `WORK_ORDER_ONLY + NQ_SCOPED_RUNTIME_CLIENT_IMPLEMENTATION_PLAN + CROSS_REPO_BOUNDARY_FREEZE + NO_CLIENT_IMPLEMENTATION + NO_REAL_HTTP + NO_REAL_PROVIDER + NO_LIVE`。

### 完成内容

- 新增 `docs/current/NQ_DH_INTEGRATION1_NQ_RUNTIME_CLIENT_WO.md`。
- 同步 `docs/current/README.md`、`STATUS.md`、`ROADMAP.md`、`WORK_ORDER.md`、`API.md`、`TESTING.md` 与本文件。
- 冻结下一轮 NQ limited runtime client 的 default disabled、dev/test only、production disabled、emergency kill switch、missing config / endpoint URL / timeout / parse / response policy fail-closed。
- 固定唯一允许方向为 `NQ limited dry-run client -> DH POST /api/ai/decision-dry-runs`；禁止 DH -> NQ callback / DB / order / risk / ledger / paper / live，禁止 NQ through DH 调 provider 或 exchange。
- 固定 request envelope、canonical `X-NQ-DH-*` header、HMAC value material、timeout/retry/idempotency、response action whitelist、audit/logging redaction、error taxonomy、no-side-effect integration point、测试矩阵和回滚要求。
- 明确 `NQ_DRYRUN` 仍为 review-gated source，不进入 production allowlist；`LONG_BIAS / SHORT_BIAS` 只能作为 readonly bias，不得映射为 `BUY / SELL`。
- 在 DH dev 仅做 `docs/current` 最小状态同步；NQ dev 只读确认，无修改。

### 验证

- NQ worktree `git status --short`：PASS；仅允许的 `docs/current` 修改与新增 WO 文件。
- NQ worktree `git branch --show-current`：PASS；`nq-dh-i1-nq-runtime-client-wo`。
...
- NQ boundary `rg`：PASS / REVIEWED；8587 行命中为既有 docs/backend 业务词、历史/禁止语境或本轮边界说明；因 forbidden-scope diff 为空，本轮无新增 backend runtime client / HTTP / provider / trading side effect。
- NQ dev read-only guard：PASS / SCOPED EMPTY；`dev` 分支，最终只读 status 显示既有 `research/py` dirty/untracked 变更，但 NQ-DH scoped unstaged 与 staged diff 均为空；本轮未修改 NQ dev。
- DH `mvn -ntp -Pquality validate`：BUILD SUCCESS；19/19 reactor SUCCESS；0 Checkstyle violations；Spotless check passed。
- 未运行 NQ Maven full / targeted Integration0 / targeted Integration1；原因是本轮 docs-only / work-order-only，未修改 Java、测试、contracts、fixture 或 migration，不声明 NQ Maven PASS。

### 边界

未实现 NQ runtime client；未新增 Java 生产代码或测试代码；未新增 WebClient / RestTemplate / OkHttp / HttpClient；未真实调用 DH；未真实 outbound HTTP；未接 real provider；未读取或输出 credential、token、cookie、apiKey、apiSecret、passphrase；未修改 NQ dev；未改 contracts / OpenAPI / JSON Schema / golden_cases / fixture JSON / migration；未触碰 order / execution / risk / ledger / account / paper / live；未把 DH output 写成自动交易信号；未把 `LONG_BIAS / SHORT_BIAS` 映射为 `BUY / SELL`；未接 Agent / LangGraph；未开启 LIVE。
...
只做 NQ-DH Integration-1 joint runtime dry-run test close review，审查 joint runtime dry-run tests 与 blocker fix 是否可以关闭，并判断是否允许进入下一阶段 Integration-1 mock runtime close review。本轮不继续实现功能，不修改 Java 生产代码，不修改测试代码，不修改 contracts / OpenAPI / JSON Schema / golden_cases / migration，不真实调用 DH，不真实 HTTP，不接 provider，不开启 LIVE。

### 审查结论

- `SIGNATURE_MATERIAL_SOURCE_NORMALIZATION_MISMATCH`: **FIXED / CLOSED**。NQ signing material 与 DH verifier 使用 wire-level canonical source value `NQ_DRYRUN`；source allowlist / tenant-source pair 在验签后 exact match；lowercase / alias source denied；signature material mismatch 返回 `SIGNATURE_INVALID`。
- `SCHEMA_VERSION_MISMATCH`: **FIXED / CLOSED**。NQ `DEFAULT_SCHEMA_VERSION=1.0.0`；NQ 接受 DH endpoint 实际 response `schemaVersion=1.0.0`；invalid schemaVersion 仍 fail-closed。
- Joint test boundary: **PASS**。测试只使用 fake transport / in-memory / MockMvc / test-only vector，未真实调用 DH、未真实 HTTP、未访问 localhost 真实服务、未访问外网、未接 provider、未开启 LIVE。
- Success path: **PASS**。覆盖 signed dry-run request、canonical `X-NQ-DH-*` headers、UTC `Z` timestamp、unique nonce、`dryRun=true`、`source=NQ_DRYRUN`、forbiddenCapabilities、DH readonly envelope、NQ record-only / bias-only acceptance。
- Fail-closed: **PASS**。覆盖 missing/invalid signature、timestamp shape/window、replay、source/tenant mismatch、dryRun=false、forbidden execution material、payload/rate/memory/audit failure、disabled/kill-switch/config/timeout/parse/error envelope、invalid response fields 与 executable response。
- No-side-effect: **PASS**。无 real HTTP、无 provider call、无 order / execution / risk / ledger / account / paper / live mutation，无 credential logging 或 secret exposure。
- Audit / trace / record: **PASS**。DH 写 `auditRef / replayRef / traceSummary`；NQ 记录 `requestId / traceId / tenantId / decisionId / auditRef` 与 dry-run result；fail-closed reason 可追踪且不记录 secret、credential 或 executable order payload。

### 验证

- NQ `git status --short`：PASS；close review 写入前为空，写入后仅允许 `docs/current`。
- NQ `git branch --show-current`：PASS；`nq-dh-i1-joint-runtime-dryrun-test-impl`。
- NQ `git diff --check`：PASS；无 whitespace error。
...
- NQ broad boundary `rg`：REVIEWED；命中约 8899 行，为既有业务词、历史/禁止语境或测试断言；结合 scoped diff 与 targeted implementation scan 未发现新增越界。
- NQ `mvn -ntp -f backend/pom.xml test`：BUILD SUCCESS；23/23 reactor SUCCESS。
- NQ `mvn -ntp -f backend/pom.xml -pl nq-app -am "-Dtest=*Integration0*" "-Dsurefire.failIfNoSpecifiedTests=false" test`：BUILD SUCCESS；17 tests。
- NQ `mvn -ntp -f backend/pom.xml -pl nq-app -am "-Dtest=*Integration1*" "-Dsurefire.failIfNoSpecifiedTests=false" test`：BUILD SUCCESS；18 tests。
- NQ `mvn -ntp -f backend/pom.xml -pl nq-app -am "-Dtest=DhDryRun*Test,NqDhIntegration1StubRecorderNoSideEffectTest" "-Dsurefire.failIfNoSpecifiedTests=false" test`：BUILD SUCCESS；30 tests。
- NQ `mvn -ntp -f backend/pom.xml -Pquality validate`：PROFILE MISSING / NOT EFFECTIVE QUALITY GATE；Maven returned `BUILD SUCCESS`，但 requested profile `quality` does not exist，不声明 NQ quality PASS。
- DH `mvn -ntp -pl dh-api -am test`、`mvn -ntp -pl dh-usecase -am test`、`mvn -ntp -Pquality validate`：BUILD SUCCESS。
- NQ dev read-only guard：SCOPED EMPTY / NOT_FULL_WORKTREE_GATE；任务输入声明存在非本轮 unrelated dirty，本轮只确认 NQ-DH / Integration-1 scoped diff 为空，未修改 NQ dev。
...
未修改 Java 生产代码；本 review 未修改测试代码；未修改 NQ dev；未改 contracts / OpenAPI / JSON Schema / golden_cases / migration；未真实调用 DH；未真实 HTTP；未访问 localhost 真实服务或外网；未接 provider；未读取或输出 credential、token、cookie、apiKey、apiSecret、passphrase；未接 AI / LangGraph；未开启 LIVE；未触碰 order / execution / risk / ledger / account / paper / live；未把 `LONG_BIAS / SHORT_BIAS` 映射为 `BUY / SELL`；未把 Runtime integration 写成 started；未把 DH 写成 integrated。
...
只做 Integration-1 mock runtime / test-only 阶段的里程碑关闭审查，并复位工作纪律。本轮不继续实现功能，不新增 WO，不修改 Java 生产代码，不修改测试代码，不修改 contracts / OpenAPI / JSON Schema / golden_cases / migration，不真实调用 DH，不真实 HTTP，不接 provider，不开启 LIVE。

### 审查结论

- Milestone close：**PASS / CLOSED / ACCEPTED**。Integration-1 mock runtime / test-only 里程碑可关闭。
- Discipline reset：**PASS**。上一轮 close review 为 docs-only commit；blocker fix 独立提交；本轮写入前 NQ worktree 与 DH dev 无未提交 close-review docs；NQ dev 只读 status 显示 unrelated backend untracked 文件，NQ-DH / Integration-1 scoped diff 为空。
- Phase chain completeness：**PASS**。Runtime API contract review、DH runtime API WO、DH endpoint implementation / close review、NQ runtime client WO、NQ limited client implementation / close review、joint runtime dry-run test WO、implementation、blocker fix 与 close review 均已完成并提交。
- Mock runtime scope：**PASS**。当前证据仅为 test-only / fake transport / MockMvc / in-memory validation；不代表 runtime integration started、DH integrated、real HTTP ready、provider ready 或 LIVE ready。
- Security boundary：**PASS**。HMAC source wire value、source exact allowlist、tenant/source pair exact match、schemaVersion `1.0.0`、invalid schemaVersion fail-closed、executable action fail-closed、bias-only handling均已由上一轮证据接受。

### 验证

- NQ worktree commit hygiene：PASS；branch=`nq-dh-i1-joint-runtime-dryrun-test-impl`；status 写入前无输出。
- DH commit hygiene：PASS；branch=`dev`；status 写入前无输出。
- NQ dev read-only：PASS / SCOPED EMPTY WITH UNRELATED DIRTY；本轮只读，`git status --short` 显示 unrelated backend untracked 文件；NQ-DH / Integration-1 scoped diff 为空；未修改 NQ dev。
- NQ worktree forbidden-scope pre-write diff：PASS / EMPTY；未改 NQ production code、test code、frontend、research、scripts、deploy、`.github`、contracts、golden_cases 或 migration。
- DH forbidden-scope pre-write diff：PASS / EMPTY；未改 DH production code、contracts、golden_cases 或 migration。
- Maven：本轮未重跑；沿用上一轮已记录并接受的 NQ backend full / Integration0 / Integration1 / dry-run targeted PASS 与 DH `dh-api` / `dh-usecase` / quality PASS。NQ `quality` profile missing，不写 quality gate PASS。
...
未修改 Java 生产代码；未修改测试代码；未修改 NQ dev；未改 contracts / OpenAPI / JSON Schema / golden_cases / migration；未真实调用 DH；未真实 HTTP；未访问 localhost 真实服务或外网；未接 provider；未读取或输出 credential、token、cookie、apiKey、apiSecret、passphrase；未接 AI / LangGraph；未开启 LIVE；未触碰 order / execution / risk / ledger / account / paper / live；未把 `LONG_BIAS / SHORT_BIAS` 映射为 `BUY / SELL`；未把 Runtime integration 写成 started；未把 DH 写成 integrated。
...
## NQ-GATES-2-FRONTEND-CONSISTENCY-DRILLDOWN-IMPLEMENTATION

- date: 2026-07-08
- scope: GateS-2 frontend implementation；NQ-only；只在现有 `/strategies/shadow-runs/:shadowRunId` detail / replay 页面新增 Paper vs Shadow Consistency Drilldown panel，并消费已实现的 `GET /api/paper-shadow/consistency/drilldown?shadowRunId={shadowRunId}`。
- result: **IMPLEMENTED / SELF-REVIEWED / READY TO COMMIT**（已实现 / 已自审 / 可进入提交前复核）。
- changed files:
  - `frontend/src/types/shadow-runs.ts`
  - `frontend/src/api/shadow-runs.ts`
  - `frontend/src/api/query-keys.ts`
  - `frontend/src/hooks/useShadowRunQueries.ts`
  - `frontend/src/pages/shadow-runs/ShadowRunDetailPage.tsx`
  - `README.md`
  - `docs/current/README.md`
  - `docs/current/STATUS.md`
  - `docs/current/TESTING.md`
  ...
  - `docs/current/FACT_SOURCE_INDEX.md`
- key changes:
  - 新增 `PaperShadowConsistencyDrilldownResponse`、shadow run、latest consistency、snapshot / event summary、blockers、warnings、nextSteps、evidenceAnchors、`PaperShadowComparisonStatus` 和 divergence severity 前端类型。
  - 新增 `getPaperShadowConsistencyDrilldown(shadowRunId)`、`paperShadowQueryKeys.consistencyDrilldown(shadowRunId)` 和 `usePaperShadowConsistencyDrilldown(shadowRunId)`；canonical query key 为 `['paper-shadow', 'consistency-drilldown', shadowRunId]`。
  - 在现有 Shadow Run detail / replay 页面新增 Consistency Drilldown panel，覆盖 loading / error / missing / no report / normal / stale / diverged / blocked / failed 展示。
