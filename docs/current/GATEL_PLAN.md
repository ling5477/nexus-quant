...
- **GateL-1 adapter contract security gaps（PARTIALLY HARDENED / NOT FROZEN）**：P1-A Binance 默认 endpoint sentinel 已 **CLOSED / ACCEPTED / FROZEN**；P1-B OKX/Binance runtime credential source hardening 已 **CLOSED / ACCEPTED / FROZEN**；P1-C order ack/snapshot rawPayload producer suppression 已 **CLOSED / ACCEPTED / FROZEN**，但 `rawPayload` 字段删除 **NOT DONE**（另起兼容性任务）；P1-D Noop marketdata no-real status hardening 已 **CLOSED / ACCEPTED / FROZEN**（`subscribed=false + NO_REAL_DISABLED`）。现有合同仍不得标记 future-real-ready，不代表允许真实 OKX/Binance 或真实 marketdata 接入；GateL-1B overall hardening 仍 **NOT FROZEN**。
