...
- Durable identity：clientOrderId格式为`nq1-<40-hex durable id>`，exact值只保存在production durable DB并由recovery读取；Order为`SENT / LIVE / price=78866.70000000 / qty=0.00012540 / externalOrderId=NULL`。该identity只用于后续query/reconciliation，不得重新生成或替换。未输出credential material或provider raw response。
...
- CI history：implementation commit=`d1f0189f862239e5ff6680e50fe06d4c4624b9a0`的exact-head run=`32952486624 / failure`；唯一失败job为pinned Gitleaks `generic-api-key`对evidence完整`nq1-<40hex>` clientOrderId的false positive，其他9 jobs成功。Forward fix仅把文档改为类型化占位，production DB exact identity、代码、CI config与allowlist不变。
...
## Query-only recovery clock freshness remediation（2026-08-26）

- Receipt recovery release：product commit=`d1f0189f862239e5ff6680e50fe06d4c4624b9a0`的exact-head CI run=`32952486624 / failure`仅因本evidence曾写入完整durable clientOrderId而被pinned Gitleaks `generic-api-key`误报；forward docs commit=`fb306537803f7bd0cb99388d99567dcbd0255b9e`将其收敛为`nq1-<40-hex durable id>`占位，exact-head CI run=`32952915705 / completed / success / 10 jobs`。Production current已code-only激活为`fb306537...`，manifest=`2f996d28eccd2e485668dff5468cbd78b564de420a9e1bb71871e5d79e3feabb`；runtime按controller合同保持stopped。
- Query-only incident：consumed recovery分支没有创建新lease，也没有调用PLACE；它按production DB内既有durable clientOrderId调用`readOrderStatus`，但在本地adapter clock guard返回UNKNOWN/error，gateway精确拒绝`PILOT_RECONCILIATION_ORDER_UNKNOWN`。DB facts未变化，禁止第二PLACE、PLACE retry、新clientOrderId或Attempt-02。
- RCA：`MinimalPilotTradingVenueGateway.context()`仍读取PLACE时冻结的`CLOCK_SYNC` observation；`OkxSpotProviderAdapter.readOrderStatus/readFills`在任何private read前要求observation仍在5秒freshness window。多轮CI/deploy后该observation必然stale，查询在任何OKX order read前即被本地`CLOCK_SKEW`拒绝；这不是venue已确认NOT_FOUND，也不能据此重试PLACE。
- Minimal fix：新增封闭typed operation=`READ_CLOCK / GET /api/v5/public/time`。只有已经存在execution-boundary intent且状态不是`CREATED/CLAIMED`的reconciliation可先读取当前OKX server time/local midpoint/skew，再生成只用于order/fills query的新`RequestContext`；session/reference/trace/correlation与原binding保持不变，maximum skew/age继续取冻结scope。该public request不带credential headers，不刷新或伪造market、balance、fee、binding、session、authority、order identity或交易额度。PLACE、PLACE后的QUERY与CANCEL继续使用原clock freshness合同，不能消费新clock获得mutation授权。
- Tests：focused adapter/transport/gateway=`34/34 PASS`，覆盖stale PLACE-time context→fresh read context、READ_CLOCK失败fail closed、public exact endpoint无credential header、response byte cap、PLACE stale clock仍在transport前拒绝。Disposable V46 DB先经46/46 Flyway与最小脱敏legacy account fixture，full Maven=`23/23 modules PASS`、`nq-app=315 tests / 0 failures / 0 errors / 35 conditional skips`；临时库已强制删除。共享本地`nexus_quant`因历史V44 checksum mismatch未repair，其首次full Maven失败不计为PASS。
- Governance：GateY exact/minimal/release/runtime=`7/100/31/51 PASS`；GateW frozen=`34/37/12 PASS`；Authority checker errors=0；Java governance PASS。主工作区Shadow仍被既有`artifacts/pre-clean-3-pip-tmp` ACL阻断，同一diff在detached worktree复核`NEW_CODE_VIOLATION_COUNT=0`并已删除。Custom secret backstop=`16 files / 0 findings`；本机未安装gitleaks，pinned scan留待exact-head CI。
- Target cleanup：operator要求清理无用root `target/`。先确认其只包含已部署`fb306537...`的可重建local release，并通过SSH验证服务器current与manifest后，已删除精确目录`E:\Project\nexus-quant\target`；服务器release与`backend/**/target`未删除。
- Targeted review：只审clock trust、public/private endpoint边界、credential lifecycle、stale observation、PLACE/CANCEL mutation isolation、query-only状态机、receipt/Order/Trade/Ledger收敛与no-blind-retry；P0=0、P1=0。无migration、dependency、API、schema、价格、数量、费用、余额、lease或PLACE额度变化。
- Current production readback：current=`fb306537...`、manifest=`2f996d28...`、service=`inactive`；Flyway=`V46 / failed0`，kill=`ENGAGED/version11`。Lease lineage=`ordinal0 EXPIRED/unconsumed → ordinal1 FAILED/unconsumed → ordinal2 FAILED/unconsumed → ordinal3 FAILED/unconsumed → ordinal4 CONSUMED`，decision=4，PLACE=1，intent=`SEND_STARTED`，receipt=0，pilot order=`SENT/LIVE/externalOrderId NULL`，Trade=0，Ledger=0。完整clientOrderId只保存在production DB，未输出到tracked evidence。首次只读psql使用错误默认socket 5432而在连接前失败；从postgres process参数解析实际port=55432后只读查询成功，数据库写入=0。
- Current decision：`QUERY_ONLY_CLOCK_REFRESH_IMPLEMENTED / LOCAL_GREEN / CI_PENDING / PRODUCTION_UNCHANGED / PLACE_COUNT_1 / NO_PLACE_RETRY / CANCEL_0 / KILL_ENGAGED / LIVE_FALSE / RECEIPT_0 / RECONCILIATION_PENDING / P0_0 / P1_0`。
- Next：精确暂存本次16份Java/测试文件与本evidence，commit、push `origin/dev`并等待exact-head CI；CI全绿后只构建并code-only部署V46 release，再次调用同一consumed recovery分支。任何UNKNOWN继续fail closed；永久禁止第二PLACE。

## Public clock credential-boundary remediation（2026-08-26）

- First clock-fix commit/CI：commit=`e7e006c31e9f1e94856f7bd1fa75f39a1434be8b`已push；exact-head CI run=`32957603879 / completed / success / 10 jobs`，包含Backend Maven、PostgreSQL/Flyway、pinned Gitleaks、Java Shadow、no-outbound与E2E。
- Code-only deployment：canonical V46 release=`e7e006c3...`、15 artifacts、manifest=`41bceb4a42a37aad611bacb43e4506d9e8a58004082057a52e546c0c40a450c2`；server installer/independent verifier确认source/installed manifest、POSIX/root/link integrity与service-user write denial。Runtime三项identity与current原子切换后，独立Start证明MainPID非0、NRestarts=0、health=`UP`且DB=`UP`；随后canonical Stop/VerifyStopped通过。未运行Flyway、未新增backup、未修改schema。
- Deployment observation：组合activation的SSH被远端关闭且未返回receipt，但只读readback与JVM journal证明应用在39.897秒内健康启动，随后由operator stop触发graceful shutdown；current/env/manifest一致，无运行中残留。该不确定输出未写成deployment PASS，直到独立Start/health/Stop证据完成。
- Invocation-path incident：第一次脚本调用使用默认`ReleaseRoot=/opt/nexus-quant/current`，在write window前被strict verifier以`RELEASE_LINK_INTEGRITY_VIOLATION`拒绝；显式exact immutable root后两次consumed recovery均进入Java，但在`refreshedReadOnlyContext`因typed clock observation携带error而fail closed。每次finally均撤销临时权限；PLACE/intent/clientOrderId没有新增或替换。
- External clock proof：独立public `GET /api/v5/public/time`第一次样本RTT/skew=`295ms/81ms`，随后5样本RTT=`198..233ms`、midpoint skew=`36..55ms`，全部低于冻结maximum skew=`100ms`。因此不是持续server clock不合格，也不能以旧binding stale observation解释第二次失败。
- RCA：`READ_CLOCK`是public endpoint，但`CredentialScopedOkxSpotProviderTransport.readClock()`错误进入exact credential JIT callback；任何credential selection/decrypt/wrapper异常均被adapter收敛为`UNKNOWN_RESULT`，使public clock refresh依赖无关credential边界且失去脱敏分类。Order/fills private read尚未发生。
- Minimal remediation：runtime transport先按persisted session identity校验请求，再直接调用已经装配的typed `OkxPrivateRealTransport.readClock()`；不读取/decrypt credential。PLACE/QUERY/CANCEL/order/fills继续走exact credential JIT，没有fallback。Gateway只增加`category/auditCode`脱敏warn，禁止raw body/header/URL/credential。新增回归证明READ_CLOCK调用real transport且credential executor zero interactions；non-web composition fixture同步typed transport bean。
- Validation：focused gateway/public-clock/composition=`13/13 PASS`；disposable V46 DB经46/46 Flyway与最小legacy account fixture，full Maven=`23/23 modules PASS`、`nq-app=315 tests / 0 failures / 0 errors / 35 conditional skips`，临时库已强制删除。GateY exact/minimal/release/runtime=`7/100/31/51 PASS`，GateW frozen=`34/37/12 PASS`，Authority errors=0，Java governance PASS；detached-worktree Shadow=`NEW_CODE_VIOLATION_COUNT=0`，custom secret backstop=`5 files / 0 findings`。
