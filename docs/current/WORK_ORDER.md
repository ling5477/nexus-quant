...
I1 joint runtime dry-run test implementation: IMPLEMENTED / FULL_VALIDATION_PASS / TEST_ONLY / FAKE_TRANSPORT_ONLY / BLOCKER_FIX_APPLIED / CLOSED_BY_CLOSE_REVIEW.
...
I1 joint runtime dry-run test close review: PASS / CLOSED / ACCEPTED / REVIEW_ONLY / NO_REAL_DH_CALL / NO_REAL_HTTP / NO_PROVIDER / NO_LIVE.
I1 mock runtime milestone close review: PASS / CLOSED / ACCEPTED / REVIEW_ONLY / MOCK_RUNTIME_MILESTONE_CLOSED / NO_REAL_DH_CALL / NO_REAL_HTTP / NO_PROVIDER / NO_LIVE.
I1 mock runtime PR prep: READY / PR_PREP_ONLY / ALLOW_PR_CREATE / NO_MERGE / NO_REAL_DH_CALL / NO_REAL_HTTP / NO_PROVIDER / NO_LIVE.
...
docs/current/NQ_DH_INTEGRATION1_MOCK_RUNTIME_CLOSE_REVIEW.md
docs/current/NQ_DH_INTEGRATION1_JOINT_RUNTIME_DRYRUN_TEST_CLOSE_REVIEW.md
```

## 2.3 NQ-DH-I1-INTEGRATION1-MOCK-RUNTIME-CLOSE-REVIEW（PASS / CLOSED / ACCEPTED）

本轮只做 review-only milestone close review 与工作纪律复位，不新增 API、runtime wiring、real HTTP、provider、contracts/OpenAPI/json-schema/golden_cases 或 migration。
...
结论：Integration-1 mock runtime / test-only 里程碑可以关闭；下一步只允许 `NQ-DH-I1-MOCK-RUNTIME-PR-PREP / PR_PREP_ONLY`。

## 2.2 NQ-DH-I1-JOINT-RUNTIME-DRYRUN-TEST-CLOSE-REVIEW（PASS / CLOSED / ACCEPTED）

本轮只做 review-only close review，不新增 API、runtime wiring、real HTTP、provider、contracts/OpenAPI/json-schema/golden_cases 或 migration。
...
结论：joint runtime dry-run tests 与 blocker fix 可以关闭；下一步只允许 `NQ-DH-I1-INTEGRATION1-MOCK-RUNTIME-CLOSE-REVIEW`。

## 2.1 NQ-DH-I1-JOINT-RUNTIME-DRYRUN-TEST-BLOCKER-FIX（IMPLEMENTED / FULL_VALIDATION_PASS / CLOSED_BY_CLOSE_REVIEW）
...
完整 validation 已通过，并已由 `NQ-DH-I1-JOINT-RUNTIME-DRYRUN-TEST-CLOSE-REVIEW / PASS / CLOSED / ACCEPTED` 接受。
...
`NQ-DH-I1-JOINT-RUNTIME-DRYRUN-TEST-BLOCKER-FIX` 已按最小范围修复 `SIGNATURE_MATERIAL_SOURCE_NORMALIZATION_MISMATCH` 与 `SCHEMA_VERSION_MISMATCH`，完整 validation 已通过，并已由 close review 接受。仍不得真实调用 DH，不得真实 HTTP，不得接 provider，不得修改 contracts/golden_cases/migration，不得触碰 order / execution / risk / ledger / account / paper / live 生产链路。
