...
> NQ-DH 集成实现线：NQ-DH-I1-JOINT-RUNTIME-DRYRUN-TEST-BLOCKER-FIX / IMPLEMENTED / FULL_VALIDATION_PASS / BLOCKER_FIX_APPLIED / READY_FOR_CLOSE_REVIEW / NO_REAL_DH_CALL / NO_REAL_HTTP / NO_PROVIDER / NO_LIVE
> 下一集成任务：NQ-DH-I1-JOINT-RUNTIME-DRYRUN-TEST-CLOSE-REVIEW / READY_FOR_CLOSE_REVIEW / REVIEW_ONLY / NO_REAL_DH_CALL / NO_REAL_HTTP / NO_PROVIDER / NO_LIVE
...
I1 joint runtime dry-run test implementation: IMPLEMENTED / FULL_VALIDATION_PASS / TEST_ONLY / FAKE_TRANSPORT_ONLY / BLOCKER_FIX_APPLIED / READY_FOR_CLOSE_REVIEW.
I1 joint runtime dry-run test blockers: SIGNATURE_MATERIAL_SOURCE_NORMALIZATION_MISMATCH FIXED; SCHEMA_VERSION_MISMATCH FIXED.
...
## 2.1 NQ-DH-I1-JOINT-RUNTIME-DRYRUN-TEST-BLOCKER-FIX（IMPLEMENTED / FULL_VALIDATION_PASS / READY_FOR_CLOSE_REVIEW）

本轮只修复两个 close review blocker，不新增 API、runtime wiring、real HTTP、provider、contracts/OpenAPI/json-schema/golden_cases 或 migration。

```text
SIGNATURE_MATERIAL_SOURCE_NORMALIZATION_MISMATCH: FIXED
SCHEMA_VERSION_MISMATCH: FIXED
NQ DEFAULT_SCHEMA_VERSION: 1.0.0
HMAC source signing material: wire-level canonical value NQ_DRYRUN
source allowlist: exact match after verification
invalid signature: fail-closed
invalid source / alias source / lowercase source: fail-closed
invalid schemaVersion: fail-closed
BUY / SELL / PLACE_ORDER / CANCEL_ORDER response: fail-closed
Runtime integration: NOT STARTED
DH integrated: NO
LIVE: DISABLED
real DH call: NO
real HTTP: NO
provider: NO
contracts/OpenAPI/json-schema/golden_cases: UNCHANGED
```

完整 validation 已通过；唯一下一步为 `NQ-DH-I1-JOINT-RUNTIME-DRYRUN-TEST-CLOSE-REVIEW / REVIEW_ONLY`，本轮不执行 close review。
...
ALLOW_JOINT_RUNTIME_DRYRUN_TEST_IMPLEMENTATION: YES / IMPLEMENTED / TARGETED_TEST_PASS / BLOCKER_FIX_APPLIED
...
```text
NQ-DH-I1-JOINT-RUNTIME-DRYRUN-TEST-CLOSE-REVIEW / READY_FOR_CLOSE_REVIEW / REVIEW_ONLY / NO_REAL_DH_CALL / NO_REAL_HTTP / NO_PROVIDER / NO_LIVE
```

`NQ-DH-I1-JOINT-RUNTIME-DRYRUN-TEST-BLOCKER-FIX` 已按最小范围修复 `SIGNATURE_MATERIAL_SOURCE_NORMALIZATION_MISMATCH` 与 `SCHEMA_VERSION_MISMATCH`，完整 validation 已通过，允许进入 close review。仍不得真实调用 DH，不得真实 HTTP，不得接 provider，不得修改 contracts/golden_cases/migration，不得触碰 order / execution / risk / ledger / account / paper / live 生产链路。
