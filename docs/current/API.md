...
- `GET /api/shadow-runs/overview`：只读聚合本地 Shadow Run overview。
  ...
  - Response：`generatedAt / evidenceMetadata / diagnosticOnly / noSideEffect / notTradingAuthorization / liveDisabled / realProviderImplemented / privateTradingImplemented / aiDhRuntimeIntegrated / totalRuns / runningRuns / blockedRuns / failedRuns / completedRuns / staleRuns / latestRun / latestConsistency / divergenceSeverity / blockers / warnings / nextSteps / evidenceAnchors / traceId`。
  - `evidenceMetadata`：统一只读证据元数据，字段为 `source / availability / lastCalculatedAt / freshnessStatus / ageSeconds / staleAfterSeconds / staleReason / diagnosticOnly / noSideEffect / notTradingAuthorization / liveDisabled`。本 endpoint 的 `source=LOCAL_DB_SHADOW_FACTS`；当前没有权威 stale threshold，因此即使存在真实 `lastCalculatedAt / ageSeconds`，`freshnessStatus` 仍为 `UNKNOWN`（无法判断），不得伪造为 `FRESH`（新鲜）。
...
- `GET /api/shadow-validation/workflow/overview`：只读聚合 GateS 本地 facts 并派生 Shadow Validation Workflow operator items。
  - Query：无请求参数；不接受 request body。
  - Response：`generatedAt / evidenceMetadata / diagnosticOnly / noSideEffect / notTradingAuthorization / liveDisabled / realProviderImplemented / privateTradingImplemented / aiDhRuntimeIntegrated / totalOperatorItems / intakeCount / evidenceReviewCount / needsEvidenceCount / readyForOperatorReviewCount / blockedCount / closedRecommendationCount / latestOperatorItem / operatorItems / blockers / warnings / nextSteps / evidenceAnchors / traceId`。
  - `evidenceMetadata`：统一只读证据元数据，字段为 `source / availability / lastCalculatedAt / freshnessStatus / ageSeconds / staleAfterSeconds / staleReason / diagnosticOnly / noSideEffect / notTradingAuthorization / liveDisabled`。本 endpoint 的 `source=LOCAL_DB_VALIDATION_WORKFLOW`，复用既有 7 天 stale policy；无事实时间戳时返回 `UNKNOWN`（无法判断），空事实时 `availability=UNAVAILABLE`（不可用），不会把 `generatedAt` 伪装成 `lastCalculatedAt`。
...
- `GET /api/paper-shadow/consistency/evidence/overview`：只读聚合 Paper vs Shadow consistency evidence overview。
  - Query：无请求参数；不接受 request body。
  - Response：`generatedAt / diagnosticOnly / noSideEffect / notTradingAuthorization / liveDisabled / realProviderImplemented / privateTradingImplemented / aiDhRuntimeIntegrated / totalEvidenceItems / consistentCount / divergedCount / partialCount / notComparableCount / failedCount / staleEvidenceCount / highSeverityCount / criticalSeverityCount / latestEvidenceItem / evidenceItems / severityBuckets / freshnessSummary / metricDeltaSummary / blockers / warnings / nextSteps / evidenceAnchors / traceId`。
  - `evidenceItems`：`evidenceItemId / shadowRunId / paperRunId / consistencyReportId / strategyVersionId / datasetId / comparisonStatus / divergenceSeverity / evidenceFreshness / metricDelta / divergenceReasons / limitations / evidenceAnchors / traceId / generatedAt / diagnosticOnly / noSideEffect / notTradingAuthorization / liveDisabled / realProviderImplemented / privateTradingImplemented / aiDhRuntimeIntegrated`。
  - `comparisonStatus` 当前取值：`CONSISTENT`（一致）、`DIVERGED`（证据不一致）、`PARTIAL`（部分可比）、`NOT_COMPARABLE`（不可比较）、`FAILED`（比较失败）、`NO_REPORT`（无报告）、`UNKNOWN`（未知）。`CONSISTENT` 不表示可交易，`DIVERGED` 不表示交易风险已处理或自动处置。
  - `divergenceSeverity` 当前取值：`NONE`（无偏离）、`LOW`（低）、`MEDIUM`（中）、`HIGH`（高）、`CRITICAL`（严重）、`UNKNOWN`（未知）。`HIGH / CRITICAL` 只表示诊断优先级，不表示自动处置或交易授权。
  - `evidenceFreshness` 当前取值：`FRESH`（新鲜）、`STALE`（过期）、`MISSING`（缺失）、`UNKNOWN`（未知）；当前 freshness window 为 7 天。
  - `metricDelta` / `metricDeltaSummary` 只返回 `metricCount / comparableMetricCount / nonComparableMetricCount / topDeltaMetrics / limitationCodes / sensitiveFieldFilteredCount / rawMetricDeltaExposed=false / profitConclusionInferred=false / tradingSignalInferred=false`；不返回 raw JSONB，不伪造收益结论，不生成交易建议。
  - 固定 boundary flags：`diagnosticOnly=true`、`noSideEffect=true`、`notTradingAuthorization=true`、`liveDisabled=true`、`realProviderImplemented=false`、`privateTradingImplemented=false`、`aiDhRuntimeIntegrated=false`。
...
- `GET /api/strategy-validation/evaluation-artifacts/preview/overview`：返回 Python Evaluation Artifact binding preview 的安全空基线。
  ...
  - Response：`generatedAt / diagnosticOnly / noSideEffect / notTradingAuthorization / liveDisabled / realProviderImplemented / privateTradingImplemented / aiDhRuntimeIntegrated / pythonMlReady / pythonLiveExecutionReady / totalArtifactPreviews / validArtifactCount / invalidArtifactCount / staleArtifactCount / checksumFailedCount / latestArtifactPreview / artifactPreviews / schemaVersionSummary / checksumSummary / metricSummaryCoverage / blockers / warnings / nextSteps / evidenceAnchors / traceId`。
  - No-file baseline 当前固定 `totalArtifactPreviews=0`、`artifactPreviews=[]`、`latestArtifactPreview=null`、`pythonMlReady=false`、`pythonLiveExecutionReady=false`，并返回 `NO_ARTIFACT_SOURCE_CONFIGURED` warning。
  - `PythonEvaluationArtifactPreviewItem` 字段已在 DTO / read model 预留：`artifactPreviewId / artifactId / experimentId / strategyId / strategyVersion / strategyVersionId / datasetId / datasetVersion / parameterSetId / schemaVersion / source / checksumStatus / artifactFreshness / metricSummaryStatus / costAssumptionsStatus / slippageAssumptionsStatus / validationWarnings / limitations / evidenceAnchors / traceId / generatedAt / diagnosticOnly / noSideEffect / notTradingAuthorization / liveExecutionReady / pythonMlReady / pythonLiveExecutionReady`。GateT-4 No-file baseline 不生成 item。
  - `checksumStatus` 当前取值：`VALID / INVALID / MISSING / NOT_CHECKED / UNKNOWN`；No-file baseline summary 使用 `NOT_CHECKED=1`。未来 `VALID` checksum 只表示 payload 与 checksum 自洽，不表示策略有效或交易授权。
  - `artifactFreshness` 当前取值：`FRESH / STALE / MISSING / UNKNOWN`；No-file baseline 不读取 source，因此没有 fresh artifact item。
  - `metricSummaryStatus` 当前取值：`PRESENT / INCOMPLETE / FAKE_FIXTURE_ONLY / MISSING / UNKNOWN`；No-file baseline summary 使用 `UNKNOWN=1`。`metricSummary` 只表示离线指标摘要，不表示真实收益；`FAKE_FIXTURE_ONLY` 必须保持 fixture-only 语义，不是真实策略表现。
  - `nextSteps` 只允许保留 No-file baseline、另起 Manifest-only / schema review、保持 not trading authorization；不得把本 endpoint 扩展为 path query、upload、import、Python execution、network fetch、DB import、Paper / Shadow / LIVE run trigger 或交易授权。
- 数据来源：无 artifact source。Service 只基于固定安全边界和 UTC clock 生成 derived overview；不读取 artifact JSON 文件、manifest、任意路径、用户目录、上传文件、外部网络或 DB artifact catalog。
- No-side-effect：Controller 只有 GET；service 标记 read-only；不 INSERT / UPDATE / DELETE；不新增 repository SQL；不创建 artifact catalog、import record、strategy validation record、operator item、consistency report、review item、Paper run、Shadow run 或 LIVE run；不启动 backtest、runner、scheduler；不调用 adapter、risk write side、order/account/ledger 服务。
- 固定 boundary flags：`diagnosticOnly=true`、`noSideEffect=true`、`notTradingAuthorization=true`、`liveDisabled=true`、`realProviderImplemented=false`、`privateTradingImplemented=false`、`aiDhRuntimeIntegrated=false`、`pythonMlReady=false`、`pythonLiveExecutionReady=false`。
- 固定禁止：不提供 `POST`、`PUT`、`PATCH`、`DELETE` 或任何 upload / import / bind / execute / validate-file / write-side / 交易动作 endpoint。
- Response 禁止字段：`canTrade`、`tradeApproved`、`tradingReady`、`liveReady`、`authorizedForTrading`、`apiKey`、`secret`、`passphrase`、`token`、`privateKey`、`credentialMaterial`、`decryptedPayload`、`encryptedPayload` 真实值、private endpoint payload、`realOrderId`、`realAccountBalance`、`realPosition`、`withdrawAddress`、`transferTarget`。
...
> 上述 run-fact 端点(sim-orders / sim-trades / sim-positions / pnl-snapshots)早已在 `BacktestRunController` 实现,此前 `API.md` 漏记,本轮补记为事实。回测权益/回撤曲线后端契约与前端对接历史计划见 [BACKTEST_EQUITY_DRAWDOWN_SERIES_API_PLAN.md](../archive/current-cleanup/post-gateq/research-backtest-plans/BACKTEST_EQUITY_DRAWDOWN_SERIES_API_PLAN.md):`pnl-snapshots` 端点与 `sim_pnl_snapshots` 表已存在,**无需新增后端 API / 表 / migration**;前端消费(B1.1,equity 曲线 + 派生 drawdown)为 **planning,尚未实现**。
...
GateJ API 历史规划入口为 [GATEJ_API_PLAN.md](../gates/gate-j/GATEJ_API_PLAN.md)。该链接仅用于历史追溯，不决定当前 Gate。
