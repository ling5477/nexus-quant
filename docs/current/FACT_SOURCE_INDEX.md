...
- GateW：`IN PROGRESS / NOT FROZEN`（进行中 / 未冻结）；GateW-1、GateW-2 为 `ACCEPTED / CI_GREEN`。GateW-3 venue-rule implementation 已提交、migration conformance review 已通过，但 exact-head CI run `29241698510` 失败，当前为 `COMMITTED|CI_FAILED|FIX_REQUIRED`，尚未 accepted；唯一下一动作是 `NQ-GATEW-3-CI-BLOCKER-FIX`。dry-run order preview 仍需在 fix commit CI green 后执行 attempt-02 重新审查。GateW-2 `REAL_SMOKE=NOT_RUN`；不得把 read-only diagnostic probe、public metadata readiness、schema review、preview review 或 CI 推断为远端 permission、private trading、LIVE 或 trading authorization。
