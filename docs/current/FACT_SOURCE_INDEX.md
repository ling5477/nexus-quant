...
- [Phase7 final baseline plan](../audit/evidence/GATEAUDIT_PHASE7_FINAL_BASELINE_PLAN.md)：`ACCEPTED_PLAN / NON_RUNTIME_AUTHORITY`，定义 final acceptance matrix、residual disposition、archive/freeze/tag sequence，并把 UI V3 作为 accepted final-baseline input；其原始接受时点为 `PHASE7_NOT_STARTED`，该历史状态不覆盖后续 Phase7-E/F release identity 与当前 frozen authority。
...
- [GateAUDIT strict canonical archive](../gates/gate-audit/README.md)：13 个 substantive role（8 mandatory + 5 conditional），Phase7-A 21-row matrix 与 17-row residual 原位保留；Phase7-D historical pair=`4800ab1d9407eeb527182328263c0bae9e6c3087 / 35803472376`，本次 refresh implementation pair=`c91f26487caf92f963fab36486efa2aebe666ccb / 35832860696 / 9 of 9 SUCCESS`。首次 refresh `5bf244b8f4dbf4d857cc6d2f45eeb23ead21f3fc / 35831845804` 的 secret scan FAIL 保留于 closeout；可选 CodeRabbit 审查连接失败，未宣称 PASS。分类为 `PRETAG_CANONICAL_ARCHIVE / NON_RUNTIME_AUTHORITY`；STATUS 唯一拥有 current machine state。archive 是冻结前生成的 pre-tag snapshot；其 `PRETAG / TAG_PENDING` 历史文字原位保留，后续 Git freeze identity 由 current STATUS 与下条 release evidence 表达。
- [GateAUDIT freeze release identity]：freeze commit=`a4cf8516382b47112852fc3f8dd9e5a65a80f993`，tree=`557bb3fa6549732cc5bdf2d40251c31a963f83a0`，annotated tag=`nq-gateaudit-freeze`，tag object=`4ee65d68471c5e25874ab6a08c3e9fc420f667b7`，peeled commit=`a4cf8516382b47112852fc3f8dd9e5a65a80f993`；`dev` push exact-head CI=`35840126216 / completed / success / 9 of 9`，release checker 与 release-mode archive checker errors=`0`。current machine authority 归 [STATUS.md](STATUS.md) 所有；冻结 archive 内容为 [GateAUDIT archive](../gates/gate-audit/README.md) 中的 pre-tag snapshot，不回写历史。

- [Original Scope Traceability Reconciliation](../audit/evidence/GATEAUDIT_ORIGINAL_SCOPE_TRACEABILITY_RECONCILIATION.md)：39 项分类、未分类 0；原 BLOCKED/S10 gap=1 保留为历史，最新关闭附录以 SQL audit evidence pair `0e200e807a1347e5ec6acd24975fa3531a31c90d / 35828020513` 将 S10 处置为 COMPLETED、remaining gaps=0。分类为 `CURRENT_AUDIT_EVIDENCE / NON_RUNTIME_AUTHORITY`；当前机器状态仅由 STATUS 拥有。
- [SQL ownership repository audit](../audit/evidence/GATEAUDIT_SQL_OWNERSHIP_REPOSITORY_AUDIT.md) 与 [independent review](../audit/evidence/GATEAUDIT_SQL_OWNERSHIP_REPOSITORY_INDEPENDENT_REVIEW.md)：435 runtime 调用点、91 owner、28 重复候选组；初审 FAIL 与修订后 delta PASS 均保留。evidence pair=`0e200e807a1347e5ec6acd24975fa3531a31c90d / 35828020513 / 9 of 9 SUCCESS`。分类为 `ACCEPTED_AUDIT_EVIDENCE / NON_RUNTIME_AUTHORITY`，不赋予 Phase7-E 执行资格。
- [Logging sensitive-data negative proof](../audit/evidence/GATEAUDIT_LOGGING_SENSITIVE_DATA_NEGATIVE_PROOF_AUDIT.md)：修复前在 `prod` profile 的 Spring Boot/Logback console encoder 最终 bytes 中，13/13 synthetic canary 可见，包括 Throwable nested cause；安全身份仍保留。分类为 `HISTORICAL_BLOCKED_AUDIT_EVIDENCE / NON_RUNTIME_AUTHORITY`；该失败事实保持有效且不证明真实凭证泄露，后续接受的实现见独立 implementation evidence。
- [Logging sensitive-data protection implementation](../audit/evidence/GATEAUDIT_LOGGING_SENSITIVE_DATA_PROTECTION_IMPLEMENTATION.md)：共享 prod console encoder 修复后 13/0 synthetic canary 泄漏，Throwable/cause/suppressed、安全字段、fail-safe 与独立审查通过；implementation pair=`48c1b1cd4c84e1429be82093e460984b6d1c805c / 35817828506 / 9 of 9 SUCCESS`。分类为 `ACCEPTED_TECHNICAL_EVIDENCE / NON_RUNTIME_AUTHORITY`；不改写前项历史负证据。
