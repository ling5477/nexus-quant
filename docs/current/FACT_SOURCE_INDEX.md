...
- GateW：`IN PROGRESS / NOT FROZEN`（进行中 / 未冻结）；GateW-1、GateW-2 为 `ACCEPTED / CI GREEN`。GateW-3 venue-rule implementation/migration conformance/fix exact-head CI 与 LIMIT-only internal order preview review 继续有效；preview implementation commit `eff79d7c7ea1b034de4e77c7ec64974c247027f5` 的 run `29308652349` 失败，当前为 `COMMITTED|CI_FAILED|FIX_REQUIRED`。E2E runner fix/review 已通过但尚未 commit，唯一下一动作是 `NQ-GATEW-3-CI-BLOCKER-FIX-COMMIT-AND-PUSH`；accepted batch 仍为 GateW-2。GateW-2 `REAL_SMOKE=NOT_RUN`；不得把本地验证、review accepted、read-only diagnostic probe、public metadata readiness、schema review、preview review 或 CI 推断为 runtime fresh facts、远端 permission、private trading、LIVE 或 trading authorization。
