...
- GateW：`IN PROGRESS / NOT FROZEN`（进行中 / 未冻结）；GateW-1、GateW-2 为 `ACCEPTED / CI GREEN`。GateW-3 venue-rule implementation/migration conformance/fix exact-head CI 继续有效，LIMIT-only internal order preview 已通过 security/risk 与独立 implementation review，当前为 `REVIEW_ACCEPTED|READY_TO_COMMIT`，尚未整体 accepted；唯一下一动作是 `NQ-GATEW-3-LIMIT-ONLY-DRY-RUN-ORDER-PREVIEW-COMMIT-AND-PUSH`。GateW-2 `REAL_SMOKE=NOT_RUN`；不得把本地验证、review accepted、read-only diagnostic probe、public metadata readiness、schema review、preview review 或 CI 推断为 runtime fresh facts、远端 permission、private trading、LIVE 或 trading authorization。
