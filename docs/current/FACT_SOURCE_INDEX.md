...
9. [GATER_PLAN.md](GATER_PLAN.md)：GateR historical planning 和 GateR-2..8 implementation 指针；GateR 当前已被 freeze closeout superseded，不再作为下一阶段入口。
10. [API.md](API.md)：已实现 HTTP API 当前事实；GateR-6 / GateR-8 Shadow Run API 只包含 read-only GET endpoint，不包含写接口；GateS-1 当前 backend 只实现 `GET /api/shadow-runs/overview` 最小 read model，不包含 GateS 全域 overview 或写接口。
11. [TESTING.md](TESTING.md)：当前验证记录和未运行说明；GateR frozen baseline、GateS-0 docs-only validation、GateS-1 work order docs/read-only validation、GateS-1 backend implementation Maven validation、GateS-1 frontend overview work order docs/read-only validation 和 GateS-1 frontend overview implementation build validation 证据以此为当前验证入口。
12. [WORKLOG.md](WORKLOG.md)：当前任务记录；GateR frozen baseline、GateS-0 docs-only scope、GateS-1 work order scope、GateS-1 backend implementation scope、GateS-1 frontend overview work order scope、GateS-1 frontend overview implementation scope 和边界以此为工作记录入口。
13. [DB_SCHEMA.md](DB_SCHEMA.md)：已落地 Flyway schema 当前事实；GateR-2 的 `V32__gate_r_shadow_run_fact_model.sql` 和 4 张 Shadow Run local fact 表以此为当前 schema 入口；GateS-1 frontend overview implementation 不新增 schema。
14. [GATER_1_SHADOW_RUN_DATA_MODEL_MIGRATION_PLAN_REVIEW.md](GATER_1_SHADOW_RUN_DATA_MODEL_MIGRATION_PLAN_REVIEW.md)：GateR-1 Shadow Run data model / migration plan review；只表示 migration 方案曾经就绪，不代表当前要新增 migration。
...
- GateS-1 work order 当前为 `PLAN READY / NOT IMPLEMENTED / READY TO COMMIT`；GateS-1 minimal backend read model 当前为 `IMPLEMENTED / SELF-REVIEWED / READY TO COMMIT`；GateS-1 frontend overview implementation 当前为 `IMPLEMENTED / SELF-REVIEWED / READY TO COMMIT`。不得误写成 GateS-1 frozen / accepted、GateS 全域 validation runtime 已实现、Dashboard v2 started 或交易授权。
