...
- Attempt-10：`NOT_CREATED / START_BLOCKED`（未创建 / 启动已阻断）。Canonical pre-create 返回 `readyForAttemptCreation=false`；没有 RunId、clock、unit 或 OKX call。不得直接重试、用新 Attempt/重置 clock/重跑 finalizer或其他服务器修改绕过 hard gate 与 Attempt-09 拒绝事实。
...
治理 authority 中唯一下一动作精确为 `NQ-GATEW-ATTEMPT-10-PRECREATE-PREREQUISITE-INTERNAL-READBACK-FAILURE-RCA-AND-FIX`。该动作只允许对 Commit A 在生产 canonical readback 中仍返回 `INTERNAL_SANITIZED_READBACK_FAILURE` 做脱敏代码级 RCA、最小修复与回归；不得直接重试 Attempt-10，不得修改生产 credential、permission、IP allowlist 或数据库，不得绕过 helper 使用手工 SQL/ad-hoc 调用。后续重新部署与 Attempt-10 启动均须另行授权；不授权扩大 OKX endpoint、LIVE、交易写侧或进入 freeze/archive/tag。
