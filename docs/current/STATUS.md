...
- GateW-ATTEMPT-10-RELEASE-CANDIDATE-STABILIZATION-FIX：`IMPLEMENTED / CI GREEN / RC REVIEW PENDING`（已实现 / CI 已通过 / 待 RC 独立审查）。Commit A `8db984f3369cec7b6f66d613daa13651e211b682` 的 exact-head CI run `30607922128` 与跨 OS LF checkout follow-up `ef803568ed56905cb9969477e1ad777d5a01faf6` 的 exact-head run `30616271884` 均为 `completed / success / 10 of 10`。唯一新 RC source 为 `ef803568...`；Windows PowerShell 5.1/7 与 no-egress Linux PowerShell 7 三次 exact build 的 manifest/bundle/artifact descriptors 精确相同，manifest SHA-256 `ba5f9c0536c3bc142ff6e44f194f12ab3ed29935e432b23e33ec55ed709752f5`、bundle SHA-256 `75ef45cf0d61cd10be76992b981f6b4ebfb3418ad19e66931a286f79016a7c17`、131 artifacts、122 JAR、132 USTAR、4 个允许且已计数的重复目录 entry。Linux Java 21/POSIX verifier、Java 17 mismatch、30s process-tree cleanup、51-test focused Maven、32/12/23 regression 与 tamper rejection 均通过。该 RC 只标记 `RC_READY_FOR_REVIEW / PRODUCTION_DEPLOYMENT_NOT_STARTED`，未被接受或部署。
...
- Attempt-10：`NOT_CREATED / NOT_AUTHORIZED`（未创建 / 未授权）。旧 RC 已因 4 个 P1 与 1 个 P2 拒绝；新整改 RC 仅达到 `RC_READY_FOR_REVIEW`，尚未独立接受。本轮未连接或修改生产，没有 RunId、clock、unit 或 OKX call。不得直接重试、用新 Attempt/重置 clock/重跑 finalizer或其他服务器修改绕过 hard gate、RC review 与 Attempt-09 拒绝事实。
...
治理 authority 中唯一下一动作精确为 `NQ-GATEW-ATTEMPT-10-RELEASE-CANDIDATE-STABILIZATION-REVIEW`。该动作只允许对 source commit `ef803568ed56905cb9969477e1ad777d5a01faf6`、manifest `ba5f9c05...52f5` 与 bundle `75ef45cf...7c17` 做独立 RC review；不得复用旧 RC 的接受结论，也不得把本修复任务的 self-review 代替独立审查。不得连接生产、部署 release、重试 Attempt-10、创建 RunId/clock/worker，亦不得修改 credential、permission、IP allowlist 或生产数据库；不授权扩大 OKX endpoint、LIVE、交易写侧或进入 freeze/archive/tag。
