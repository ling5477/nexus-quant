...
accepted_batch=GateX-0A
accepted_batch_status=ACCEPTED|CI_GREEN
accepted_batch_implementation_commit=49851276cdbbe7daf49506cc3af327e42973788b
accepted_batch_acceptance_head=61d9292b0b77d9c25f36232bee9512b87ac256c6
accepted_batch_ci_run=31318868410
work_batch=GateX-0B
work_batch_status=IMPLEMENTED|SELF_REVIEWED
work_batch_commit=UNCOMMITTED
work_batch_ci_run=NOT_RUN
next_action=NQ-GATEX-0B-COMMIT-AND-PUSH
...
- GateX-0B：`IMPLEMENTED / SELF REVIEWED / UNCOMMITTED`（已实现 / 已自审 / 尚未提交）；生产 class/package 已改用稳定 capability/domain naming，新配置 key 与 `nq.gatew.*` legacy alias 的确定性兼容已覆盖测试；模块级与全后端 Maven 回归均通过。未改变公开 API、数据库、交易状态机、LIVE 或真实交易边界。
...
GateX-0A 已由 exact-head CI 接受；GateX-0B 已实现、自审并完成本地回归，但尚未提交且 CI 未运行。治理 authority 中唯一下一动作精确为 `NQ-GATEX-0B-COMMIT-AND-PUSH`；不得进入 GateX-0C 或 Strategy Release productionization，不得创建 Attempt-14、开启 LIVE、下单、撤单、转账或提现。
