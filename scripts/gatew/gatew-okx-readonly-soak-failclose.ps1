...
function Assert-RunId {
    ...
    if ($Value -cnotmatch $script:RunIdPattern) { throw 'BLOCKED / RUN_ID_INVALID' }
}
...
function Invoke-Native {
    ...
    if (-not $AllowFailure -and $exitCodeValue -ne 0) { throw 'FAIL / NATIVE_COMMAND_FAILED' }
...
function Write-TextCreateOnce {
    ...
    if (Test-Path -LiteralPath $Path) { throw 'BLOCKED / IMMUTABLE_CONTROL_EXISTS' }
...
function Assert-NoProviderMaterial {
    ...
    if ([string]::IsNullOrWhiteSpace($credentialRoot)) { throw 'BLOCKED / FAILCLOSE_CREDENTIAL_MISSING' }
...
function Assert-FrozenConfig {
    ...
    if ([string]$Config.databaseUrl -notmatch '^jdbc:postgresql://(127\.0\.0\.1|localhost):[0-9]{1,5}/[A-Za-z0-9_]*(gatew|soak)[A-Za-z0-9_]*$' -or
        ...
        ([string]$Config.runMode -eq 'REAL' -and [string]$Config.databaseSchema -ne 'public') -or
        ([string]$Config.runMode -eq 'OFFLINE_ACCEPTANCE' -and
...
function Invoke-RecoveryLauncher {
    ...
    $launcherResult = $null
    try { $null = & $script:JavaPath @arguments 2>$null } catch {
...
function Stop-AndObserveWorker {
    ...
    do {
        $state = Get-UnitState
        if ($state.MainPID -eq 0 -and $state.ActiveState -in @('inactive', 'failed')) { break }
...
function Get-HistoricalEvidenceSnapshot {
    ...
    foreach ($root in $roots) {
        ...
        foreach ($file in Get-ChildItem -LiteralPath $root -File -Recurse -ErrorAction Stop) {
            if ($file.FullName -like "$(Get-RunRoot $RunId)*") { continue }
...
function Get-EvidenceState {
    ...
    foreach ($line in Get-Content -LiteralPath $samplesPath) {
        ...
        if ($sequence -le $lastSequence) { throw 'FAIL / EVIDENCE_SEQUENCE_INVALID' }
        $lastSequence = $sequence
        if ([string]$sample.resultStatus -eq 'PASSED_READ_ONLY') { $lastSuccessful = $sequence }
...
function Assert-ExitFact {
    ...
    if ($null -eq $ExitFact) { throw 'BLOCKED / SYSTEMD_EXIT_FACT_MISSING' }
...
function Resolve-TerminalDecision {
    ...
    try { Assert-ExitFact $ExitFact } catch {
    ...
    if ($null -ne $Completion) {
        ...
        if ([string]$Config.runMode -ne 'REAL' -or [string]$Completion.runId -ne $RunId -or
    ...
    if ([string]$Config.runMode -eq 'OFFLINE_ACCEPTANCE' -and
...
function Set-LifecycleState {
    ...
    if ($currentState -eq $NextState) { return }
...
function Complete-Lifecycle {
    ...
    if ($current -eq $TerminalStatus) { return }
...
function Validate-ExistingTerminal {
    ...
    if ([string]$Terminal.schemaVersion -ne 'gatew-soak-terminal-v1' -or
        ...
        [bool]$Terminal.acceptanceClockStarted -or [bool]$Terminal.credentialAccessed -or
...
function Finalize-FormalRun {
    ...
    try {
        ...
        if ($configValid) {
            try { $recovery = Invoke-BoundedRecovery $config } catch {
                $recovery = New-RecoveryFailure 'ENGAGE_STATUS_UNKNOWN'
            }
        }
        ...
        try { $worker = Stop-AndObserveWorker } catch {
        ...
        try { $historicalImmutable = Test-HistoricalEvidenceImmutable } catch {
            $historicalImmutable = $false
        }
        ...
        $workerStart = $null
        try { $workerStart = Read-OptionalJson "$(Get-EvidenceRoot $RunId)/worker-start.json" } catch {
        ...
        $manifestSha256 = if (Test-Path -LiteralPath $manifestPath -PathType Leaf) {
            try { Get-Sha256File $manifestPath } catch { 'UNKNOWN' }
        }
        else { 'UNKNOWN' }
        $finalChainHash = if ($null -eq $evidenceVerified) { 'UNKNOWN' } else { [string]$evidenceVerified.lastHash }
        $terminal = [ordered]@{
...
function Invoke-FailCloseSelfTest {
    ...
    if ($missingExitDecision.Status -ne 'BLOCKED') { throw 'missing exit fact self-test failed' }
    $caseCount++
    ...
    try {
        ...
        $rejected = $false
        try { Write-TextCreateOnce $path '{"terminalStatus":"OPERATOR_STOPPED"}' } catch { $rejected = $true }
        if (-not $rejected -or
        ...
        try { Assert-ExistingTerminalReturnable $blockedTerminal } catch {
            $blockedRetryRejected = $_.Exception.Message -eq 'FAIL / FAILCLOSE_SAFETY_NOT_PROVEN'
        }
        if (-not $blockedRetryRejected) { throw 'existing BLOCKED retry self-test failed' }
        $caseCount++
    }
    finally {
...
try {
    $result = switch ($Action) {
        'finalize' { Finalize-FormalRun }
        'self-test' { Invoke-FailCloseSelfTest }
    }
