...
DECLARE
    ...
    v_alt_exchange_account_id BIGINT;
...
BEGIN
    ...
    INTO v_conflict_owner_user_id
    FROM exchange_accounts
    WHERE exchange_account_id = 900002
      AND owner_user_id <> v_owner_user_id
    LIMIT 1;

    IF v_conflict_owner_user_id IS NOT NULL THEN
        RAISE EXCEPTION 'exchange_account_id=900002 already belongs to owner_user_id=%', v_conflict_owner_user_id;
    END IF;

    SELECT exchange_account_id
    INTO v_alt_exchange_account_id
    FROM exchange_accounts
    WHERE owner_user_id = v_owner_user_id
      AND (
          exchange_account_id = 900002
          OR legacy_account_id = 900002
          OR (
              exchange_code = 'OKX'
              AND trade_env = 'SIM'
              AND account_alias = 'rc1-admin-alt'
          )
      )
    ORDER BY CASE WHEN exchange_account_id = 900002 THEN 0 ELSE 1 END, exchange_account_id
    LIMIT 1;

    IF v_alt_exchange_account_id IS NULL THEN
        INSERT INTO exchange_accounts (
            exchange_account_id,
            owner_user_id,
            exchange_code,
        ...
        RETURNING exchange_account_id INTO v_alt_exchange_account_id;
    ELSE
        UPDATE exchange_accounts
        SET exchange_account_id = 900002,
            exchange_code = 'OKX',
            trade_env = 'SIM',
            account_alias = 'rc1-admin-alt',
            status = 'ACTIVE',
            legacy_account_id = 900002,
            is_default = FALSE,
            updated_at = NOW()
        WHERE exchange_account_id = v_alt_exchange_account_id;
    END IF;
    ...
    PERFORM setval(
        ...
        GREATEST(
            900002,
            COALESCE((SELECT MAX(exchange_account_id) FROM exchange_accounts), 900002)
        ),
        TRUE
    );
